...
import systematic_trading_lab.program_017_ohlcv_authority as authority
...
def test_program_017_runtime_implementation_is_bound_and_non_authorizing() -> None:
    ...
    assert binding["source_commit"] == "efcf5ef2abf56167be7f3741c10a98e2acf3e5f8"
    assert binding["source_tree"] == "6bd870133b606d2232e00146cfe4777709a4888c"
...
def _open_program_017_root(repository: Path) -> int:
...
def _child_identity(*, extra_capability: str | None = None) -> dict[str, Any]:
    ...
    return {
        ...
        "program_id": authority.PROGRAM_ID,
        "operation_manifest": authority.OPERATION_MANIFEST,
        "runtime_entrypoint": "src/systematic_trading_lab/program_017_ohlcv_authority.py",
...
def _write_program_016_page(
    descriptor: int,
    active: dict[str, Any],
    state: program_016._PredecessorState,
    request: program_011.SessionRequest,
    *,
    complete: bool,
    ...
    predecessor._append(
        descriptor,
        f"{prefix}.intent.json",
        program_016._intent_payload(active, state, request, intent),
    )
    ...
    if not complete:
        return
    receipt = program_016._response_receipt(
        active,
        state,
        request,
        intent,
...
def _derive_synthetic_predecessor(
    ...
    *,
    ...
    source_commit = "9" * 40
    p15_state = _predecessor_state(requests[0]).program_015_state
    historical_identity = {
    ...
    for root in (
        ...
        path.chmod(0o700)
    root = tmp_path / authority.PROGRAM_016_PRIVATE_ROOT
    descriptor = authority._open_root(tmp_path, authority.PROGRAM_016_PRIVATE_ROOT, create=False)
    predecessor._append(
        descriptor, "active-authority.json", (canonical_json(active_016) + "\n").encode()
    )
    predecessor._append(descriptor, "predecessor-import-manifest.json", p15_state.payload)
    if mutation == "partial-predecessor":
        _write_program_016_page(descriptor, active_016, p15_state, requests[0], complete=False)
    else:
        _write_program_016_page(descriptor, active_016, p15_state, requests[0], complete=True)
        if mutation == "page-1-frontier":
            _write_program_016_page(
                descriptor,
                active_016,
                p15_state,
                requests[1],
            ...
            predecessor._append(
                descriptor,
                f"{prefix}.intent.json",
                program_016._intent_payload(active_016, p15_state, requests[1], intent),
            )
    ...
    if mutation == "later-evidence":
        _write_program_016_page(descriptor, active_016, p15_state, requests[2], complete=False)
    monkeypatch.setattr(science, "acquisition_requests", lambda: requests)
    monkeypatch.setattr(
        authority,
        "validate_operation_contract",
        lambda *_args, **_kwargs: {
            "predecessor": {"program_016_terminal_result": {"path": "terminal", "sha256": "x"}}
        },
    )
    monkeypatch.setattr(
        authority,
        "_load_bound",
        lambda *_args, **_kwargs: {
            "authority_fingerprint": active_016["authority_fingerprint"],
            "source_commit": source_commit,
        },
    )
    monkeypatch.setattr(
        authority,
        "_historical_program_016_identity",
        lambda *_args: historical_identity,
    )
    monkeypatch.setattr(program_016, "validate_operation_contract", lambda *_args, **_kwargs: {})
    monkeypatch.setattr(program_016, "_derive_predecessor_state", lambda *_args: p15_state)
    ...
    monkeypatch.setattr(program_016, "_public_terminal_payload", lambda *_args: b"public\n")
...
def test_operation_contract_revalidates_program_017_v5() -> None:
...
def test_exact_predecessor_prefix_and_frontier_are_rederived(
    ...
    assert len(state.manifest["program_016_completed_sessions"]) == 1
    assert len(state.manifest["discarded_program_016_partial_session"]["completed_pages"]) == 1
...
def test_invalid_predecessor_shapes_fail_before_reuse(
    ...
    with pytest.raises(authority.Program017AuthorityError, match=message):
...
def test_control_lock_order_uses_only_the_fixed_roots(
    ...
    def locked(_descriptor: int) -> Iterator[None]:
        events.append("program-017-enter")
        held["program-017"] = True
        try:
            yield
        finally:
            held["program-017"] = False
            events.append("program-017-exit")
    ...
    with authority._locked_controls(tmp_path, _active_authority(), create_program_017=True) as (
        program_017_root,
        program_016_root,
        program_015_root,
        program_014_root,
...
def test_overbroad_child_fails_before_credentials_or_private_roots(
    ...
    with pytest.raises(authority.Program017AuthorityError, match="reviewed child identity"):
...
def test_protected_overlap_fails_before_every_private_root_entrypoint(
    ...
    monkeypatch.setattr(
        ...
        lambda *_args: (_ for _ in ()).throw(
            authority.Program017AuthorityError(
                "Program 017 request chronology overlaps protected data"
            )
        ),
    )
    monkeypatch.setattr(authority, "_open_root", lambda *_args, **_kwargs: opened.append(True))
    ...
    for operation in (
        ...
        with pytest.raises(authority.Program017AuthorityError, match="overlaps protected"):
            operation()
...
def test_runtime_source_precedes_exact_child_and_review_topology(tmp_path: Path) -> None:
    ...
    _git(tmp_path, "config", "user.name", "Program 017 Test")
    _git(tmp_path, "config", "user.email", "program-017@example.invalid")
    ...
    with pytest.raises(authority.Program017AuthorityError, match="synchronized-main lineage"):
...
def test_transport_and_closeout_use_metadata_without_predecessor_reconstruction(
    ...
    authority._revalidate_transport_boundary(tmp_path, 1, 2, 3, 4, 5, 6, active, state)
    authority._revalidate_closeout_boundary(tmp_path, 1, 2, 3, 4, 5, 6, active, state)
    ...
    with pytest.raises(authority.Program017AuthorityError, match="authority changed"):
        authority._revalidate_transport_boundary(tmp_path, 1, 2, 3, 4, 5, 6, active, state)
    with pytest.raises(authority.Program017AuthorityError, match="authority changed"):
        authority._revalidate_closeout_boundary(tmp_path, 1, 2, 3, 4, 5, 6, active, state)
...
def test_mutable_root_tracker_accepts_tracked_writes_and_rejects_unaccounted_drift(
    ...
    try:
        ...
        with pytest.raises(authority.Program017AuthorityError, match="mutable root changed"):
            snapshot.validate_continuation()
    finally:
...
def test_closeout_rejection_precedes_private_success_terminal(
    ...
    monkeypatch.setattr(
        ...
        lambda *_args: (_ for _ in ()).throw(
            authority.Program017AuthorityError("Program 017 closeout rejected")
        ),
    )

    with pytest.raises(authority.Program017PostClaimPersistenceError):
        authority._execute_mock_acquisition(
            tmp_path,
            environ=_credentials(),
            transport=authority.MockBarsTransport(
...
def test_crash_after_durable_activation_seals_before_unlock(
    ...
    assert private["program_017_credential_loads"] == 0
...
def test_crash_after_body_seals_without_a_second_transport(
    ...
    assert private["program_017_credential_loads"] == 1
    assert private["cumulative_request_intents"] == 6
    assert private["cumulative_responses"] == 0
    assert len(transport.intents) == 1
    with pytest.raises(authority.Program017AuthorityError, match="terminally sealed"):
...
def _concurrent_recovery_worker(
    ...
    try:
        start.wait()
        with authority._locked_controls(Path(repository), active, create_program_017=False) as (
            descriptor,
...
def test_two_processes_serialize_one_credential_and_frontier_transport(
    ...
    # Program 017 is explicitly single-process: a launcher checkpoint created
    # by one process must reject every separately launched process before
    # credential access or provider transport.
    assert all(
        error in {"Program017AuthorityError", "_IncompletePageCheckpoint"} and pages == 0
...
def test_launcher_rejects_relaunch_or_restart_policy_change(tmp_path: Path, mutation: str) -> None:
    ...
    with pytest.raises(authority.Program017AuthorityError, match="launcher changed"):
...
def test_private_terminal_rejects_boolean_strategy_zero(
    ...
    try:
        with pytest.raises(authority.Program017AuthorityError, match="private terminal binding"):
...
def test_credential_sequences_reject_invalid_evidence(tmp_path: Path, mutation: str) -> None:
    ...
    with pytest.raises(authority.Program017AuthorityError, match="credential"):
...
def test_unpaired_attempt_counts_one_load_and_latch_blocks_second_access(
    ...
    with pytest.raises(authority.Program017AuthorityError, match="already consumed"):
...
def test_parse_failure_receipt_precedes_propagation_and_consumes_latch(tmp_path: Path) -> None:
    ...
    with pytest.raises(authority.Program017AuthorityError, match="credentials are required"):
...
def test_combined_budget_counts_predecessor_frontier_and_program_017_pages() -> None:
    state = _predecessor_state(completed_responses=22_171)
    budget = authority._Budget(state)

    assert (budget.requests, budget.responses) == (22_176, 22_171)
...
def test_exhausted_inherited_envelope_seals_without_credentials_or_transport(
    ...
    _, request, _ = _configure_finite_execution(
        tmp_path,
        monkeypatch,
        completed_responses=science.MAXIMUM_REQUESTS_AND_RESPONSES - 5,
    )
    transport = authority.MockBarsTransport(
        [raw_contract.RawResponse(200, _body(request, 0, None))]
    )
    ...
    assert private["cumulative_responses"] == 22_171
    assert private["provider_transport_attempted"] is False
...
def test_invalid_public_terminal_fields_fail_before_credentials_or_private_roots(
    ...
    with pytest.raises(authority.Program017AuthorityError):
        authority.credential_presence_preflight(tmp_path, environ={})
...
def test_every_lifecycle_entrypoint_rejects_an_exact_public_terminal_first(
    ...
    for operation in (
        ...
        with pytest.raises(authority.Program017AuthorityError, match="terminally revoked"):
            operation()
...
def test_valid_shaped_terminal_without_immutable_binding_rejects_before_credentials(
    ...
    with pytest.raises(authority.Program017AuthorityError, match="lacks immutable binding"):
...
def test_missing_terminal_rejects_before_credentials_or_private_roots(
    ...
    for operation in (
        ...
        with pytest.raises(authority.Program017AuthorityError, match="artifact is absent"):
            operation()
...
def test_changed_terminal_observation_time_rejects_before_credentials(
    ...
    with pytest.raises(authority.Program017AuthorityError, match="semantics differ"):
        authority.credential_presence_preflight(_REPOSITORY, environ={})
...
def test_atomic_publication_never_overwrites_different_public_bytes(
    ...
    with pytest.raises(authority.Program017AuthorityError, match="artifact differs"):
        authority._append_public_atomic(tmp_path, descriptor, payload)
...
def test_repository_terminal_bytes_exactly_equal_cli_output(
    ...
    monkeypatch.setattr(
        base_cli,
        "load_dotenv",
        lambda: pytest.fail("Program 017 CLI loaded dotenv"),
    )
    monkeypatch.setenv("TRADING_LAB_MODE", "research")

    assert dispatcher.main(("data", "acquire", "program-017-ohlcv", "run")) == 0
...
def test_secret_guard_allows_reserved_program_017_control_artifacts_only(
    ...
    observation = Path("config/research/program-017-market-observations.json")
    private = Path(".trading-lab/program-017-private-terminal.json")
