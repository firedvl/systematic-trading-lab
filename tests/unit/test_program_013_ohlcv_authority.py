...
import systematic_trading_lab.program_013_ohlcv_authority as authority
...
def test_program_013_runtime_implementation_is_exact_and_non_authorizing() -> None:
...
def _make_private_roots(repository: Path) -> None:
    for root in (authority.PRIVATE_ROOT, authority.PREDECESSOR_PRIVATE_ROOT):
        path = repository / root
        path.mkdir(parents=True, mode=0o700, exist_ok=True)
        path.chmod(0o700)
    predecessor_lock = repository / authority.PREDECESSOR_PRIVATE_ROOT / "run.lock"
    predecessor_lock.touch(mode=0o600, exist_ok=True)
    predecessor_lock.chmod(0o600)


def _open_program_013_root(repository: Path) -> int:
...
def _child_identity(*, extra_capability: str | None = None) -> dict[str, Any]:
    ...
    return {
        ...
        "program_id": authority.PROGRAM_ID,
        "operation_manifest": authority.OPERATION_MANIFEST,
        "runtime_entrypoint": "src/systematic_trading_lab/program_013_ohlcv_authority.py",
...
def _make_program_012_active(source_commit: str) -> dict[str, Any]:
    unsigned: dict[str, Any] = {
        "schema_version": "program-012-raw-sip-acquisition-active-authority-v1",
        "status": "ACTIVE-ONE-USE-RECOVERABLE",
        "authority_id": predecessor.CHILD_AUTHORITY_ID,
        "program_id": predecessor.PROGRAM_ID,
        "operation_manifest": predecessor.OPERATION_MANIFEST,
        "control_lineage": {"runtime_source_commit": source_commit},
    }
    return {**unsigned, "authority_fingerprint": fingerprint(unsigned)}


def _write_program_012_page(
    descriptor: int,
    active: dict[str, Any],
    request: program_011.SessionRequest,
    *,
    ...
    source_commit = str(active["control_lineage"]["runtime_source_commit"])
    intent = program_011.PageIntent(request.identity, 1, request.url(), None)
    prefix = predecessor._page_prefix(request, 1)
    predecessor._append(
        descriptor,
        f"{prefix}.intent.json",
        predecessor._intent_payload(active, source_commit, request, intent),
    )
    ...
    if not complete:
        return
    receipt = predecessor._response_receipt(
        active,
        source_commit,
        request,
        intent,
...
def _derive_synthetic_predecessor(
    ...
    *,
    ...
    source_commit = "9" * 40
    active_012 = _make_program_012_active(source_commit)
    root = tmp_path / authority.PREDECESSOR_PRIVATE_ROOT
    root.mkdir(parents=True, mode=0o700)
    root.chmod(0o700)
    descriptor = authority._open_root(tmp_path, authority.PREDECESSOR_PRIVATE_ROOT, create=False)
    predecessor._append(
        descriptor, "active-authority.json", (canonical_json(active_012) + "\n").encode()
    )
    if mutation == "partial-predecessor":
        _write_program_012_page(descriptor, active_012, requests[0], complete=False)
    else:
        _write_program_012_page(descriptor, active_012, requests[0], complete=True)
        _write_program_012_page(
            descriptor,
            active_012,
            requests[1],
            complete=False,
    ...
    if mutation == "later-evidence":
        _write_program_012_page(descriptor, active_012, requests[2], complete=False)
    monkeypatch.setattr(science, "acquisition_requests", lambda: requests)
    monkeypatch.setattr(
        authority,
        "validate_operation_contract",
        lambda *_args, **_kwargs: {
            "predecessor": {"program_012_terminal_result": {"path": "terminal", "sha256": "x"}}
        },
    )
    monkeypatch.setattr(
        authority,
        "_load_bound",
        lambda *_args, **_kwargs: {
            "authority_fingerprint": active_012["authority_fingerprint"],
    ...
    monkeypatch.setattr(
        ...
        lambda *_args: (
            "terminal-failure.json",
            {
                "status": "FAIL-CONSUMED-NO-RETRY",
                "admission_passed": False,
                "terminal_fingerprint": "8" * 64,
            },
        ),
    )
    try:
        return authority._derive_predecessor_state(tmp_path, descriptor, _active_authority())
    finally:
        os.close(descriptor)


def test_operation_contract_revalidates_program_013_v5() -> None:
...
def test_exact_predecessor_prefix_and_frontier_are_rederived(
    ...
    assert len(state.manifest["completed_sessions"]) == 1
...
@pytest.mark.parametrize(
    ...
    (
        ...
        ("changed-checkpoint", "reviewed first-page intent-only frontier"),
...
def test_invalid_predecessor_shapes_fail_before_reuse(
    ...
    with pytest.raises(authority.Program013AuthorityError, match=message):
...
def test_control_lock_order_uses_only_the_fixed_roots(
    ...
    held = {"program-013": False, "program-012": False, "git": False}
    ...
    def locked(_descriptor: int) -> Iterator[None]:
        events.append("program-013-enter")
        held["program-013"] = True
        try:
            yield
        finally:
            held["program-013"] = False
            events.append("program-013-exit")

    @contextmanager
    def read_locked(_descriptor: int) -> Iterator[None]:
        events.append("program-012-enter")
        held["program-012"] = True
        try:
            yield
        finally:
            held["program-012"] = False
            events.append("program-012-exit")
    ...
    with authority._locked_controls(tmp_path, _active_authority(), create_program_013=True) as (
        program_013_root,
        program_012_root,
    ):
    ...
    assert opened == [
        ...
        (authority.PREDECESSOR_PRIVATE_ROOT, False),
    ]
    assert events == [
        "program-013-enter",
        "program-012-enter",
...
def test_overbroad_child_fails_before_credentials_or_private_roots(
    ...
    with pytest.raises(authority.Program013AuthorityError, match="reviewed child identity"):
...
def test_protected_overlap_fails_before_every_private_root_entrypoint(
    ...
    monkeypatch.setattr(
        ...
        lambda *_args: (_ for _ in ()).throw(
            authority.Program013AuthorityError(
                "Program 013 request chronology overlaps protected data"
            )
        ),
    )
    monkeypatch.setattr(authority, "_open_root", lambda *_args, **_kwargs: opened.append(True))
    ...
    for operation in (
        ...
        with pytest.raises(authority.Program013AuthorityError, match="overlaps protected"):
            operation()
...
def test_runtime_source_precedes_exact_child_and_review_topology(tmp_path: Path) -> None:
    ...
    _git(tmp_path, "config", "user.name", "Program 013 Test")
    _git(tmp_path, "config", "user.email", "program-013@example.invalid")
    ...
    with pytest.raises(authority.Program013AuthorityError, match="synchronized-main lineage"):
...
def test_transport_and_closeout_revalidate_authority_predecessor_and_firewall(
    ...
    authority._revalidate_transport_boundary(tmp_path, 1, 2, active, state)
    authority._revalidate_closeout_boundary(tmp_path, 2, active, state)

    changed = dict(active)
    changed["authority_fingerprint"] = "0" * 64
    monkeypatch.setattr(authority, "_derive_control_validated_authority", lambda _repo: changed)
    with pytest.raises(authority.Program013AuthorityError, match="authority changed"):
        authority._revalidate_transport_boundary(tmp_path, 1, 2, active, state)
    with pytest.raises(authority.Program013AuthorityError, match="authority changed"):
        authority._revalidate_closeout_boundary(tmp_path, 2, active, state)
    ...
    with pytest.raises(authority.Program013AuthorityError, match="evidence changed"):
        authority._revalidate_transport_boundary(tmp_path, 1, 2, active, state)
    with pytest.raises(authority.Program013AuthorityError, match="evidence changed"):
        authority._revalidate_closeout_boundary(tmp_path, 2, active, state)
...
def test_closeout_rejection_precedes_private_success_terminal(
    ...
    monkeypatch.setattr(
        ...
        lambda *_args: (_ for _ in ()).throw(
            authority.Program013AuthorityError("Program 013 closeout rejected")
        ),
    )

    with pytest.raises(authority.Program013PostClaimPersistenceError):
...
def test_crash_after_intent_seals_without_credentials_or_transport(
    ...
    assert private["program_013_credential_loads"] == 0
    ...
    with pytest.raises(authority.Program013AuthorityError, match="terminally sealed"):
        authority._execute_mock_acquisition(
            tmp_path,
            environ=_credentials(),
            transport=authority.MockBarsTransport([]),
...
def test_crash_after_body_seals_without_a_second_transport(
    ...
    assert private["program_013_credential_loads"] == 1
    assert private["cumulative_request_intents"] == 2
    assert private["cumulative_responses"] == 0
...
def test_completed_page_replays_locally_and_resumes_only_the_continuation(
    ...
    descriptor = _open_program_013_root(tmp_path)
...
def _concurrent_recovery_worker(
    ...
    try:
        start.wait()
        with authority._locked_controls(Path(repository), active, create_program_013=False) as (
            descriptor,
            _predecessor_descriptor,
        ):
...
def test_two_processes_serialize_one_credential_and_frontier_transport(
    ...
    assert all(error is None and pages == 1 for error, pages, _ in results)
    assert sum(transports for _, _, transports in results) == 1
...
def test_credential_sequences_reject_invalid_evidence(tmp_path: Path, mutation: str) -> None:
    ...
    with pytest.raises(authority.Program013AuthorityError, match="credential"):
...
def test_unpaired_attempt_counts_one_load_and_latch_blocks_second_access(
    ...
    with pytest.raises(authority.Program013AuthorityError, match="already consumed"):
...
def test_combined_budget_counts_predecessor_frontier_and_program_013_pages() -> None:
    state = _predecessor_state(completed_responses=22_174)
    budget = authority._Budget(state)

    assert (budget.requests, budget.responses) == (22_175, 22_174)
    budget.reserve_request()
    intent = program_011.PageIntent(_request().identity, 1, _request().url(), None)
    budget.accept_response(_request(), intent, 200, b"{}", datetime(2026, 9, 3, tzinfo=UTC))
    assert (budget.requests, budget.responses) == (22_176, 22_175)
...
def test_exhausted_inherited_envelope_seals_without_credentials_or_transport(
    ...
    _, request, _ = _configure_finite_execution(
        tmp_path,
        monkeypatch,
        completed_responses=science.MAXIMUM_REQUESTS_AND_RESPONSES - 1,
    )
    ...
    with pytest.raises(authority.Program013AuthorityError, match="sealed failure"):
    ...
    assert private["cumulative_responses"] == 22_175
    assert private["provider_transport_attempted"] is False
...
@pytest.mark.parametrize("operation", ("preflight", "activate", "load"))
def test_lifecycle_entrypoints_seal_ambiguous_consumed_state_before_credentials(
    ...
    active, request, state = _configure_finite_execution(tmp_path, monkeypatch)
    descriptor = authority._open_root(tmp_path, authority.PRIVATE_ROOT, create=False)
    intent = program_011.PageIntent(request.identity, 1, request.url(), None)
    prefix = predecessor._page_prefix(request, 1)
    ...
    with pytest.raises(authority.Program013AuthorityError, match="terminally sealed"):
        if operation == "preflight":
            ...
            authority.activate_authority(tmp_path, environ={})
        else:
            authority.load_active_authority(tmp_path)
...
def test_every_surviving_operational_artifact_seals_before_credentials_or_transport(
    ...
    assert private["result_kind"] == "RUNTIME-FAILURE"
    assert private["provider_transport_attempted"] is False
    assert private["program_013_credential_loads"] == (1 if artifact == "credential" else 0)
    assert private["cumulative_request_intents"] <= science.MAXIMUM_REQUESTS_AND_RESPONSES
    assert private["cumulative_responses"] <= science.MAXIMUM_REQUESTS_AND_RESPONSES - 1
...
def test_preflight_keeps_completed_page_available_for_local_replay(
    tmp_path: Path, monkeypatch: MonkeyPatch
) -> None:
    active, request, state = _configure_finite_execution(tmp_path, monkeypatch)
    descriptor = authority._open_root(tmp_path, authority.PRIVATE_ROOT, create=False)
    ...
    assert authority.credential_presence_preflight(tmp_path, environ=_credentials()) == ()
    assert not (tmp_path / authority.PRIVATE_ROOT / authority._TERMINAL_KEY).exists()
...
def test_working_space_check_uses_available_bytes_not_existing_evidence(
    ...
    with pytest.raises(authority.Program013AuthorityError, match="8 GiB"):
        authority._require_working_disk_capacity(descriptor)
    os.close(descriptor)
...
def test_invalid_public_terminal_fields_fail_before_credentials_or_private_roots(
    ...
    with pytest.raises(authority.Program013AuthorityError):
        authority.credential_presence_preflight(tmp_path, environ={})
...
def test_atomic_publication_never_overwrites_different_public_bytes(
    ...
    with pytest.raises(authority.Program013AuthorityError, match="artifact differs"):
        authority._append_public_atomic(tmp_path, descriptor, payload)
...
def test_repository_terminal_bytes_exactly_equal_cli_output(
    ...
    monkeypatch.setattr(
        base_cli,
        "load_dotenv",
        lambda: pytest.fail("Program 013 CLI loaded dotenv"),
    )
    monkeypatch.setenv("TRADING_LAB_MODE", "research")

    assert dispatcher.main(("data", "acquire", "program-013-ohlcv", "run")) == 0
...
def test_secret_guard_allows_reserved_program_013_control_artifacts_only(
    ...
    spec = importlib.util.spec_from_file_location(
        "program_013_check_secrets", _REPOSITORY / "scripts/check_secrets.py"
    )
