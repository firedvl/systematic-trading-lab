...
def test_frozen_exposed_004_plan_rekeys_only_identity_and_scheduling() -> None:
    plan = load_intraday_exposed_004_plan(_REPOSITORY)
    ...
    review = _REPOSITORY / "config/research/intraday-exposed-004-plan-independent-review-v1.json"
    ...
    assert plan.periods == source.periods == source.source_plan.periods
    assert sum(len(item.neighbor_ids) for item in plan.configurations) == 140

    for current, previous, original in zip(
        plan.configurations,
        source.configurations,
        source.source_plan.configurations,
        strict=True,
    ):
        assert current.candidate_id == previous.candidate_id.replace("ie003-", "ie004-", 1)
        assert current.source_exposed_003_candidate_id == previous.candidate_id
        assert current.source_candidate_id == previous.source_candidate_id == original.candidate_id
        assert current.family_id == previous.family_id == original.family_id
        assert current.family_ordinal == previous.family_ordinal == original.family_ordinal
        assert current.parameters == previous.parameters == original.parameters
        assert current.neighbor_ids == tuple(
            neighbor.replace("ie003-", "ie004-", 1) for neighbor in previous.neighbor_ids
        )
...
def test_exposed_004_launch_and_protected_boundaries_are_fail_closed() -> None:
    plan = load_intraday_exposed_004_plan(_REPOSITORY)
    ...
    assert plan.payload["identity"]["attempt_id_format"].startswith("ie004a-")
    assert plan.june_disposition["program_effect"]["june_read_allowed"] is False
    assert not any(plan.payload["authority"].values())


def test_exposed_004_plan_rejects_changed_bytes(tmp_path: Path) -> None:
    relative = Path("config/research/intraday-exposed-004-plan-v1.json")
    ...
    with pytest.raises(ValueError, match="plan SHA-256 differs"):
        load_intraday_exposed_004_plan(tmp_path)
