...
def test_loads_exact_reviewed_campaign_two_plan_and_state_chain() -> None:
    ...
    assert [str(item.minimum_joint_relative_volume) for item in plan.configurations] == [
        "1.2",
        "1.5",
        "2",
        "1.2",
    ...
    assert plan.state["phase"] == "campaign-2-plan-reviewed-implementation-pending"
    assert (
        plan.state["campaign_terminal_evidence"] == plan.source_state["campaign_terminal_evidence"]
    )
    assert not any(plan.authority.values())


def test_loaded_campaign_two_controls_are_deeply_immutable() -> None:
    plan = load_intraday_relative_volume_drift_001_plan(_REPOSITORY)
    ...
    with pytest.raises(TypeError):
        plan.source_state["campaign_terminal_evidence"]["cohort_size"] = 1
    with pytest.raises(TypeError):
...
@pytest.mark.parametrize(
    ...
    [
        (
            "config/research/intraday-relative-volume-drift-001-plan-v1.json",
...
def test_rejects_changed_campaign_two_control_bytes(
    ...
    with pytest.raises(ValueError, match=message):
        load_intraday_relative_volume_drift_001_plan(tmp_path)
...
def test_rejects_changed_campaign_two_dependency(tmp_path: Path, relative: str) -> None:
    _frozen_repository(tmp_path)
    path = tmp_path / relative
    path.write_bytes(path.read_bytes() + b"\n")
...
def test_rejects_changed_exposed_evidence(tmp_path: Path) -> None:
    _frozen_repository(tmp_path)
    path = tmp_path / "docs/research-campaigns/intraday-exposed-005-final-report.md"
    path.write_bytes(path.read_bytes() + b"\n")

    with pytest.raises(ValueError, match="evidence SHA-256 differs"):
        load_intraday_relative_volume_drift_001_plan(tmp_path)
...
def test_plan_hash_and_parse_use_one_byte_snapshot(
    ...
    _frozen_repository(tmp_path)
    plan_path = (
        tmp_path / "config/research/intraday-relative-volume-drift-001-plan-v1.json"
    ).resolve()
    ...
    plan = load_intraday_relative_volume_drift_001_plan(tmp_path)
...
def test_rejects_duplicate_json_keys_before_semantic_loading(
    ...
    _frozen_repository(tmp_path)
    path = tmp_path / "config/research/intraday-relative-volume-drift-001-plan-v1.json"
    raw = path.read_bytes().replace(b"{", b'{"program_id":"duplicate",', 1)
    path.write_bytes(raw)
    monkeypatch.setattr(
        "systematic_trading_lab.intraday_relative_volume_drift_001_plan.PLAN_SHA256",
...
@pytest.mark.parametrize(
    ("section", "field"),
    [
        ("chronology", "baseline_update_rule"),
        ("strategy_contract", "same_clock_volume_rule"),
        ("report_contract", "terminal_semantic_validation_rule"),
    ],
)
def test_rejects_campaign_owned_semantic_change_with_recomputed_plan_hashes(
    tmp_path: Path,
    ...
    payload[section][field] = "changed after review"
    unsigned = dict(payload)
    unsigned.pop("plan_fingerprint")
    changed_fingerprint = fingerprint(unsigned)
    payload["plan_fingerprint"] = changed_fingerprint
    ...
    monkeypatch.setattr(
        "systematic_trading_lab.intraday_relative_volume_drift_001_plan.PLAN_FINGERPRINT",
        changed_fingerprint,
    )

    with pytest.raises(ValueError, match="plan semantic fingerprint differs"):
        load_intraday_relative_volume_drift_001_plan(tmp_path)
