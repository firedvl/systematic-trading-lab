...
import systematic_trading_lab.program_018_ohlcv_authority as program_018
import systematic_trading_lab.program_019_ohlcv_authority as authority
...
def _predecessor_state(
    ...
    *,
    ...
    program_017_manifest: dict[str, Any] = {
        **program_016_manifest,
        "program_017_public_terminal": {},
        "program_017_active_authority_fingerprint": "a" * 64,
        "program_017_child_identity_fingerprint": "b" * 64,
        "program_017_source_commit": "c" * 40,
        "program_017_private_terminal_fingerprint": "d" * 64,
        "program_017_completed_sessions": [],
        "consumed_intent_frontiers": [program_012_manifest["frontier"]] * 6,
    ...
    program_017_payload = (canonical_json(program_017_manifest) + "\n").encode()
    program_017_state = program_018._PredecessorState(
        manifest=program_017_manifest,
        payload=program_017_payload,
        sha256=hashlib.sha256(program_017_payload).hexdigest(),
    ...
    manifest: dict[str, Any] = {
        ...
        "discarded_program_018_partial_session": {"completed_pages": []},
        "frontier": {
            "session": request.session.isoformat(),
            "page_index": 1,
            "request_identity": request.identity,
...
def _make_private_roots(repository: Path) -> None:
    for root in (
        ...
        authority.PROGRAM_018_PRIVATE_ROOT,
...
def _open_program_019_root(repository: Path) -> int:
...
def _child_identity(*, extra_capability: str | None = None) -> dict[str, Any]:
    ...
    return {
        ...
        "program_id": authority.PROGRAM_ID,
        "operation_manifest": authority.OPERATION_MANIFEST,
        "runtime_entrypoint": "src/systematic_trading_lab/program_019_ohlcv_authority.py",
...
def _configure_finite_execution(
    ...
    *,
    ...
    proposal_019 = {
        "predecessor": {
            "program_018_terminal_result": {
                "path": program_018.PUBLIC_TERMINAL_PATH.as_posix(),
                "sha256": "3" * 64,
            }
        }
    }
    ...
    monkeypatch.setattr(
        ...
        lambda *_args, **_kwargs: proposal_019,
    )
    monkeypatch.setattr(
        program_018,
        "validate_operation_contract",
...
def _write_program_018_page(
    descriptor: int,
    active: dict[str, Any],
    state: program_018._PredecessorState,
    request: program_011.SessionRequest,
    *,
    complete: bool,
    ...
    predecessor._append(
        descriptor,
        f"{prefix}.intent.json",
        program_018._intent_payload(active, state, request, intent),
    )
    ...
    if not complete:
        return
    receipt = program_018._response_receipt(
        active,
        state,
        request,
        intent,
...
def _derive_synthetic_predecessor(
    ...
    *,
    ...
    source_commit = "9" * 40
    p17_state = _predecessor_state(requests[0]).program_017_state
    historical_identity = {
    ...
    for root in (
        ...
        path.chmod(0o700)
    root = tmp_path / authority.PROGRAM_018_PRIVATE_ROOT
    descriptor = authority._open_root(tmp_path, authority.PROGRAM_018_PRIVATE_ROOT, create=False)
    predecessor._append(
        descriptor, "active-authority.json", (canonical_json(active_018) + "\n").encode()
    )
    predecessor._append(descriptor, "predecessor-import-manifest.json", p17_state.payload)
    if mutation == "partial-predecessor":
        _write_program_018_page(descriptor, active_018, p17_state, requests[0], complete=False)
    else:
        _write_program_018_page(descriptor, active_018, p17_state, requests[0], complete=True)
        if mutation == "page-1-frontier":
            _write_program_018_page(
                descriptor,
                active_018,
                p17_state,
                requests[1],
            ...
            predecessor._append(
                descriptor,
                f"{prefix}.intent.json",
                program_018._intent_payload(active_018, p17_state, requests[1], intent),
            )
    ...
    if mutation == "later-evidence":
        _write_program_018_page(descriptor, active_018, p17_state, requests[2], complete=False)
    monkeypatch.setattr(science, "acquisition_requests", lambda: requests)
    monkeypatch.setattr(
        authority,
        "validate_operation_contract",
        lambda *_args, **_kwargs: {
            "predecessor": {"program_018_terminal_result": {"path": "terminal", "sha256": "x"}}
        },
    )
    monkeypatch.setattr(
        authority,
        "_load_bound",
        lambda *_args, **_kwargs: {
            "authority_fingerprint": active_018["authority_fingerprint"],
            "source_commit": source_commit,
        },
    )
    monkeypatch.setattr(
        authority,
        "_historical_program_018_identity",
        lambda *_args: historical_identity,
    )
    monkeypatch.setattr(program_018, "validate_operation_contract", lambda *_args, **_kwargs: {})
    monkeypatch.setattr(program_018, "_derive_predecessor_state", lambda *_args: p17_state)
    ...
    monkeypatch.setattr(program_018, "_public_terminal_payload", lambda *_args: b"public\n")
    ...
    p17_descriptor = authority._open_root(
...
def test_operation_contract_revalidates_program_019_v2() -> None:
...
def test_missing_child_stops_cli_before_credentials_private_roots_and_dotenv(
    ...
    monkeypatch.setattr(base_cli, "load_settings", forbidden)
    monkeypatch.setenv("TRADING_LAB_MODE", "research")
    assert base_cli.main(["data", "acquire", "program-019-ohlcv", action]) == 2
    assert "child authority" in capsys.readouterr().err
...
def test_exact_predecessor_prefix_and_frontier_are_rederived(
    ...
    assert len(state.manifest["program_018_completed_sessions"]) == 1
    assert len(state.manifest["discarded_program_018_partial_session"]["completed_pages"]) == 1
    assert state.manifest["consumed_intent_without_response_count"] == 7
...
def test_invalid_predecessor_shapes_fail_before_reuse(
    ...
    with pytest.raises(authority.Program019AuthorityError, match=message):
        _derive_synthetic_predecessor(tmp_path, monkeypatch, mutation=mutation)


def test_projection_excludes_all_three_nested_partial_sessions(monkeypatch: MonkeyPatch) -> None:
    requests = science.acquisition_requests()[:11]
    state = _predecessor_state(requests[-1])
    p17 = state.program_017_state
    p16 = p17.program_016_state
    p15 = p16.program_015_state
    p14 = p15.program_014_state
    p13 = p14.program_013_state
    p12 = p13.program_012_state

    def manifest(value: Any, responses: int, discarded: int | None = None) -> dict[str, Any]:
        result = {
            **value.manifest,
            "completed_request_count": responses,
            "completed_response_count": responses,
            "completed_response_bytes": responses,
        }
        if discarded is not None:
            result[f"discarded_program_{discarded:03d}_partial_session"] = {
                "completed_pages": [{"response_bytes": 1}]
            }
        return result

    p12 = replace(p12, frontier_request_index=1, manifest=manifest(p12, 1))
    p13 = replace(p13, frontier_request_index=2, manifest=manifest(p13, 2), program_012_state=p12)
    p14 = replace(p14, frontier_request_index=3, manifest=manifest(p14, 3), program_013_state=p13)
    p15 = replace(p15, frontier_request_index=4, manifest=manifest(p15, 4), program_014_state=p14)
    p16 = replace(
        p16, frontier_request_index=6, manifest=manifest(p16, 6, 16), program_015_state=p15
    )
    p17 = replace(
        p17, frontier_request_index=8, manifest=manifest(p17, 8, 17), program_016_state=p16
    )
    state = replace(
        state, frontier_request_index=10, manifest=manifest(state, 10, 18), program_017_state=p17
    )
    seen: list[tuple[int, int]] = []

    def source(root: int, *_args: Any) -> tuple[int, Any]:
        return root, _args[-1]

    def execute(request: program_011.SessionRequest, retained: tuple[int, Any]) -> Any:
        root, budget = retained
        index = requests.index(request)
        assert index not in {5, 7, 9}
        seen.append((index, root))
        if root != 19:
            budget.requests += 1
            budget.responses += 1
            budget.response_bytes += 1
        rows, _ = request.parse_page(_body(request, 0, None))
        return program_011.SessionResult(request, rows, (), program_011.Missingness((), ()))
    ...
    for module in (
        ...
        monkeypatch.setattr(module, "_ReplaySource", source)
    monkeypatch.setattr(authority, "_ExistingSessionSource", source)
    monkeypatch.setattr(authority, "_execute_session", execute)
    monkeypatch.setattr(science, "acquisition_requests", lambda: requests)
    budget = authority._Budget(state)
    projection = authority._combined_projection(
        19,
        18,
        17,
        16,
    ...
    assert seen == [(0, 12), (1, 13), (2, 14), (3, 15), (4, 16), (6, 17), (8, 18), (10, 19)]
    assert projection.raw_row_count == 8
    assert set(projection.missing) == {requests[index].session for index in (5, 7, 9)}
    for index in (5, 7, 9):
        request = requests[index]
        assert projection.missing[request.session] == {
...
def test_control_lock_order_uses_only_the_fixed_roots(
    ...
    def locked(_descriptor: int) -> Iterator[None]:
        events.append("program-019-enter")
        held["program-019"] = True
        try:
            yield
        finally:
            held["program-019"] = False
            events.append("program-019-exit")
    ...
    with authority._locked_controls(tmp_path, _active_authority(), create_program_019=True) as (
        program_019_root,
        program_018_root,
        program_017_root,
        program_016_root,
    ...
    assert opened == [
        ...
        (authority.PROGRAM_018_PRIVATE_ROOT, False),
...
def test_child_requires_both_latest_predecessor_source_and_terminal_bindings(
    tmp_path: Path, monkeypatch: MonkeyPatch, path: str
) -> None:
    identity = _child_identity()
    identity["runtime_binding"]["source_files"] = [
        binding
        for binding in identity["runtime_binding"]["source_files"]
        if binding["path"] != path
    ]
    _configure_control_derivation(monkeypatch, identity)
    monkeypatch.setattr(
        authority, "_open_root", lambda *_args, **_kwargs: pytest.fail("private root")
    )
    monkeypatch.setattr(
        credential_contract,
        "credential_presence_preflight",
        lambda *_args: pytest.fail("credentials"),
    )
    with pytest.raises(authority.Program019AuthorityError, match="reviewed child identity differs"):
        authority.derive_active_authority(tmp_path)
...
def test_overbroad_child_fails_before_credentials_or_private_roots(
    ...
    with pytest.raises(authority.Program019AuthorityError, match="reviewed child identity"):
...
def test_protected_overlap_fails_before_every_private_root_entrypoint(
    ...
    monkeypatch.setattr(
        ...
        lambda *_args: (_ for _ in ()).throw(
            authority.Program019AuthorityError(
                "Program 019 request chronology overlaps protected data"
            )
        ),
    )
    monkeypatch.setattr(authority, "_open_root", lambda *_args, **_kwargs: opened.append(True))
    ...
    for operation in (
        ...
        with pytest.raises(authority.Program019AuthorityError, match="overlaps protected"):
            operation()
...
def test_runtime_source_precedes_exact_child_and_review_topology(tmp_path: Path) -> None:
    ...
    _git(tmp_path, "config", "user.name", "Program 019 Test")
    _git(tmp_path, "config", "user.email", "program-019@example.invalid")
    ...
    with pytest.raises(authority.Program019AuthorityError, match="synchronized-main lineage"):
...
def test_transport_and_closeout_use_metadata_without_predecessor_reconstruction(
    ...
    authority._revalidate_transport_boundary(tmp_path, 1, 2, 3, 4, 5, 6, 7, 8, active, state)
    authority._revalidate_closeout_boundary(tmp_path, 1, 2, 3, 4, 5, 6, 7, 8, active, state)
    ...
    with pytest.raises(authority.Program019AuthorityError, match="authority changed"):
        authority._revalidate_transport_boundary(tmp_path, 1, 2, 3, 4, 5, 6, 7, 8, active, state)
    with pytest.raises(authority.Program019AuthorityError, match="authority changed"):
        authority._revalidate_closeout_boundary(tmp_path, 1, 2, 3, 4, 5, 6, 7, 8, active, state)
...
def test_unaccounted_drift_after_page_stops_before_next_transport(
    ...
    with pytest.raises(authority.Program019PostClaimPersistenceError, match="terminal persistence"):
...
def test_mutable_root_tracker_accepts_tracked_writes_and_rejects_unaccounted_drift(
    ...
    try:
        ...
        with pytest.raises(authority.Program019AuthorityError, match="mutable root changed"):
            snapshot.validate_continuation()
    finally:
...
def test_partial_body_write_enters_failure_closeout_without_reissue(
    ...
    with pytest.raises(authority.Program019AuthorityError, match="sealed failure"):
        authority._execute_mock_acquisition(
            tmp_path,
            environ=_credentials(),
            transport=transport,
...
def test_impossible_atomic_body_temp_blocks_failure_terminal(
    ...
    def partial_append(root_descriptor: int, key: str, payload: bytes) -> None:
        if key.endswith(".body"):
            original_append(root_descriptor, key, payload[: max(1, len(payload) // 2)])
            original_append(root_descriptor, f"tmp-program-019-write-{key}", b"partial")
            raise OSError("synthetic partial body write")
        original_append(root_descriptor, key, payload)
...
def test_restarted_wrong_stage_combined_temp_fails_terminal_persistence(
    ...
    predecessor._append(root, "tmp-program-019-combined-canonical-raw", b"partial")
    os.close(root)
...
def test_canonical_publish_failure_seals_with_its_special_temp(
    ...
    def fail_canonical_link(source: str, target: str, *args: Any, **kwargs: Any) -> None:
        if source == "tmp-program-019-combined-canonical-raw":
    ...
    assert (tmp_path / authority.PRIVATE_ROOT / "tmp-program-019-combined-canonical-raw").exists()
...
def test_restarted_unreachable_claim_temp_fails_terminal_persistence(
    ...
    _configure_finite_execution(tmp_path, monkeypatch, activated=True)
    root = authority._open_root(tmp_path, authority.PRIVATE_ROOT, create=False)
    predecessor._append(root, "tmp-program-019-write-claim.json", b"partial")
    os.close(root)
...
def test_restarted_wrong_sequence_credential_temp_fails_terminal_persistence(
    ...
    predecessor._append(root, "tmp-program-019-write-credential-load-999999.attempt.json", b"x")
    os.close(root)
...
def test_restarted_complete_private_terminal_temp_is_promoted_without_credentials(
    ...
    payload = authority._runtime_failure_payload(
        root,
        state,
        authority.Program019AuthorityError("synthetic interrupted closeout"),
        active,
    )
    predecessor._append(root, "tmp-program-019-write-terminal.json", payload)
    os.close(root)
    ...
    assert not (tmp_path / authority.PRIVATE_ROOT / "tmp-program-019-write-terminal.json").exists()
    assert (
...
def test_restarted_partial_private_terminal_temp_fails_closed(
    ...
    _configure_finite_execution(tmp_path, monkeypatch, activated=True)
    root = authority._open_root(tmp_path, authority.PRIVATE_ROOT, create=False)
    predecessor._append(root, "tmp-program-019-write-terminal.json", b"partial")
    os.close(root)
...
@pytest.mark.parametrize(
    "sibling",
    ("tmp-unrelated-sibling", "tmp-program-019-combined-canonical-raw"),
)
...
def test_closeout_rejection_precedes_private_success_terminal(
    ...
    monkeypatch.setattr(
        ...
        lambda *_args: (_ for _ in ()).throw(
            authority.Program019AuthorityError("Program 019 closeout rejected")
        ),
    )
...
def test_crash_after_durable_activation_seals_before_unlock(
    ...
    assert private["program_019_credential_loads"] == 0
...
def test_crash_after_intent_seals_without_credentials_or_transport(
    ...
    assert private["provider_transport_attempted"] is False
    assert private["program_019_credential_loads"] == 0
    assert private["cumulative_request_intents"] == 8
    assert private["cumulative_responses"] == 0
    assert transport.intents == ()
    with pytest.raises(authority.Program019AuthorityError, match="terminally sealed"):
        authority._execute_mock_acquisition(
...
def test_completed_page_replays_locally_and_resumes_only_the_continuation(
    ...
    descriptor = _open_program_019_root(tmp_path)
...
def _concurrent_recovery_worker(
    ...
    try:
        start.wait()
        with authority._locked_controls(Path(repository), active, create_program_019=False) as (
            descriptor,
            _program_018_descriptor,
...
def test_two_processes_serialize_one_credential_and_frontier_transport(
    ...
    # Program 019 is explicitly single-process: a launcher checkpoint created
    # by one process must reject every separately launched process before
    # credential access or provider transport.
    assert all(
        error in {"Program019AuthorityError", "_IncompletePageCheckpoint"} and pages == 0
...
def test_launcher_rejects_relaunch_or_restart_policy_change(tmp_path: Path, mutation: str) -> None:
    ...
    with pytest.raises(authority.Program019AuthorityError, match="launcher changed"):
...
def test_private_terminal_rejects_boolean_strategy_zero(
    ...
    descriptor = authority._open_root(tmp_path, authority.PRIVATE_ROOT, create=False)
    program_018_descriptor = authority._open_root(
        tmp_path, authority.PROGRAM_018_PRIVATE_ROOT, create=False
    )
    ...
    try:
        with pytest.raises(authority.Program019AuthorityError, match="private terminal binding"):
            authority._load_terminal_record(
                tmp_path,
                descriptor,
                program_018_descriptor,
        ...
        os.close(descriptor)
        os.close(program_018_descriptor)
...
def test_credential_sequences_reject_invalid_evidence(tmp_path: Path, mutation: str) -> None:
    ...
    with pytest.raises(authority.Program019AuthorityError, match="credential"):
...
def test_unpaired_attempt_counts_one_load_and_latch_blocks_second_access(
    ...
    with pytest.raises(authority.Program019AuthorityError, match="already consumed"):
...
def test_parse_failure_receipt_precedes_propagation_and_consumes_latch(tmp_path: Path) -> None:
    ...
    with pytest.raises(authority.Program019AuthorityError, match="credentials are required"):
...
def test_combined_budget_counts_predecessor_frontier_and_program_019_pages() -> None:
    state = _predecessor_state(completed_responses=22_169)
    budget = authority._Budget(state)

    assert (budget.requests, budget.responses) == (22_176, 22_169)
...
def test_exhausted_inherited_envelope_seals_without_credentials_or_transport(
    ...
    _, request, _ = _configure_finite_execution(
        tmp_path,
        monkeypatch,
        completed_responses=science.MAXIMUM_REQUESTS_AND_RESPONSES - 7,
    )
    transport = authority.MockBarsTransport(
        [raw_contract.RawResponse(200, _body(request, 0, None))]
    )
    ...
    assert private["cumulative_responses"] == 22_169
    assert private["provider_transport_attempted"] is False
...
def test_working_space_check_uses_available_bytes_not_existing_evidence(
    ...
    with pytest.raises(authority.Program019AuthorityError, match="16 GiB"):
        authority._require_working_disk_capacity(descriptor)
    os.close(descriptor)
...
def test_working_space_fails_before_credential_presence(
    ...
    monkeypatch.setattr(
        ...
        lambda _fd: (_ for _ in ()).throw(
            authority.Program019AuthorityError("Program 019 requires 16 GiB")
        ),
    )

    with pytest.raises(authority.Program019AuthorityError, match="requires 16 GiB"):
        if operation == "preflight":
...
def test_invalid_public_terminal_fields_fail_before_credentials_or_private_roots(
    ...
    with pytest.raises(authority.Program019AuthorityError):
        authority.credential_presence_preflight(tmp_path, environ={})
...
def test_missing_terminal_rejects_before_credentials_or_private_roots(
    ...
    for operation in (
        ...
        with pytest.raises(authority.Program019AuthorityError, match="artifact is absent"):
            operation()
...
def test_changed_terminal_observation_time_rejects_before_credentials(
    ...
    with pytest.raises(authority.Program019AuthorityError, match="semantics differ"):
        authority.credential_presence_preflight(_REPOSITORY, environ={})
...
def test_failure_terminal_recovery_rejects_changed_completed_receipt(
    ...
    with pytest.raises(authority.Program019PostClaimPersistenceError):
        authority._execute_mock_acquisition(
            ...
            after_page=lambda: (_ for _ in ()).throw(
                authority.Program019AuthorityError("synthetic post-page failure")
            ),
        )
    ...
    with pytest.raises(authority.Program019AuthorityError, match="terminal checkpoint"):
        authority.credential_presence_preflight(tmp_path, environ=_ForbiddenCredentialEnvironment())
...
def test_publication_recovers_deterministic_public_temp(
    ...
    predecessor._append(descriptor, "tmp-program-019-public-terminal", retained)
    temp_descriptor = os.open(
        "tmp-program-019-public-terminal",
    ...
    assert not predecessor._exists(descriptor, "tmp-program-019-public-terminal")
    os.close(descriptor)
...
def test_atomic_publication_never_overwrites_different_public_bytes(
    ...
    with pytest.raises(authority.Program019AuthorityError, match="artifact differs"):
        authority._append_public_atomic(tmp_path, descriptor, payload)
...
def test_repository_terminal_bytes_exactly_equal_cli_output(
    ...
    monkeypatch.setattr(
        base_cli,
        "load_dotenv",
        lambda: pytest.fail("Program 019 CLI loaded dotenv"),
    )
    monkeypatch.setenv("TRADING_LAB_MODE", "research")

    assert dispatcher.main(("data", "acquire", "program-019-ohlcv", "run")) == 0
...
def test_secret_guard_allows_reserved_program_019_control_artifacts_only(
    tmp_path: Path, monkeypatch: MonkeyPatch, capsys: CaptureFixture[str]
) -> None:
    spec = importlib.util.spec_from_file_location(
        "program_019_check_secrets", _REPOSITORY / "scripts/check_secrets.py"
    )
    ...
    observation = Path("config/research/program-019-market-observations.json")
    private = Path(".trading-lab/program-019-private-terminal.json")
