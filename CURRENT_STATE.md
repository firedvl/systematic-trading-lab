...
- Work in progress: approval-only repository change awaiting merge; the next candidate remains deferred. Thresholds and strategy code remain separate.
...
- Test status: 56 tests pass; ruff format and lint, strict mypy, secret scan, and diff checks pass on Python 3.12.13. Approved passing evidence can authorize one exact holdout; approved failing evidence cannot. Relative-strength ranking, complete-session portfolio controls, bounded research reads, evidence integrity, and one-time access checks pass; live and broker modes remain rejected.
- Safety: defaults offline; live execution and broker submission are absent.
- Next task: predeclare one new candidate family under the locked gates and run only its bounded 2020–2022 training campaign. Keep gate changes, strategy code, and later data access in separate reviews.
- Branch: `codex/m3-approve-qualification-gates`.
