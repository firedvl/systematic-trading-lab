...
The development-only V3 foundation uses state-transition trades and a FIFO N-bar delay so unchanged long states do not rebalance and pending transitions do not suppress later decisions. It adds fixed 12-bar MA, six-bar momentum, and six-bar opening-range breakout strategies plus a paired realistic/zero-cost diagnostic. A repository exposure audit proposes one exposed Training window and three forward validation windows, but external freshness is unresolved and every validation approval remains false. The unchanged v1 qualification thresholds now have a V3-only realistic-cost report binding. After merge, the build workflow can produce an exact whole-package source manifest and the non-authoritative preassessment can verify its attestation, wheel, lock, dependencies, installed package, isolated runtime, and verifier identity. No V3 plan is sealed, no dataset or candidate exists, and no V3 contract can execute or qualify. The separate V1 research runner cannot produce V3 evidence. See [the V3 draft](docs/research-campaigns/intraday-campaign-v3-draft.md) and [exposure inventory](docs/research-campaigns/intraday-exposure-inventory.md).
