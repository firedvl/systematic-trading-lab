...
Raw SIP trades are qualification evidence only. Before the one-use authority exists, exact provider-authored bytes must freeze the included and excluded condition codes, corrections and cancellations, duplicates and late reports, SIP timestamp order, five-minute buckets, OHLCV calculation, source revision, and SHA-256. If Massive cannot supply that exact contract, qualification fails before credentials or transport. A bounded audit may then compare raw trades with provider aggregates. Raw trades may diagnose an omission; they may never fill one. Aggregating all prints would change OHLCV and volume to escape the missing-data rule.
