# Program 007 corporate-action metadata plan v2
...
`config/research/program-007-corporate-action-metadata-source-plan-v2.json`. It supersedes v1 before
any metadata response or strategy result. V1 required a provider-documented maximum creation lag.
Current Alpaca documentation says no such guarantee exists, so that gate could never establish
negative-event completeness.
