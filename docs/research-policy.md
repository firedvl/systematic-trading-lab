...
Official research records every candidate, parameter set, failure, code revision, dataset fingerprint, universe, cost and execution model, random seed, data split, and artifact hash. Compare results with cash, SPY, relevant buy-and-hold instruments, a fixed-weight ETF portfolio, and approved baselines after costs and risk.
...
CLI runs use only immutable cataloged datasets that pass integrity validation. The command records the dataset fingerprint, code commit, strategy parameters, cost version, execution version, split, and reason before simulation. It rejects parameters the selected strategy would ignore.
