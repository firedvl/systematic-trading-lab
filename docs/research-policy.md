...
The Campaign V3 foundation uses separate `intraday-experiment-v2` and `intraday-backtest-report-v2` development contracts. It records desired state after every completed five-minute slice, queues changed states under `state-transition-delayed-fifo-v1`, and does not rebalance unchanged states. Its paired zero-cost replay is fingerprinted diagnostic evidence, not a qualification input. The V3-only binding retains the unchanged research thresholds and accepts gate metrics only from the realistic-cost object after exact report, lineage, source, and 60-candidate accounting checks. Candidate Training and Validation A/B/C periods are fingerprinted but not approved: repository evidence excludes known dated overlap, while ignored runtime and external exposure remain unresolved. The V3 design remains unpreregistered and has no final plan fingerprint, datasets, source review, reservations, dataset binding, or registry runner. No V3 contract or candidate can execute or qualify. The generic V1 runner remains separate and its V1 schemas cannot become V3 evidence; using it on candidate dates creates new exposure. A separate external freshness attestation must precede any seal.
