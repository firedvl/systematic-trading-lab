...
A prospectively reviewed campaign may execute independent runs within one stage through the bounded local process executor. The coordinator alone advances stages and applies frozen screening after every run in the current stage is terminal. The default is four spawned worker processes; a campaign command may expose `--workers N`. Each worker owns at most one claim at a time, heartbeats its own lease, computes outside SQLite transactions, and publishes through `research-attempts-v1`. Worker exit does not authorize immediate reassignment. The coordinator drains unaffected work, and the expired no-result lease remains the only retry path. Results are read in frozen specification order, so completion order cannot affect screening.

Do not raise the default worker count from four by assumption. Benchmark the same frozen rows with one, four, and then six workers. Compare wall time, per-attempt duration, worker RSS and peak RSS, available memory, load, heartbeat continuity, and SQLite busy or lock failures. Use six or more only when throughput improves without swap or memory pressure, missed heartbeats, or material database contention. Keep one worker as the deterministic diagnostic baseline.
