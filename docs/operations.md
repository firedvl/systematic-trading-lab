...
Recovery: artifact directories are authoritative. After moving a disposable `catalog.sqlite3` aside, run `trading-lab data rebuild-catalog`. Never edit an artifact in place. The execution store can list journal-verified `submission-unknown` orders without changing state. Each requires exact broker lookup and full clean reconciliation before any future retry authority; a missing lookup result alone is insufficient. There is no broker process, retry, cancel action, or live recovery procedure in this phase.
