...
- Decision: derive one synchronized-main commit `C`, read all operation and protected-chronology inputs from Git objects at `C`, and have the lifecycle process own the Git lockfiles for `refs/heads/main` and `refs/remotes/origin/main`. Hold the locks through active-record persistence during activation and through the whole qualification during execution. After request pacing, rederive the same authority before the one-use claim and every transport.
- Context: the prior runtime checked clean synchronized `main` before activation and execution, then reopened policy inputs from the worktree. Another process could advance synchronized `main` after validation but before active-record persistence, the claim, or transport. The first remediation delegated ref-lock ownership to a prepared `git update-ref --stdin` helper. If that helper exited independently, Git removed both locks and the lifecycle process continued until context exit. Parent-owned lockfiles bind lock lifetime to the process that can persist state or contact the provider.
- Consequences: activation fails without an active record if the snapshot cannot be locked or revalidated. Execution fails without a claim or provider call when authority drifts before first transport. Normal Git updates to either synchronized-main ref fail while the snapshot is held and work again after release. Unsupported Git ref storage fails closed. The qualification keeps the existing fixed request, raw-first persistence, one-use, zero-retry, and terminal-evidence behavior.
...
- Context: repeated external-root handoffs were audit ceremony for routine work already within the user's delegated scope. The first independent review found Program 010-only coupling, insufficient runtime binding, and unchecked Git provenance. Later reviews found untracked child controls, an untracked root-mandate path, and an operation manifest outside the Git-bound runtime surface. The final source at commit/tree `81b7f41a99c2df32b3e5b370e3d35bde2d7a25b9` / `3ae0c8d2c79bffa1017aacc9b182c03abbbcfab6` resolves those findings.
- Consequences: the exact grant SHA-256 is `b41cee4f7248fdb4f0f850faff9a7bf882007f8416aa5e7f5c0db931622d868a`; mandate fingerprint is `3465648952ccc094697c8edc24860658c6098235989a868610c1e1b12746d502`; finding-free review SHA-256/fingerprint is `f8cf12416c405e8aab04e2578403d80e7ba461695470d25135a0e619f33522a9` / `c30ecc3f7f9849f2d2f645172ceb68d307016454e485be7a563023ff7359480c`. Controlled/protected access, purchases, actual PAPER execution, broker writes, and live execution remain outside the mandate. Missing required credentials and every other recorded human-attention boundary still stop the autonomous loop.
- Revisit when a concrete child exposes a missing shared invariant. Keep operation-specific endpoint, chronology, budget, storage, and terminal rules in that child until a second real consumer proves common code is smaller.
...
- Consequences: Program 010 records a terminal or passed-frontier absence as `SOURCE-MISSING` and a later incomplete-chain absence as `UNOBSERVED-BECAUSE-CHAIN-INCOMPLETE`. One isolated source gap does not alone fail source qualification. Every required symbol must contain a strict majority of its session coordinates: 40 of 78 on a normal session or 22 of 42 on an early close. Structural, pagination, persistence, action, resource, received-order, and below-majority coverage defects fail. Review found source-closure, meaningful-coverage, public-artifact guard, invalid-request closure, received-order, and retained-order proof defects. V5 remediates them and preserves v1-v4 as review history. The deterministic fresh sample contains five sessions and 4,602 coordinates. The implementation is synthetic-only and the proposal is `PROPOSED-NOT-AUTHORIZED`; independent-review SHA-256/fingerprint `41062ea7286a339651818025886dbbf049f2372d7cefc18431322c442e85bfa9` / `5b39c5b6107f3d63292da184a55b32d837e6f513c42d2253a3b38fbda70e4b04` is finding-free. No external root, credentials, provider request, dataset, strategy result, or protected/execution action exists.
- Revisit after a clean merge. The next phase is a new reviewed exact one-use Program 010 child authority with fixed real transport and ignored persistent storage. Derive its internal identity under the standing mandate; do not require an external user root.
...
- Context: Program 008 closed the metadata prerequisite as `TERMINAL-PASS-CONSUMED-NO-REPLAY`. The successor reuses the finding-free Program 007 raw parser, raw-first persistence, XNYS projection, pagination, and exact rational split-volume normalization. Independent review found and required two corrections before acceptance: reject direct execution before private-root creation when authority or credentials are absent, and make the fsynced claim seal `FAIL-CONSUMED-NO-RETRY` whenever no valid PASS receipt can be persisted. A fresh reviewer then passed all 22 required challenges with no findings.
- Consequences: source commit/root is `842b9db6a973c074f57dc6f1746afc9e02b1c619` / `00839ab4108b336270809a1444bb0077e6045c8f5688c8354c0f5aec6246ab20`. Request-plan SHA-256/fingerprint is `b7ff3f3339f0a896e08e86ca381eba3ce3fd0db21fa94474d9c3842051ffca64` / `cc8152fdf2eb4cd78cf52c302509a622a5b71b9c9c72cfabe18d7cd5510a7783`; authority-proposal SHA-256/fingerprint is `853cd2868704a047a62abcd543e628f5853e768c401aa1cad4a72194ee136d88` / `8594d1872a7c5d3a7db9c695483581251dfb11dea2840bf0ca2b6103aa61b429`; review SHA-256/fingerprint is `348f14735a258f8db17bb9af20bf54f20798b1c150638fd67fb4abc7375ebfbd` / `355690a1ec9e9fe23fea32852019e4690a39677394e668bc7b2ab1b42d111773`. The private Program 009 root stays absent until activation. Proposal and review create no external root, active packet, claim, credential-value access, provider request, response, byte, dataset, or strategy result.
- Revisit after clean synchronized-main delivery to derive and report exactly one external root outside Git. Activation and execution still require the user's exact later grant. Any changed bound source, request plan, proposal, review, or synchronized-main identity requires a new review and root.

## 2026-08-30 - Accept the Program 008 terminal metadata PASS

- Decision: record the exact one-use Program 008 CUSIP-chain qualification as `TERMINAL-PASS-CONSUMED-NO-REPLAY`, preserve its private raw-first evidence, and revoke the consumed metadata authority. The PASS establishes only bounded as-of Alpaca corporate-action corroboration under the frozen structural gates.
- Context: exact root `90b60b95caef446d84e8242ae9aa3e0086d2655c7f55da9dc28c7d252153ad9e` activated on clean synchronized main `37af39a2213ac8bb29b2877d452bf480a33b096a`. One credential load and one zero-retry CUSIP request produced one 83,338-byte HTTP 200 page with 354 unique events. All records mapped to one ledger identity; all five split controls passed; no identity, date, event-ID, pagination, budget, or relevant-action discrepancy failed.
- Consequences: public terminal-success SHA-256/fingerprint is `23bf0e29b4f8b4b4655d7eeb470e4ceb1bc319717c854ec3d82790ef52c1762b` / `151091ac4d863d73561afc24dc0138e5326dd237183dfb7da178cb5584871fcd`. The private response manifest is `program-008-corporate-action-metadata-response-manifest-2026-08-30-v1` at SHA-256/fingerprint `53f4b5ff0012e03e228c1f8b5be07b1a9aaa04566554cd3b536ce7a0d24780c2` / `52f2104c1d4add2ea07ba6c99869cbaadf3996dd23b2b2af953ebc50292c8bee`. Raw metadata remains outside Git. The finding-free terminal review binds closeout commits `9c034c62404e8166c1d9192f761493cbd477437f` and `dbc393c9f357d32ec3a663a657eb4418c0cd1391`; review SHA-256/fingerprint is `65eda8a7e6bc6d262b382227db7ca82ecd42dc38caa97d637d6dd1b207a4e6c9` / `143f8e5023b96bc950f9ec86d37e966fca42ceefd1c07c691d6d3c2af7c66dad`. Program 007 replay, another metadata request, OHLCV, dataset admission, strategy work, controlled/protected access, PAPER, broker writes, and live execution remain prohibited; every effective authority flag is false.
- Enforcement: bind the terminal-success artifact in the authority module and reject root derivation, activation, and direct execution before credentials or private state. Preserve the READY proposal and its review as historical evidence.
- Revisit only after a new explicit user authorization limited to creating and independently reviewing an exact one-use Program 008 successor raw Alpaca SIP OHLCV structural qualification authority proposal. That task must not execute OHLCV.

## 2026-08-30 - Accept the reviewed Program 008 authority proposal without activation

- Decision: accept `program-008-corporate-action-metadata-qualification-authority-proposal-2026-08-29-v1` as READY for a later exact user authorization. It freezes only the never-executed thirteen-CUSIP chain against `GET https://data.alpaca.markets/v1/corporate-actions`; the Program 007 symbol query remains prohibited. Production execution owns the fixed redirect-disabled transport. The test-only executor accepts one exact finite canned-response type and never passes it a credential-bearing request.
- Context: a first independent review rejected the public credential-bearing transport callback. The unpublished implementation and proposal commits were rewritten from starting main `f8332e6fcab2bbf97dc18dc465aa716d94526526`; fresh review then found no material issue. Empty CUSIP remains conditional on one unambiguous public-ledger identity, conflicting non-empty CUSIP remains fatal, ISIN remains optional and noncanonical, all five split controls remain mandatory, and every unexpected relevant action stops for public-ledger reconciliation.
- Consequences: implementation commit/root is `f0ed2534e768d26cc5969d574d16f0ef951e28ac` / `186168ba7fd738e2fa54e9a785808df726a97928a32a9f3e6969f8ff77adc507`. Request-plan SHA-256/fingerprint is `882682b16bbf9f5fa81742a75577546f364d7c76bb04e6e15abf2d9a64043465` / `18ae8ef4152f827fe70261c76f20baf026b44e1a091ae98f1537dcacda818fe7`; authority-proposal SHA-256/fingerprint is `2f7ca46c70fb1aeef298728c780fca28c16c9121bedba1ec1ea5d99954268da5` / `6173cb00ab3119ad17846c0251d253c4448e017e344b882b104ef966c7bc3f78`; finding-free review SHA-256/fingerprint is `c80decf235f7447740b895f400c3faa9386f8f5db10322bdfeb64a66219a7f66` / `ec2b111145b20d59f92dd7b3cdbab8a04572d1889db388732690709a3d73f66f`. All current authority flags remain false. No external root was generated during proposal or review, no private root or claim exists, and no provider, OHLCV, strategy, controlled/protected, PAPER, broker, or live action occurred.
- Revisit when: green CI and a clean synchronized-main merge permit exactly one external-root derivation. Activation and execution still require the user's exact separate authorization; any changed bound implementation, request plan, proposal, review, or final main identity requires a different root and review as enforced by the lifecycle.
...
- Consequences: endpoint access was observed, but the five positive controls were not qualified. The public ledger remains unchanged. Current code binds the exact terminal artifact and rejects activation and direct execution before credential access or private-state creation. Raw metadata stays outside Git. No OHLCV request, dataset, strategy calculation or return, protected-state access, PAPER action, broker write, or live action occurred. Public failure SHA-256/fingerprint is `99bc4397909f364efac2f189351bff9ebaae9b886833fc7e0555b3fa5751119f` / `991bd9892ee32f4badc08350160a03c3514e0ae1a33dfa623406b534c73bd352`. The finding-free terminal review binds closeout commit `e3de52b3b375d1128bfe27453ae9be48420092fc`; review SHA-256/fingerprint is `e2c003595f074eaba749df60787d234f7e04d007927c2dd14213522c0be71fcb` / `26b09a8c40ca69a73a000d9d95d76b1487e0c269fec88cca9d54d8058a0baca9`.
...
## 2026-08-29 - Use Alpaca metadata only as bounded as-of corroboration

- Decision: make public ledger v3 the Program 007 action foundation and use a future Alpaca corporate-action query only to corroborate known controls and find discrepancies as of observation. Keep `types` omitted, parse all sixteen current REST arrays, require returned `process_date` values inside the inclusive requested interval, and use only documented action-specific economic dates for applicability.
- Context: current Alpaca documentation gives no guarantee on corporate-action creation time. It documents inclusive `start` and `end` bounds and `process_date` sorting but does not literally state which field the bounds filter. A waiting period or later query end therefore cannot prove that no delayed record remains. Current OpenAPI still includes `capital_gains_distribution`.
- Consequences: `METADATA_QUERY_END` is fixed at `2026-08-29`, while a future `METADATA_OBSERVATION_AS_OF` records the later query time without claiming finality. Missing positive controls, new relevant events, identity differences, ambiguous economic timing, pagination defects, and unknown arrays fail closed. The module has a dormant fixed-host GET-only bounded transport and create-only private store, but its persistent executor accepts only an explicit mock and rejects the real transport. The finding-free implementation review SHA-256/fingerprint is `2a1199e223d036723040420e4e75abb9cbdfc503a35a88277164f9660c20a80b` / `d3aac3e4774096311339814277ed575df6d1744ca032702c0e50c4e36222b2e5`. Program 007 stays proposed with every authority false and zero real credential or provider access.
- Revisit when: Alpaca changes its endpoint contract or new public evidence contradicts ledger v3. A real query still requires a separate reviewed one-use authority proposal and explicit user grant.
...
- Context: current official REST documentation defines sixteen event arrays and `data_quality=complete`, but does not guarantee Basic-plan access, prove that current identities recover predecessor-only events, state which date field `start` and `end` filter, or bound action-processing delay. Those gaps can turn an empty query into a false negative. Unit splits include an alternate security and undocumented rate orientation; stock-dividend rate semantics are also undocumented for the historical REST schema.
- Consequences: identity history and source finality are pre-authority blockers. The frozen synthetic contract can fail on unknown arrays, identity mismatch, ambiguous dates or factors, pagination, size, controls, and HTTP 403. HTTP 403 ends as `METADATA-ACCESS-FAIL` with no retry, purchase, or fallback. Ledger v2 stays authoritative and incomplete at 10/13; IWM, MDY, and SPY remain unresolved, and synthetic output cannot create ledger v3. No credential value was loaded or used, no provider or OHLCV request occurred, and no strategy or protected execution state changed.
- Revisit when: reviewed predecessor identities or provider documentation close identity history, provider documentation identifies the filter field and gives a bounded lag, that lag has elapsed after `2026-07-31`, and a new query end is frozen. A later phase must then implement and review a separate authority-gated real transport and ignored persistent store before the user grants one metadata-only use.
...
- Context: Program 006 parsed before storage and rejected a valid in-bounds row solely for missing the RTH grid, so response 23 was lost. Its paired provider-adjusted view also coupled split and spin-off price behavior to reciprocal volume without a valid common factor. Public evidence establishes 2-for-1 splits for XLB, XLE, XLK, XLU, and XLY effective before the 2025-12-05 open. Same-session returns cancel a constant unit factor, and raw dollar volume is invariant because `(p/r)*(v*r)=p*v`.
- Consequences: valid extended-hours rows stay in raw evidence but not canonical research data. Weekend, holiday, malformed, misaligned, out-of-bound, duplicate, foreign, oversized, cyclic, incomplete, or ambiguous inputs fail closed. Raw prices remain the only price view. Reverse splits use the same exact ratio orientation; sequential actions multiply; non-transformable actions stop admission. The frozen sample, provider, chronology, science, costs, delays, missingness policy, and budgets do not change. No transport, credential, authority, acquisition, dataset, or strategy surface was added.
- Revisit when: a new public action changes the ledger or a future feature compares prices across sessions. Either change requires a new reviewed control artifact. Program 007 source qualification still requires a finding-free implementation review, clean synchronized main, a separate exact one-use authority proposal, and explicit user authorization.

## 2026-08-28 - Separate Program 006 failure from source suitability

- Decision: preserve Program 006 as immutable `FAIL`, classify response 23 as `INDETERMINATE`, and record the every-provider-row-on-RTH-grid rule as a qualification-specification defect. Classify the paired `raw` / `split,spin-off` rule as a qualification-specification defect because it demanded exact row-level OHLC scaling and reciprocal volume from one combined price factor. Propose Program 007 with raw pages, RTH projection after raw validation, raw same-session prices, and exact split normalization only for cross-session relative-volume share counts.
- Context: response 23's body, timestamp, and symbol were not retained. Current Alpaca documentation permits extended-hours trades to update minute bars, lists no RTH-only historical-bars parameter, sorts multi-symbol results by symbol then time, and distinguishes split price-and-volume adjustment from spin-off price-only adjustment. All 10,677 retained volume observations match the public split ledger, while 3,966 rows fail the old exact OHLC-factor rule. Zero strategy return was generated or read.
- Consequences: Program 006 source suitability remains `INDETERMINATE`; bad Alpaca data is not established. The 22-session Program 006 sample is exposed engineering evidence. Program 007 is `PROPOSED-NOT-AUTHORIZED` and uses a deterministic 15-session sample with zero overlap against the hash-bound 185-session Programs 002-006 observation union or explicit protected and controlled date inventory. It binds the unchanged Program 005 whole-session missing-data policy and requires every selected qualification session to be complete. It fixes six raw chains, seven to 11 responses, 16 MiB, one credential load, and zero retries. The forensic independent review passed all eleven required challenges with no findings; its SHA-256/fingerprint is `103c58c47b0c07c768bd5b3efc577704a74f83cdd223f9945b2df00a3b6f099a` / `0d389c0209753a64dbe0362dba9b817f33103e3a0b7c7ed89164e2c2747d32fe`. Every authority flag remains false.
- Revisit when: never for Program 006. Program 007 can advance only after a complete issuer/exchange unit-changing action ledger, raw-first implementation, finding-free implementation review, clean synchronized main, a new exact one-use authority proposal, and separate user authorization.

## 2026-08-28 — Stop Program 006 after structural source qualification failed

- Decision: record Program 006 source qualification as `FAIL`, preserve its private active authority, claim, pages, checkpoints, locks, and terminal failure record unchanged, and prohibit replay or replacement authority. Bind the public terminal artifact in the authority derivation path so current code rejects it before credential access or state creation. Admit only aggregate terminal evidence and its review to public Git.
- Context: exact root `56125cd74d917c938b1076160f3a6e7c408149d4048d8b0f55f693262fec47c2` activated on clean synchronized main `8ed3bc3f700bbf2b014527c12dc45e0e8cb26def`. The run passed both credential-presence gates, loaded credentials once, created claim fingerprint `00aa50ffb367c764526cd6a893b86768bf68fdda84d34fd440f8d5028cc0e99d`, and consumed the use immediately before transport. Request 23 failed while parsing the first raw pagination-split response because a bar timestamp was outside the exact XNYS grid. Offline checks also found non-constant paired adjustment price factors in all 11 completed pairs.
- Consequences: 23 requests and responses transferred 2,503,402 bytes with zero retries. The 22 stored pages preserve 13/13 symbols, the nine fixed MDY gaps in both views, and zero unexpected gaps. No receipt, source manifest, dataset, strategy return, controlled/protected read, PAPER action, broker write, or live action exists. Failure SHA-256/fingerprint is `3c693b2a31c5d5eef11388b55e41f38b9203c21e7f8ce604fc8f89c9c15e9687` / `acbbbf0f28cc5d555224e05fb8d34cf4e9ae1a60fe75020ca2fee03da91180e5`. Finding-free independent-review SHA-256/fingerprint is `7e79c4534ad2df67c99d70de013033f823ad650d8e35c3bdf9a8d782bba0adf9` / `59533d3b6c151865d304ed11aaf1a9917050625ed736b7d5edb1b38995b35359`; it binds closeout commit `d3e7f10d33d5926074b59913143b7190b0f1bf75` and verifies explicit terminal rejection before credential access or state creation. Every effective authority flag is false.
- Revisit when: never for Program 006. Do not retry, issue replacement authority, change the source, acquire the remaining range, admit a dataset, or execute a strategy.
...
- Context: Program 005 consumed authority before credential lookup and then failed because its required environment variables were absent. It made zero provider requests and observed no provider data or strategy return, so this is a `CONTROL-PLANE-CREDENTIAL-AVAILABILITY` repair rather than provider shopping or scientific adaptation. The current integrated-terminal runtime lacks `PROGRAM_006_ALPACA_API_KEY_ID` and `PROGRAM_006_ALPACA_API_SECRET_KEY`; no credential-file path is supported. The exact 22-session, 13-symbol, 13-range, 26-chain Alpaca Basic SIP qualification and every missingness, quarantine, corporate-action, cost, delay, and protected-state control remain unchanged.
- Consequences: implementation commit/root is `b171ac84dede1f98190f071c600227d09fecc772` / `09864e09a02fd98cb75670555dfdfe127e6c72bfc473c97bf212522c0cdf713c`; finding-free implementation-review SHA-256/fingerprint is `2308b33aebe23a3d69f18abe0e718da3b85de3c165df486d22e88b7f97ae406f` / `6d8bfa3884c0dfcb17012ba45202417adfcb87f1a89322a99394ef53032828f0`. Blocked proposal `program-006-source-qualification-authority-proposal-2026-08-28-v1` has SHA-256/fingerprint `38e28483386fbfac917894558136208b6d0f4418a0c03b6e0e84797a3976ad8e` / `3c4248fd85dea7455ac2800a428e843e7f5d26f8fe7688b7b3820609c12cbbce`; finding-free review `program-006-source-qualification-authority-proposal-independent-review-2026-08-28-v1` has SHA-256/fingerprint `a4bb22c50f4505d2cc93500f9406d1b38108cb0d2e15716374441616a6a4fbde` / `51309ca1383f298278bbc081801a7cb0dd0983188bd83d98cf916ade87f31e33`. Missing credentials write no active authority or claim. Every authority flag remains false, and no external authorization root exists while preflight is blocked.
- Revisit when: both names are visible to `uv run trading-lab data acquire program-006 credential-preflight` in the same integrated-terminal shell. A later handoff may then derive an exact root for separate user authorization; it must not execute the qualification.
