...
- Consequences: Program 006 source suitability remains `INDETERMINATE`; bad Alpaca data is not established. The 22-session Program 006 sample is exposed engineering evidence. Program 007 is `PROPOSED-NOT-AUTHORIZED` and uses a deterministic 15-session sample with zero overlap against the 185-session Programs 002-006 observation union. It fixes six raw chains, seven to 11 responses, 16 MiB, one credential load, and zero retries. Every authority flag remains false.
- Revisit when: never for Program 006. Program 007 can advance only after a complete issuer/exchange unit-changing action ledger, raw-first implementation, finding-free review, clean synchronized main, a new exact one-use authority proposal, and separate user authorization.
