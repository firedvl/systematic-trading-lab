...
- Consequences: force fill, interpolation, provider blending, symbol dropping, and date replacement are prohibited. Missing data excludes the whole thirteen-symbol session under return-blind global and concentration ceilings. Normal alone cannot qualify; the inherited robustness gates retain higher-cost and isolated delay checks. Controlled A's warmup, Block B dependency, acquisition/review/evaluation sequence, one-use protected reads, hidden metrics, and calendar-update stop remain unchanged. One later structural qualification would bind fifteen sessions, 14,742 bars, 221 GET chains and responses, 16 MiB, and one credential load, but it is blocked before authority. No subscription, credential, request, acquisition, strategy return, controlled/protected access, PAPER action, broker write, or live action occurred; all authority remains false.
