...
- Decision: add one daily-only Yahoo chart adapter for versioned USD ETF universes. Map vendor timestamps through `America/New_York`, scale open/high/low by adjusted close divided by close, retain adjusted close and vendor volume, and label the result `yahoo-adjusted-ohlc-v1` with no claimed feed.
