...
- Consequences: Program 010 records a terminal or passed-frontier absence as `SOURCE-MISSING` and a later incomplete-chain absence as `UNOBSERVED-BECAUSE-CHAIN-INCOMPLETE`. One isolated source gap does not alone fail source qualification. Every required symbol must contain a strict majority of its session coordinates: 40 of 78 on a normal session or 22 of 42 on an early close. Structural, pagination, persistence, action, resource, and below-majority coverage defects fail. Initial review required deterministic top-level synthetic-source closure and meaningful coverage; v2 remediated both. Closure review found that the private-data guard rejected the v2 artifact paths, and root verification found an invalid-request closure gap. V3 remediates both and preserves v1/v2 as review history. The deterministic fresh sample contains five sessions and 4,602 coordinates. The implementation is synthetic-only and the proposal is `PROPOSED-NOT-AUTHORIZED`; fresh closure review is pending. No external root, credentials, provider request, dataset, strategy result, or protected/execution action exists.
