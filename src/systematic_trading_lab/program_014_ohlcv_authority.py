"""Single-process Program 014 recovery of the Program 012 and 013 exposed prefix."""
...
CHILD_AUTHORITY_ID = (
    ...
    "structural-admission-child-2026-09-05-v1"
)
CONSUMPTION_BOUNDARY = "immediately before first provider transport invocation"
PRIVATE_ROOT = Path(".trading-lab/program-014-exposed-prefix-raw-alpaca-sip-v1")
PROGRAM_013_PRIVATE_ROOT = program_013.PRIVATE_ROOT
PROGRAM_012_PRIVATE_ROOT = predecessor.PRIVATE_ROOT
PREDECESSOR_PRIVATE_ROOT = PROGRAM_013_PRIVATE_ROOT
PUBLIC_TERMINAL_PATH = Path(
    "config/research/program-014-exposed-prefix-raw-alpaca-sip-recovery-and-"
...
OPERATION_MANIFEST = {
    ...
    "sha256": "bdd2cdefd3f89f513fc19087d4a6191ad8e8cfbd891d77172de6736865425d1e",
    "fingerprint": "e3969f650dd0bee166793c10c76e85dae81b25e9f4402dd02c2b1a1fc8e0a442",
}
PROPOSAL_REVIEW = {
    "path": (
        "config/research/program-014-exposed-prefix-raw-alpaca-sip-recovery-and-"
        "structural-admission-independent-review-v3.json"
    ),
    "sha256": "c34bd327fd065dee1eb93f0c631f0df2dfa2f85d9cbb72195ff4c9da71b93e98",
    "fingerprint": "a6c6720d40dfe020b5c4a02d3dc987bc0fd44d8e5ebf104e66efc43a5d8b9a1a",
}
...
_DERIVED_KEYS = {
    ...
    "program-014-response-manifest.json",
}
...
_PRIVATE_EVIDENCE_KEYS = {
    "program_014_response_manifest_sha256",
...
class Program014AuthorityError(ValueError):
    """Fail-closed Program 014 runtime or authority error."""


class Program014PostClaimPersistenceError(Program014AuthorityError):
    """Terminal evidence could not be persisted after possible transport."""


class CombinedRequestBudgetExhausted(Program014AuthorityError):
...
class _IncompletePageCheckpoint(Program014AuthorityError):
    pass
...
class _CombinedProjection:
    ...
    program_014_response_manifest: Mapping[str, Any]
    budget: _Budget
...
class _Budget:
    def __init__(self, predecessor_state: _PredecessorState) -> None:
        ...
        if completed_requests != completed_responses or consumed_intents != 2:
            raise Program014AuthorityError("Program 014 predecessor budget differs")
        ...
        if (
            ...
            raise Program014AuthorityError("Program 014 inherited budget exceeds its ceiling")
    ...
    def reserve_request(self) -> None:
        if self.requests >= science.MAXIMUM_REQUESTS_AND_RESPONSES:
            raise CombinedRequestBudgetExhausted(
                "Program 014 combined request ceiling is exhausted"
            )
        self.requests += 1
    ...
    def accept_response(
        ...
        if (
            ...
            raise Program014AuthorityError("Program 014 response timestamp must be UTC")
        if self.responses >= self.effective_response_ceiling:
            raise CombinedRequestBudgetExhausted(
                "Program 014 combined response ceiling is exhausted"
            )
        self.responses += 1
        ...
        if len(body) > science.MAXIMUM_RESPONSE_PAGE_BYTES:
            raise Program014AuthorityError("Program 014 response exceeds the 8 MiB page ceiling")
        if self.session_bytes[request.session] > science.MAXIMUM_SESSION_RESPONSE_BYTES:
            raise Program014AuthorityError("Program 014 session exceeds the 8 MiB byte ceiling")
        if self.response_bytes > science.MAXIMUM_TOTAL_RESPONSE_BYTES:
            raise Program014AuthorityError("Program 014 total response byte ceiling exceeded")
...
class _ReadLockedRoot:
    ...
    def __enter__(self) -> None:
        try:
            ...
            raise Program014AuthorityError(f"{self._program} read-only lock is absent") from error
        self._handle = os.fdopen(descriptor, "rb", buffering=0)
        metadata = os.fstat(self._handle.fileno())
        if not stat.S_ISREG(metadata.st_mode) or stat.S_IMODE(metadata.st_mode) & 0o077:
            self._handle.close()
            raise Program014AuthorityError(f"{self._program} read-only lock is invalid")
...
def credential_presence_preflight(
    ...
    with _locked_controls(repository, authority, create_program_014=True) as (
        program_014_root,
        program_013_root,
        program_012_root,
    ):
        ...
        if locked != authority:
            raise Program014AuthorityError("Program 014 authority changed under policy lock")
        predecessor_state = _derive_predecessor_state(
            repository, program_013_root, program_012_root, authority
        )
...
def validate_operation_contract(
    ...
    try:
        program_013.validate_operation_contract(repository, commit=commit)
    except program_013.Program013AuthorityError as error:
        raise Program014AuthorityError(str(error).replace("Program 013", "Program 014")) from None
    ...
    program_013_terminal = _mapping(
        predecessor_bindings.get("program_013_terminal_result"),
    ...
    if (
        ...
        or review.get("reviewed_source_commit") != "07899b4846ad646f87640f3b385c41116f2c52cc"
        or review.get("reviewed_proposal") != OPERATION_MANIFEST
        or predecessor_bindings.get("program_013_scientific_contract")
        != program_013.OPERATION_MANIFEST
        or predecessor_bindings.get("program_013_scientific_contract_review")
        != program_013.PROPOSAL_REVIEW
        or program_013_terminal.get("path") != program_013.PUBLIC_TERMINAL_PATH.as_posix()
        or inheritance.get("base_contract") != "program_013_scientific_contract"
        ...
        or recovery.get("program_014_private_root") != PRIVATE_ROOT.as_posix()
        ...
        or recovery.get("program_014_private_root_must_be_distinct") is not True
        or recovery.get("program_014_private_root_override_allowed") is not False
        or recovery.get("accepted_predecessor_shape")
        != "EXACT-COMPLETED-WHOLE-SESSION-PREFIX-PLUS-ONE-INTENT-ONLY-PAGE-FRONTIER"
        or recovery.get("first_program_014_intent_must_equal_rederived_frontier_identity")
        is not True
        or recovery.get("frontier_request_under_program_014_max") != 1
        or recovery.get("program_014_request_reissue_allowed") is not False
        or recovery.get("program_014_automatic_retries") != 0
        or any(
            ...
            for key in (
                ...
                "program_012_and_program_013_public_and_private_terminal_binding_required",
        ...
        or recovery.get("incomplete_program_013_session_or_page_reuse_allowed") is not False
        or restart.get("program_014_lock_path") != f"{PRIVATE_ROOT.as_posix()}/run.lock"
        ...
        != science.MAXIMUM_REQUESTS_AND_RESPONSES - 2
        or budgets.get("consumed_intent_without_response_count") != 2
        ...
        or budgets.get("credential_loads_per_program_014_process_max") != 1
        ...
        or private_terminal.get("schema_version") != "program-014-private-terminal-v1"
        ...
        != _PRIVATE_EVIDENCE_KEYS
        or static_private.get("schema_version") != "program-014-private-terminal-v1"
        ...
        or public_terminal.get("schema_version") != "program-014-exposed-prefix-terminal-result-v1"
        or set(_sequence(public_terminal.get("exact_top_level_keys"), "public terminal keys"))
        != _PUBLIC_TERMINAL_KEYS
        or static_public
        != {
            "schema_version": "program-014-exposed-prefix-terminal-result-v1",
        ...
        raise Program014AuthorityError("Program 014 operation contract differs")
    return proposal
...
def _derive_control_validated_authority(repository: Path) -> Mapping[str, Any]:
    ...
    try:
        ...
        raise Program014AuthorityError(str(error)) from None
    ...
    if (
        ...
        raise Program014AuthorityError("Program 014 reviewed child identity differs")
    ...
    try:
        ...
        raise Program014AuthorityError(str(error).replace("Program 011", "Program 014")) from None
    ...
    if _repository_preflight(repository, identity) != lineage:
        raise Program014AuthorityError("Program 014 repository changed during validation")
...
def _locked_controls(
    ...
    *,
    create_program_014: bool,
) -> Iterator[tuple[int, int, int]]:
    program_014_root = _open_root(repository, PRIVATE_ROOT, create=create_program_014)
    try:
        with git_controls._LockedRoot(program_014_root):
            program_013_root = _open_root(repository, PROGRAM_013_PRIVATE_ROOT, create=False)
            try:
                with _ReadLockedRoot(program_013_root, "Program 013"):
                    ...
                    try:
                        with (
                            ...
                            yield program_014_root, program_013_root, program_012_root
                    finally:
...
def _activate_locked_authority(
    ...
    if entries not in (
        ...
        raise Program014AuthorityError("Program 014 one-use authority state already exists")
...
def _execute_mock_acquisition(
    ...
    *,
    ...
    if type(transport) is not MockBarsTransport or environ is os.environ:
        raise Program014AuthorityError("Program 014 test execution requires finite inputs")
    return _execute_acquisition(
        repository,
        environ=environ,
...
def _execute_acquisition(
    ...
    *,
    ...
    with _locked_controls(repository, expected_authority, create_program_014=True) as (
        program_014_root,
        program_013_root,
        program_012_root,
    ):
        ...
        try:
            if _derive_control_validated_authority(repository) != authority:
                raise Program014AuthorityError("Program 014 authority changed under lock")
            _recover_private_terminal_if_present(
                repository,
                program_014_root,
                program_013_root,
            ...
            if after_activation is not None:
                after_activation()
            _validate_launcher(program_014_root, authority, predecessor_state)
            _load_active(program_014_root, authority)
            budget = _reconstruct_state(
                program_014_root,
                predecessor_state,
                authority=authority,
            )
            _require_working_disk_capacity(program_014_root)
            ...
            def consume() -> None:
                ...
                if predecessor._exists(program_014_root, "claim.json"):
                    _validate_claim(program_014_root, authority, predecessor_state)
                    return
                predecessor._append_atomic(
                    program_014_root,
                    "claim.json",
            ...
            temp_key, temp_descriptor = predecessor._new_temp(
                program_014_root, "combined-canonical-raw"
            )
            completed = False
            try:
                ...
                if not completed:
                    with suppress(FileNotFoundError):
                        os.unlink(temp_key, dir_fd=program_014_root)
                        os.fsync(program_014_root)
            ...
            if budget.new_requests != budget.new_responses:
                raise Program014AuthorityError("Program 014 new request and response counts differ")
            ...
            for key, payload in cast(Mapping[str, bytes], evidence["payloads"]).items():
                predecessor._append_or_validate(program_014_root, key, payload)
            ...
            predecessor._append_atomic(program_014_root, _TERMINAL_KEY, terminal_payload)
            ...
            try:
                ...
                raise Program014PostClaimPersistenceError(
                    "Program 014 public terminal persistence failed after possible transport"
            ...
            if (
                ...
                or _has_consumed_state(program_014_root)
            ) and not predecessor._exists(program_014_root, _TERMINAL_KEY):
                try:
                    ...
                    raise Program014PostClaimPersistenceError(
                        "Program 014 terminal persistence failed after possible transport"
                    ) from persistence_error
            if published_failure and isinstance(error, Exception):
                raise Program014AuthorityError(
                    "Program 014 acquisition ended with a sealed failure"
                ) from None
            raise
...
class _PersistentSessionSource:
    ...
    def response(self, intent: program_011.PageIntent) -> raw_contract.RawResponse:
        if self._pending is not None:
            raise Program014AuthorityError("Program 014 persistent source state differs")
        ...
        if any(present):
            if present[0]:
                ...
                if predecessor._read(self._root_descriptor, f"{prefix}.intent.json") != expected:
                    raise Program014AuthorityError(
                        "Program 014 partial request intent checkpoint differs"
                    )
            raise Program014AuthorityError(
                "Program 014 ambiguous page checkpoint forbids request reissue"
            )
    ...
    def retain(self, page_index: int, body: bytes) -> program_011.RetainedPage:
        ...
        if (
            ...
            raise Program014AuthorityError("Program 014 retained response differs")
        self._pending = None
        return pending
...
class _ReplaySource:
    ...
    def response(self, intent: program_011.PageIntent) -> raw_contract.RawResponse:
        ...
        if not all(present):
            if (
                ...
                raise Program014AuthorityError("Program 014 partial page checkpoint differs")
            self._budget.reserve_request()
    ...
    def retain(self, page_index: int, body: bytes) -> program_011.RetainedPage:
        ...
        if pending is None or pending.page_index != page_index:
            raise Program014AuthorityError("Program 014 replayed response differs")
        self._pending = None
        return pending
...
class _ExistingSessionSource:
    ...
    def retain(self, page_index: int, body: bytes) -> program_011.RetainedPage:
        ...
        if (
            ...
            raise Program014AuthorityError("Program 014 existing response differs")
        self._pending = None
        return pending
...
def _combined_projection(
    ...
    *,
    ...
    program_013_budget = program_013._Budget(predecessor_state.program_012_state)
    program_012_frontier = predecessor_state.program_012_state.frontier_request_index
    ...
    for request_index, request in enumerate(science.acquisition_requests()):
        if request_index < program_012_frontier:
            source: Any = predecessor._ReplaySource(
                program_012_root,
                request,
                predecessor_state.program_012_state.active_authority,
                predecessor_state.program_012_state.source_commit,
                program_012_budget,
            )
            ...
            source = program_013._ReplaySource(
                ...
                predecessor_state.program_012_state,
                program_013_budget,
            )
        elif loader is None:
            source = _ExistingSessionSource(program_014_root, request, authority, predecessor_state)
        else:
        ...
        for row in result.rows:
            ...
            if canonical_file is not None and canonical_file.write(line) != len(line):
                raise Program014AuthorityError("Program 014 canonical raw write was incomplete")
            canonical_hash.update(line)
            raw_row_count += 1
    if (
        program_012_budget.requests
        != _count(
            predecessor_state.program_012_state.manifest.get("completed_request_count"),
            "Program 012 completed request count",
        )
        or program_012_budget.responses
        != _count(
            predecessor_state.program_012_state.manifest.get("completed_response_count"),
            "Program 012 completed response count",
        )
        or program_012_budget.response_bytes
        != _count(
            predecessor_state.program_012_state.manifest.get("completed_response_bytes"),
        ...
        raise Program014AuthorityError("Program 014 predecessor replay budget differs")
    response_manifest = {
        "schema_version": "program-014-private-response-manifest-v1",
        "program_id": PROGRAM_ID,
    ...
    return _CombinedProjection(
        ...
        missing=missing,
        morning=morning,
        program_014_response_manifest=response_manifest,
        budget=budget,
    )
...
def _execute_session(
    ...
    try:
        ...
        replacement = Program014AuthorityError(str(error).replace("Program 012", "Program 014"))
        ...
        raise Program014AuthorityError(str(error).replace("Program 012", "Program 014")) from error
...
def _reconstruct_state(
    ...
    *,
    ...
    if entries & _DERIVED_KEYS and _TERMINAL_KEY not in entries:
        raise Program014AuthorityError("Program 014 derived evidence lacks a terminal")
    allowed = {
        "active-authority.json",
        "claim.json",
        _LAUNCHER_KEY,
    ...
    for entry in entries - allowed:
        if _PAGE_KEY.fullmatch(entry):
            ...
            raise Program014AuthorityError("Program 014 private checkpoint contains unknown state")
    ...
    if page_entries:
        if claim_exists:
            if successful_loads == 0:
                raise Program014AuthorityError(
                    "Program 014 page evidence lacks a successful credential audit"
                )
            ...
            raise Program014AuthorityError("Program 014 provider evidence lacks a transport claim")
    elif claim_exists:
        raise Program014AuthorityError("Program 014 transport claim lacks page evidence")
    if credential_loads and not page_entries:
        raise Program014AuthorityError("Program 014 credential audit lacks request evidence")
    ...
    for entry in page_entries:
        ...
        try:
            ...
            raise Program014AuthorityError("Program 014 page key is invalid") from error
        if session not in expected_sessions or not 1 <= page_index <= 16:
            raise Program014AuthorityError("Program 014 page key is outside the recovery plan")
    ...
    for request in requests[predecessor_state.frontier_request_index :]:
        ...
        if frontier_found:
            if session_entries:
                raise Program014AuthorityError("Program 014 checkpoint skips a request session")
            continue
        ...
        try:
            ...
            if frozenset(session_entries) not in {
                ...
                raise Program014AuthorityError(
                    "Program 014 terminal checkpoint is not a reachable prefix"
                ) from None
            frontier_found = True
            ...
            if session_entries != predecessor._expected_session_entries(request, completed_pages):
                raise Program014AuthorityError(
                    "Program 014 checkpoint skips a request page"
                ) from None
            if require_complete:
                raise Program014AuthorityError(
                    "Program 014 acquisition evidence is incomplete"
                ) from None
            frontier_found = True
        except (Program014AuthorityError, program_011.Program011Error):
            if not allow_terminal_failure:
                raise
            completed_pages = sum(
                page["session"] == request.session.isoformat() for page in budget.pages
            ...
            if session_entries != predecessor._expected_session_entries(request, len(result.pages)):
                raise Program014AuthorityError(
                    "Program 014 checkpoint continues after a terminal page"
                )
    if require_complete and frontier_found:
        raise Program014AuthorityError("Program 014 acquisition evidence is incomplete")
    return budget
...
def _runtime_failure_credential_load_count(
    ...
    except Program014AuthorityError:
        return len(
            {
...
def _claim_value(
    ...
    unsigned: dict[str, Any] = {
        "schema_version": "program-014-private-acquisition-claim-v1",
        "program_id": PROGRAM_ID,
        "authority_id": authority["authority_id"],
        "authority_fingerprint": authority["authority_fingerprint"],
        "operation_manifest": OPERATION_MANIFEST,
...
def _validate_claim(
    ...
    if predecessor._read(root_descriptor, "claim.json") != _claim_payload(
        ...
        raise Program014AuthorityError("Program 014 transport claim differs")
...
def _intent_value(
    ...
    unsigned: dict[str, Any] = {
        "schema_version": "program-014-private-request-intent-v1",
        "program_id": PROGRAM_ID,
        "authority_fingerprint": authority["authority_fingerprint"],
        "operation_manifest": OPERATION_MANIFEST,
        "runtime_binding": dict(_runtime_binding(authority)),
...
def _response_receipt(
    ...
    if type(response.status) is not int or not 100 <= response.status <= 599:
        raise Program014AuthorityError("Program 014 response status is invalid")
    ...
    claim = _claim_value(authority, predecessor_state)
    unsigned: dict[str, Any] = {
        "schema_version": "program-014-private-response-receipt-v1",
        "program_id": PROGRAM_ID,
        "authority_fingerprint": authority["authority_fingerprint"],
        "operation_manifest": OPERATION_MANIFEST,
        "runtime_binding": dict(_runtime_binding(authority)),
...
def _load_completed_page(
    ...
    if predecessor._read(root_descriptor, f"{prefix}.intent.json") != expected_intent:
        raise Program014AuthorityError("Program 014 request intent checkpoint differs")
    ...
    if (
        ...
        raise Program014AuthorityError("Program 014 response checkpoint differs")
...
def _validate_page_intent(
    ...
    if (
        ...
        raise Program014AuthorityError("Program 014 request intent differs")
    ...
    if (
        ...
        raise Program014AuthorityError("Program 014 first intent differs from the frontier")
...
def _credential_attempt(
    ...
    if type(sequence) is not int or sequence < 1:
        raise Program014AuthorityError("Program 014 credential sequence differs")
    unsigned: dict[str, Any] = {
        "schema_version": "program-014-private-credential-load-attempt-v1",
        "authority_fingerprint": authority["authority_fingerprint"],
        "runtime_binding": dict(_runtime_binding(authority)),
...
def _credential_receipt_payload(attempt_identity: str, status: str) -> bytes:
    if _HEX_64.fullmatch(attempt_identity) is None or status not in {"SUCCESS", "FAILURE"}:
        raise Program014AuthorityError("Program 014 credential receipt differs")
    value = {
...
def _launcher_value(
    ...
    unsigned = {
        "schema_version": "program-014-private-single-process-launcher-v1",
        "authority_fingerprint": authority["authority_fingerprint"],
        "predecessor_import_manifest_sha256": predecessor_state.sha256,
...
def _validate_launcher(
    ...
    if (
        ...
        raise Program014AuthorityError("Program 014 launcher changed or the process was restarted")
...
def _credential_load_counts(
    ...
    for entry in os.listdir(root_descriptor):
        ...
        if raw != (canonical_json(value) + "\n").encode():
            raise Program014AuthorityError("Program 014 credential audit is not canonical")
        target = attempts if match.group("kind") == "attempt" else receipts
        if sequence in target:
            raise Program014AuthorityError("Program 014 credential sequence is duplicated")
        target[sequence] = value
    sequences = sorted(attempts)
    if sequences != list(range(1, len(sequences) + 1)) or not set(receipts) <= set(attempts):
        raise Program014AuthorityError("Program 014 credential sequence is not contiguous")
    ...
    for sequence in sequences:
        ...
        if set(attempt) != _CREDENTIAL_ATTEMPT_KEYS or attempt != expected_attempt:
            raise Program014AuthorityError("Program 014 credential attempt binding differs")
        receipt = receipts.get(sequence)
        if receipt is None:
            if sequence != sequences[-1]:
                raise Program014AuthorityError("Program 014 credential receipt sequence has a gap")
            continue
        if (
            set(receipt) != _CREDENTIAL_RECEIPT_KEYS
            or receipt.get("schema_version") != "program-014-private-credential-load-receipt-v1"
            ...
            raise Program014AuthorityError("Program 014 credential receipt binding differs")
        if receipt["status"] == "SUCCESS":
            successful += 1
        elif sequence != sequences[-1]:
            raise Program014AuthorityError("Program 014 failed credential load is not terminal")
...
def _reserve_process_credential_access(write_attempt: Callable[[], None]) -> None:
    ...
    with _PROCESS_CREDENTIAL_LOCK:
        if process_id == _PROCESS_CREDENTIAL_PID:
            raise Program014AuthorityError(
                "Program 014 credential access is already consumed in this process"
            )
        try:
            write_attempt()
        finally:
...
def _revalidate_transport_boundary(
    ...
    if _derive_control_validated_authority(repository) != authority:
        raise Program014AuthorityError("Program 014 authority changed at transport boundary")
    current_predecessor = _derive_predecessor_state(
        repository, program_013_root, program_012_root, authority
    )
    if current_predecessor.payload != predecessor_state.payload:
        raise Program014AuthorityError(
            "Program 014 predecessor evidence changed at transport boundary"
        )
    _require_working_disk_capacity(program_014_root)
...
def _revalidate_closeout_boundary(
    ...
    if _derive_control_validated_authority(repository) != authority:
        raise Program014AuthorityError("Program 014 authority changed at closeout")
    current_predecessor = _derive_predecessor_state(
        repository, program_013_root, program_012_root, authority
    )
    if current_predecessor.payload != predecessor_state.payload:
        raise Program014AuthorityError("Program 014 predecessor evidence changed at closeout")
    _require_zero_protected_overlap(repository, _authority_commit(authority))
...
def _require_zero_protected_overlap(repository: Path, commit: str) -> None:
    ...
    if any(
        ...
        raise Program014AuthorityError("Program 014 request chronology overlaps protected data")
...
def _require_working_disk_capacity(root_descriptor: int) -> None:
    ...
    if filesystem.f_bavail * filesystem.f_frsize < science.WORKING_DISK_RESERVATION_BYTES:
        raise Program014AuthorityError("Program 014 requires 8 GiB of available working space")
...
def _runtime_binding(authority: Mapping[str, Any]) -> Mapping[str, Any]:
    ...
    if (
        ...
        raise Program014AuthorityError("Program 014 runtime binding differs")
    return runtime


def _count(value: Any, label: str) -> int:
    if type(value) is not int or value < 0:
        raise Program014AuthorityError(f"Program 014 {label} differs")
    return value
...
def _derived_admission_evidence(
    ...
    program_014_proposal = validate_operation_contract(
    ...
    admission["schema_version"] = "program-014-private-structural-admission-report-v1"
    admission["program_id"] = PROGRAM_ID
    admission["status"] = (
        "ADMITTED-PROGRAM-014-RAW-STRUCTURAL-PREFIX"
    ...
    missing_value = {
        "schema_version": "program-014-private-combined-missing-coordinates-v1",
        "program_id": PROGRAM_ID,
        "predecessor_import_manifest_sha256": predecessor_state.sha256,
        "sessions": {
    ...
    response_payload = (canonical_json(projection.program_014_response_manifest) + "\n").encode()
    ...
    private_dataset_identity = fingerprint(
        {
            ...
            "predecessor_import_manifest_sha256": predecessor_state.sha256,
            "program_014_response_manifest_sha256": response_sha,
    ...
    public_lineage = (
        {
            "schema_version": ("program-014-public-raw-structural-prefix-lineage-manifest-v1"),
            "program_id": PROGRAM_ID,
            "authority_id": authority["authority_id"],
            "authority_fingerprint": authority["authority_fingerprint"],
            "source_commit": _source_commit(authority),
            "status": "ADMITTED-PROGRAM-014-RAW-STRUCTURAL-PREFIX",
    ...
    if public_lineage is not None and (
        ...
        raise Program014AuthorityError("Program 014 public and private identities collide")
    dataset_manifest = {
        "schema_version": "program-014-private-combined-dataset-manifest-v1",
        "program_id": PROGRAM_ID,
        "authority_id": authority["authority_id"],
        "authority_fingerprint": authority["authority_fingerprint"],
        "source_commit": _source_commit(authority),
    ...
    return {
        ...
        "payloads": {
            "program-014-response-manifest.json": response_payload,
...
def _admission_terminal_payload(
    ...
    if _HEX_64.fullmatch(private_identity) is None:
        raise Program014AuthorityError("Program 014 private dataset identity differs")
    lineage_identity = (
    ...
    record: dict[str, Any] = {
        ...
        "program_014_credential_loads": _credential_load_counts(
            root_descriptor, authority, predecessor_state
        )[0],
...
def _runtime_failure_payload(
    ...
    private_evidence = {
        "program_014_response_manifest_sha256": _evidence_sha256_if_present(
            root_descriptor, "program-014-response-manifest.json"
        ),
    ...
    record: dict[str, Any] = {
        ...
        "program_014_credential_loads": _runtime_failure_credential_load_count(
            root_descriptor, authority, predecessor_state
        ),
...
def _load_terminal_record(
    ...
    if (
        ...
        or record.get("schema_version") != "program-014-private-terminal-v1"
        ...
        raise Program014AuthorityError("Program 014 private terminal binding differs")
    predecessor._parse_observed_at(record.get("observed_at"))
    current_predecessor = _derive_predecessor_state(
        repository, program_013_root, program_012_root, authority
    )
    if current_predecessor.payload != predecessor_state.payload:
        raise Program014AuthorityError("Program 012 evidence changed before terminal validation")
    ...
    if _count(
        record.get("program_014_credential_loads"), "terminal credential count"
    ) != durable_credentials or record.get(
        "provider_transport_attempted"
    ) is not predecessor._exists(root_descriptor, "claim.json"):
        raise Program014AuthorityError("Program 014 terminal credential count differs")
    ...
    if result_kind == "RUNTIME-FAILURE":
        ...
        if (
            ...
            raise Program014AuthorityError("Program 014 runtime terminal semantics differ")
        _validate_runtime_failure_counts(record, counts)
        for field, key in (
            ("program_014_response_manifest_sha256", "program-014-response-manifest.json"),
            ...
            if private_evidence[field] != _evidence_sha256_if_present(root_descriptor, key):
                raise Program014AuthorityError("Program 014 runtime evidence hash differs")
        ...
        for key, expected in payloads.items():
            if predecessor._read(root_descriptor, key) != expected:
                raise Program014AuthorityError("Program 014 derived evidence differs")
        if (
            _evidence_sha256(root_descriptor, "combined-canonical-raw.jsonl")
            != projection.canonical_sha256
        ):
            raise Program014AuthorityError("Program 014 combined canonical evidence differs")
        admission = _mapping(evidence.get("admission"), "structural admission")
        admission_passed = admission.get("admission_passed") is True
        ...
        if (
            ...
            raise Program014AuthorityError("Program 014 admission terminal semantics differ")
        _validate_budget_counts(record, budget)
    else:
        raise Program014AuthorityError("Program 014 private terminal result kind differs")
    return record
...
def _validate_budget_counts(record: Mapping[str, Any], budget: _Budget) -> None:
    if (
        ...
        raise Program014AuthorityError("Program 014 terminal budget differs")
...
def _publish_public_terminal(
    ...
    _revalidate_closeout_boundary(
        repository, program_013_root, program_012_root, authority, predecessor_state
    )
    ...
    if validated != record:
        raise Program014AuthorityError("Program 014 terminal changed before publication")
    ...
    try:
        ...
        if existing_descriptor is not None:
            ...
            if not stat.S_ISREG(metadata.st_mode) or existing != payload:
                raise Program014AuthorityError("Program 014 public terminal artifact differs")
        ...
        try:
            with os.fdopen(temp_descriptor, "wb") as handle:
                if handle.write(payload) != len(payload):
                    raise Program014AuthorityError(
                        "Program 014 public terminal write was incomplete"
                    )
                handle.flush()
            ...
            try:
                ...
                raise Program014AuthorityError(
                    "Program 014 public terminal artifact already exists"
                ) from None
            os.fsync(public_descriptor)
        finally:
            with suppress(FileNotFoundError):
...
def _recover_private_terminal_if_present(
    ...
    if predecessor._exists(root_descriptor, _TERMINAL_KEY):
        ...
        raise Program014AuthorityError("Program 014 acquisition is terminally sealed")
    ...
    try:
        ...
        _load_active(root_descriptor, authority)
        raise Program014AuthorityError("Program 014 single-process transaction was interrupted")
        ...
        raise Program014AuthorityError("Program 014 acquisition is terminally sealed") from None
...
def _reject_terminal_state(repository: Path) -> None:
    ...
    try:
        ...
        if _PUBLIC_TERMINAL_SHA256 is None:
            return
        raise Program014AuthorityError("Program 014 terminal result artifact is absent") from error
    except OSError as error:
        raise Program014AuthorityError("Program 014 terminal result artifact is invalid") from error
    try:
        metadata = os.fstat(descriptor)
        if not stat.S_ISREG(metadata.st_mode) or metadata.st_size > 1_048_576:
            raise Program014AuthorityError("Program 014 terminal result artifact is invalid")
    ...
    if raw != (canonical_json(value) + "\n").encode():
        raise Program014AuthorityError("Program 014 public terminal is not canonical")
    _validate_public_terminal_shape(value)
    if _PUBLIC_TERMINAL_SHA256 is None:
        raise Program014AuthorityError("Program 014 terminal result lacks immutable binding")
    if hashlib.sha256(raw).hexdigest() != _PUBLIC_TERMINAL_SHA256:
        raise Program014AuthorityError("Program 014 terminal result semantics differ")
    raise Program014AuthorityError("Program 014 authority is terminally revoked")


def _validate_public_terminal_shape(value: Mapping[str, Any]) -> None:
    if (
        set(value) != _PUBLIC_TERMINAL_KEYS
        or value.get("schema_version") != "program-014-exposed-prefix-terminal-result-v1"
        ...
        raise Program014AuthorityError("Program 014 public terminal schema differs")
    ...
    if result_kind == "RUNTIME-FAILURE":
        ...
        expected_status = "ADMITTED-PROGRAM-014-RAW-STRUCTURAL-PREFIX"
        expected_admission = True
    else:
        raise Program014AuthorityError("Program 014 public terminal result kind differs")
    if (
        value.get("status") != expected_status
        or value.get("admission_passed") is not expected_admission
    ):
        raise Program014AuthorityError("Program 014 public terminal branch differs")
    lineage = value.get("dataset_lineage_manifest")
    if expected_admission:
        _validate_public_lineage(lineage, value)
    elif lineage is not None:
        raise Program014AuthorityError("Program 014 failure terminal publishes lineage")
...
def _validate_public_lineage(value: Any, terminal: Mapping[str, Any]) -> dict[str, Any]:
    ...
    if (
        ...
        != "program-014-public-raw-structural-prefix-lineage-manifest-v1"
        ...
        or lineage.get("status") != "ADMITTED-PROGRAM-014-RAW-STRUCTURAL-PREFIX"
        or _HEX_64.fullmatch(str(lineage.get("dataset_lineage_identity"))) is None
    ):
        raise Program014AuthorityError("Program 014 public dataset lineage differs")
    return lineage


def _exact_object(value: Any, keys: set[str], label: str) -> dict[str, Any]:
    if type(value) is not dict or set(value) != keys:
        raise Program014AuthorityError(f"Program 014 {label} schema differs")
...
def _evidence_sha256(root_descriptor: int, key: str) -> str:
    if _EVIDENCE_KEY.fullmatch(key) is None:
        raise Program014AuthorityError("Program 014 evidence key is invalid")
    try:
        descriptor = os.open(
            key,
            os.O_RDONLY | getattr(os, "O_CLOEXEC", 0) | getattr(os, "O_NOFOLLOW", 0),
        ...
        raise Program014AuthorityError(f"Program 014 evidence is absent: {key}") from error
    with os.fdopen(descriptor, "rb") as handle:
        metadata = os.fstat(handle.fileno())
        if not stat.S_ISREG(metadata.st_mode) or stat.S_IMODE(metadata.st_mode) & 0o077:
            raise Program014AuthorityError("Program 014 evidence is not private")
...
def _publish_temp_or_validate(
    ...
    if predecessor._exists(root_descriptor, final_key):
        if _evidence_sha256(root_descriptor, final_key) != expected_sha256:
            raise Program014AuthorityError("Program 014 canonical raw dataset differs")
    else:
        try:
            os.link(
                temp_key,
            ...
            raise Program014AuthorityError("Program 014 canonical raw publish raced") from None
        os.fsync(root_descriptor)
    os.unlink(temp_key, dir_fd=root_descriptor)
    os.fsync(root_descriptor)
...
def _historical_fingerprinted_artifact(
    ...
    if _HEX_64.fullmatch(str(stored)) is None or stored != fingerprint(unsigned):
        raise Program014AuthorityError(f"Program 014 historical {label} fingerprint differs")
    return value, {
        "path": path.as_posix(),
...
def _validate_historical_source_binding(repository: Path, runtime: Mapping[str, Any]) -> None:
    ...
    if (
        ...
        raise Program014AuthorityError("Program 013 historical runtime binding differs")
    seen: set[str] = set()
    for item in source_files:
        ...
        if not path or path in seen or _HEX_64.fullmatch(str(binding.get("sha256"))) is None:
            raise Program014AuthorityError("Program 013 historical runtime source differs")
        seen.add(path)
...
def _historical_program_013_identity(
    repository: Path,
    proposal: Mapping[str, Any],
    active: Mapping[str, Any],
    program_014_commit: str,
) -> Mapping[str, Any]:
    forensic_binding = _mapping(
        _mapping(proposal.get("predecessor"), "predecessor bindings").get(
            "redacted_private_forensic_disposition"
        ),
        "Program 014 forensic binding",
    )
    forensic = _load_bound(
        repository,
        forensic_binding,
        "forensic_disposition_fingerprint",
        commit=program_014_commit,
    )
    historical = _mapping(forensic.get("predecessor"), "historical Program 013 binding")
    child_binding = _mapping(historical.get("child_authority"), "Program 013 child binding")
    review_binding = _mapping(
        historical.get("child_authority_review"), "Program 013 child review binding"
    )
    sync_commit = str(
        _mapping(active.get("control_lineage"), "Program 013 control lineage").get(
            "synchronized_main_commit"
        )
    )
    if _HEX_40.fullmatch(sync_commit) is None:
        raise Program014AuthorityError("Program 013 synchronized-main binding differs")
    ...
    runtime = _mapping(child.get("runtime_binding"), "Program 013 child runtime binding")
    child_authority = _mapping(child.get("authority"), "Program 013 child authority")
    required_challenges = tuple(
        cast(
            Sequence[str],
            _sequence(child.get("required_review_challenges"), "Program 013 review challenges"),
        )
    )
    ...
    if (
        child.get("child_authority_id") != program_013.CHILD_AUTHORITY_ID
        or child.get("program_ordinal") != program_013.PROGRAM_ORDINAL
        or child.get("program_id") != program_013.PROGRAM_ID
        or child.get("operation_manifest") != program_013.OPERATION_MANIFEST
        ...
        raise Program014AuthorityError("Program 013 historical reviewed child differs")
...
def _derive_predecessor_state(
    ...
    terminal_binding = _mapping(
        predecessor_bindings.get("program_013_terminal_result"),
        "Program 013 public terminal binding",
    )
    ...
    active_raw = predecessor._read(program_013_root, "active-authority.json")
    active = _json_object(active_raw, "Program 013 active authority")
    if active_raw != (canonical_json(active) + "\n").encode():
        raise Program014AuthorityError("Program 013 active authority is not canonical")
    unsigned_active = dict(active)
    active_fingerprint = unsigned_active.pop("authority_fingerprint", None)
    control_lineage = _mapping(active.get("control_lineage"), "Program 013 control lineage")
    runtime = _mapping(active.get("runtime_binding"), "Program 013 runtime binding")
    source_commit = str(control_lineage.get("runtime_source_commit"))
    historical_identity = _historical_program_013_identity(
    ...
    if (
        ...
        raise Program014AuthorityError("Program 013 active authority binding differs")
    sync_commit = str(control_lineage.get("synchronized_main_commit"))
    try:
        program_013.validate_operation_contract(repository, commit=sync_commit)
        program_012_state = program_013._derive_predecessor_state(
            repository, program_012_root, active
        )
        if predecessor._read(program_013_root, "predecessor-import-manifest.json") != (
            program_012_state.payload
        ):
            raise Program014AuthorityError("Program 013 predecessor import manifest differs")
        private_terminal = program_013._load_terminal_record(
            repository,
            program_013_root,
            program_012_root,
            active,
    ...
    except (program_013.Program013AuthorityError, predecessor.Program012AuthorityError) as error:
        raise Program014AuthorityError(str(error).replace("Program 013", "Program 014")) from None
    if (
        ...
        or program_013._public_terminal_payload(private_terminal)
        != _git(repository, "show", f"{_authority_commit(authority)}:{terminal_binding['path']}")
    ):
        raise Program014AuthorityError("Program 013 terminal disposition differs")

    requests = science.acquisition_requests()
    page_entries = {
        entry for entry in os.listdir(program_013_root) if program_013._PAGE_KEY.fullmatch(entry)
    }
    budget = program_013._Budget(program_012_state)
    completed_sessions: list[dict[str, Any]] = []
    frontier: dict[str, Any] | None = None
    frontier_request_index = -1
    for request_index, request in enumerate(
        requests[program_012_state.frontier_request_index :],
        start=program_012_state.frontier_request_index,
    ):
        session_entries = {
            entry for entry in page_entries if entry.startswith(f"session-{request.session}-")
        }
        if frontier is not None:
            if session_entries:
                raise Program014AuthorityError("Program 013 evidence continues after frontier")
            continue
        source = program_013._ReplaySource(
            program_013_root, request, active, program_012_state, budget
        )
        before_requests = budget.requests
        before_responses = budget.responses
        before_bytes = budget.response_bytes
        try:
            result = program_013._execute_session(request, source)
        except program_013._IncompletePageCheckpoint:
            completed_pages = budget.responses - before_responses
            page_index = completed_pages + 1
            if page_index != 1:
                raise Program014AuthorityError(
                    "Program 013 reviewed frontier is not first-page intent-only"
                ) from None
            incoming_token = None
            if completed_pages:
                ...
                if receipt.get("status") != 200:
                    raise Program014AuthorityError("Program 013 frontier status differs") from None
            intent = program_011.PageIntent(
            ...
            if session_entries != expected_entries:
                raise Program014AuthorityError("Program 013 frontier checkpoint differs") from None
            frontier = {
                "session": request.session.isoformat(),
                ...
                "intent_sha256": hashlib.sha256(
                    predecessor._read(program_013_root, f"{prefix}.intent.json")
                ).hexdigest(),
                "body_present": False,
        ...
        except (program_013._TransportRequired, program_013.Program013AuthorityError) as error:
            raise Program014AuthorityError(
                "Program 013 evidence is not the reviewed intent-only frontier"
            ) from error
        except program_011.Program011Error as error:
            raise Program014AuthorityError(
                "Program 013 completed prefix does not reparse"
            ) from error
        else:
            expected_entries = predecessor._expected_session_entries(request, len(result.pages))
            if session_entries != expected_entries:
                raise Program014AuthorityError("Program 013 completed session evidence differs")
            pages = []
                ...
                pages.append(
                    {
                        ...
                        "body_sha256": hashlib.sha256(
                            predecessor._read(program_013_root, f"{prefix}.body")
                        ).hexdigest(),
                        "receipt_sha256": hashlib.sha256(
                            predecessor._read(program_013_root, f"{prefix}.receipt.json")
                        ).hexdigest(),
            ...
            if budget.requests - before_requests != len(result.pages):
                raise Program014AuthorityError("Program 013 completed request count differs")
    if frontier is None or frontier_request_index < 0 or not completed_sessions:
        raise Program014AuthorityError("Program 013 reviewed recovery frontier is absent")
    if (
        ...
        or budget.requests - budget.responses != 2
    ):
        raise Program014AuthorityError("Program 013 cumulative terminal budget differs")
    ...
    unsigned_manifest: dict[str, Any] = {
        ...
        "program_012_predecessor_manifest_sha256": program_012_state.sha256,
        "program_012_completed_sessions": program_012_state.manifest["completed_sessions"],
        "program_012_frontier": program_012_state.manifest["frontier"],
        "program_013_public_terminal": dict(terminal_binding),
        "program_013_active_authority_fingerprint": active_fingerprint,
        "program_013_child_identity_fingerprint": active["child_identity_fingerprint"],
        "program_013_source_commit": source_commit,
        "program_013_private_terminal_fingerprint": private_terminal["terminal_fingerprint"],
        "program_013_completed_sessions": completed_sessions,
        ...
        "consumed_intent_frontiers": [program_012_state.manifest["frontier"], frontier],
        "frontier": frontier,
...
def _validate_predecessor_manifest(root_descriptor: int, expected: _PredecessorState) -> None:
    if predecessor._read(root_descriptor, "predecessor-import-manifest.json") != expected.payload:
        raise Program014AuthorityError("Program 014 predecessor import manifest differs")
...
def _append_atomic_or_validate(root_descriptor: int, key: str, payload: bytes) -> None:
    if predecessor._exists(root_descriptor, key):
        if predecessor._read(root_descriptor, key) != payload:
            raise Program014AuthorityError(f"Program 014 derived evidence differs: {key}")
        return
    try:
        predecessor._append_atomic(root_descriptor, key, payload)
    except predecessor.Program012AuthorityError as error:
        raise Program014AuthorityError(str(error).replace("Program 012", "Program 014")) from None
...
def _load_active(root_descriptor: int, expected: Mapping[str, Any]) -> Mapping[str, Any]:
    ...
    if value != expected or raw != (canonical_json(expected) + "\n").encode():
        raise Program014AuthorityError("Program 014 active authority differs")
    return value
...
def _repository_preflight(repository: Path, identity: Mapping[str, Any]) -> dict[str, str]:
    ...
    try:
        ...
        raise Program014AuthorityError("Program 014 repository identity is unavailable") from error
    if (
        ...
        raise Program014AuthorityError("Program 014 reviewed synchronized-main lineage differs")
    return {
...
def _authority_commit(authority: Mapping[str, Any]) -> str:
    ...
    if _HEX_40.fullmatch(value) is None:
        raise Program014AuthorityError("Program 014 authority policy commit differs")
    return value
...
def _source_commit(authority: Mapping[str, Any]) -> str:
    ...
    if _HEX_40.fullmatch(value) is None:
        raise Program014AuthorityError("Program 014 runtime source commit differs")
    return value
...
def _require_credentials_present(environ: Mapping[str, str] | None) -> None:
    ...
    if missing:
        raise Program014AuthorityError("Program 014 credentials missing: " + ", ".join(missing))
    ...
    *,
    ...
    try:
        ...
        if not fingerprint_optional:
            raise Program014AuthorityError(
                str(error).replace("Program 011", "Program 014")
            ) from None
        path = Path(str(binding.get("path")))
        raw = _git(repository, "show", f"{commit or 'HEAD'}:{path.as_posix()}")
        if hashlib.sha256(raw).hexdigest() != binding.get("sha256"):
            raise Program014AuthorityError("Program 014 bound artifact differs") from None
    ...
    try:
        for part in root.parts:
            ...
            try:
                ...
                raise Program014AuthorityError(
                    "Program 014 private evidence root is absent"
                ) from error
            os.close(descriptor)
            descriptor = child
        metadata = os.fstat(descriptor)
        if not stat.S_ISDIR(metadata.st_mode) or stat.S_IMODE(metadata.st_mode) & 0o077:
            raise Program014AuthorityError("Program 014 private evidence root is not private")
        return descriptor
...
def _repository(repository: Path) -> Path:
    if not isinstance(repository, Path):
        raise Program014AuthorityError("Program 014 repository root is invalid")
    resolved = repository.resolve()
    if not resolved.is_dir():
        raise Program014AuthorityError("Program 014 repository root is absent")
    return resolved


def _mapping(value: Any, label: str) -> Mapping[str, Any]:
    if not isinstance(value, Mapping):
        raise Program014AuthorityError(f"Program 014 {label} is invalid")
    return value
...
def _json_object(raw: bytes, label: str) -> dict[str, Any]:
    try:
        ...
        raise Program014AuthorityError(f"Program 014 {label} is invalid JSON") from error
    if type(value) is not dict:
        raise Program014AuthorityError(f"Program 014 {label} is not an object")
    return value
...
def _git(repository: Path, *arguments: str) -> bytes:
    try:
        ...
        raise Program014AuthorityError("Program 014 Git identity is unavailable") from error
