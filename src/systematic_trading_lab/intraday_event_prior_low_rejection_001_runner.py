"""Restart-safe process runner for Intraday Event Prior-Low Rejection 001."""
...
from .intraday_event_prior_low_rejection_001_launch_control import (
    REVIEWED_LAUNCH_CONTROL_FINGERPRINT,
    REVIEWED_LAUNCH_CONTROL_SHA256,
)
from .intraday_event_prior_low_rejection_001_plan import (
    PLAN_FINGERPRINT,
    PLAN_RELATIVE_PATH,
    PLAN_SHA256,
    PROGRAM_ID,
...
from .intraday_event_prior_low_rejection_001_strategies import (
...
RUNNER_VERSION = "intraday-event-prior-low-rejection-001-runner-v1"
RUN_SCHEMA = "intraday-event-prior-low-rejection-001-run-v1"
RUN_REPORT_SCHEMA = "intraday-event-prior-low-rejection-001-backtest-report-v1"
FINAL_FREEZE_SCHEMA = "intraday-event-prior-low-rejection-001-final-freeze-v1"
FINAL_REPORT_SCHEMA = "intraday-event-prior-low-rejection-001-final-report-v1"
PROGRAM_BINDING_SCHEMA = "intraday-event-prior-low-rejection-001-program-binding-v1"
DATABASE_NAME = "intraday-event-prior-low-rejection-001.sqlite3"
ENGINE_VERSION = "intraday-exposed-002-engine-v1"
STRATEGY_VERSION = "scheduled-event-spy-prior-low-rejection-v1"
LAUNCH_CONTROL_RELATIVE_PATH = Path(
    "config/research/intraday-event-prior-low-rejection-001-launch-control-review-v1.json"
)
_LAUNCH_CONTROL_SCHEMA = "intraday-event-prior-low-rejection-001-launch-control-review-v1"
_LAUNCH_CONTROL_FILES = (
    ...
    "src/systematic_trading_lab/intraday_event_prior_low_rejection_001_plan.py",
    "src/systematic_trading_lab/intraday_event_prior_low_rejection_001_strategies.py",
    "src/systematic_trading_lab/intraday_event_prior_low_rejection_001_runner.py",
...
_LAUNCH_CONTROL_POST_REVIEW_FILES = frozenset(
    {
        ...
        "src/systematic_trading_lab/intraday_event_prior_low_rejection_001_launch_control.py",
        "tests/unit/test_intraday_event_prior_low_rejection_001_runner.py",
        "CURRENT_STATE.md",
        "DECISIONS.md",
        "ROADMAP.md",
...
def _require_non_broker_environment(
    ...
    if forbidden:
        raise ValueError(
            "Intraday Event Prior-Low Rejection 001 rejects broker credentials and paper-write "
...
class IntradayEventPriorLowRejection001Store:
    ...
    def reserve(self, specifications: Sequence[Mapping[str, object]]) -> None:
        ...
        if len(set(run_ids)) != len(run_ids):
            raise ValueError("Event Prior-Low Rejection 001 run specifications collide")
        existing = {str(row["run_id"]) for row in self.attempts.list_runs()}
        if len(existing | set(run_ids)) > 46:
            raise ValueError("Event Prior-Low Rejection 001 run budget exceeds 46 specifications")
...
class _Worker:
    ...
    def __init__(
        ...
        self.plan = load_intraday_event_prior_low_rejection_001_plan(self.repository)
        ...
        self.attempt_store = IntradayEventPriorLowRejection001Store(runtime_root)
        ...
        with (
            ...
            try:
                ...
                strategy = ScheduledEventSpyPriorLowRejectionStrategy(
                    ...
                    configuration.confirmation_bars,
                    frozenset(
    ...
    def _configuration(self, candidate_id: str) -> EventPriorLowRejectionConfiguration:
        for item in self.plan.configurations:
            if item.candidate_id == candidate_id:
                return item
        raise ValueError(f"unknown Event Prior-Low Rejection 001 candidate: {candidate_id}")

    def _period(self, period_id: str) -> EventDriftPeriod:
        for item in self.plan.periods:
            if item.period_id == period_id:
                return item
        raise ValueError(f"unknown Event Prior-Low Rejection 001 period: {period_id}")


class IntradayEventPriorLowRejection001Runner:
    """Coordinate immutable Prior-Low Rejection stages through spawned workers."""
    ...
    def __init__(
        ...
        *,
        ...
        if REVIEWED_LAUNCH_CONTROL_SHA256 is None or REVIEWED_LAUNCH_CONTROL_FINGERPRINT is None:
            raise ValueError(
                "Intraday Event Prior-Low Rejection 001 launch control is not hash-bound"
            )
        ...
        self.attempt_store = IntradayEventPriorLowRejection001Store(self.runtime_root)
    ...
    def _specification(
        self,
        configuration: EventPriorLowRejectionConfiguration,
    ...
    def _execute(self, specifications: Sequence[Mapping[str, object]]) -> None:
        ...
        for specification in specifications:
            ...
            if row["status"] == "completed":
                ...
                raise AttemptStateError(f"Event Prior-Low Rejection 001 run is terminal: {run_id}")
            elif row["status"] == "running":
                raise AttemptStateError(
                    f"Event Prior-Low Rejection 001 run has an active attempt: {run_id}"
                )
            else:
    ...
    def _load_report(self, row: Mapping[str, object]) -> Mapping[str, Any]:
        if row.get("status") != "completed":
            raise ValueError("Event Prior-Low Rejection 001 run is not completed")
        relative = Path(_required_text(row.get("report_path"), "report path"))
        if relative.is_absolute() or ".." in relative.parts:
            raise ValueError("Event Prior-Low Rejection 001 report path is unsafe")
        raw = (self.runtime_root / relative).read_bytes()
        if hashlib.sha256(raw).hexdigest() != row.get("report_sha256"):
            raise ValueError("Event Prior-Low Rejection 001 report SHA-256 differs")
        ...
        if (
            ...
            raise ValueError("Event Prior-Low Rejection 001 report fingerprint differs")
        return value
    ...
    def _report_for(
        ...
        if len(matches) != 1:
            raise ValueError("Event Prior-Low Rejection 001 canonical run relationship differs")
    ...
    def _normal_zero_reports(
        ...
        if _signal_fingerprint(normal) != _signal_fingerprint(zero):
            raise _CoordinatorValidationError(
                ...
                "ValueError: Event Prior-Low Rejection 001 cross-scenario signals differ",
            )
        return normal, zero
    ...
    def _require_no_failures(self) -> None:
        ...
        if failed:
            raise AttemptStateError(
                f"Event Prior-Low Rejection 001 has {len(failed)} terminal failed run(s); no retry "
                "is allowed"
            )
    ...
    def _run_discovery(self) -> dict[str, object]:
        ...
        if len(specifications) != 6:
            raise ValueError("Event Prior-Low Rejection 001 discovery budget differs")
    ...
    def _run_walk_forward(self, discovery: Mapping[str, object]) -> dict[str, object]:
        ...
        if len(specifications) > 16:
            raise ValueError("Event Prior-Low Rejection 001 walk-forward budget differs")
        self._execute(specifications)
    ...
    def _run_serious(self, walk_forward: Mapping[str, object]) -> dict[str, object]:
        ...
        if stress_scenarios != (
            ...
            raise ValueError("Event Prior-Low Rejection 001 stress scenarios differ")
        ...
        if len(stress_specifications) > 16:
            raise ValueError("Event Prior-Low Rejection 001 stress budget differs")
        ...
        if new_neighbor_count > 8:
            raise _CoordinatorValidationError(
                ...
                "ValueError: Event Prior-Low Rejection 001 neighbor budget differs",
            )
                ...
                for period_index, period in enumerate(periods):
                    ...
                    if _signal_fingerprint(report) != base_signals[period_index]:
                        raise _CoordinatorValidationError(
                            ...
                            "ValueError: Event Prior-Low Rejection 001 stress signal differs from "
                            "Normal",
                        )
    ...
    def _select_cohort(self, serious: Mapping[str, object]) -> tuple[str, ...]:
        ...
        if len(cohort) > 1:
            raise ValueError(
                "Event Prior-Low Rejection 001 final cohort exceeds its frozen maximum"
            )
        for item in ledger:
    ...
    def _freeze(
        ...
        if len(runs) > 46 or Decimal(str(attempt_summary["total_attempts"])) > 138:
            raise ValueError("Event Prior-Low Rejection 001 frozen search budget exceeded")
    ...
    def _final_report(
        ...
        payload: dict[str, object] = {
            ...
            "terminal_message": (
                "INTRADAY EVENT PRIOR-LOW REJECTION 001 COMPLETE — NO CONTROLLED-QUALIFIED "
                "CANDIDATE"
                if empty
                else "INTRADAY EVENT PRIOR-LOW REJECTION 001 COMPLETE — WAITING FOR FUTURE "
                "UNTOUCHED DATA"
            ),
            "source_commit": self.source_commit,
    ...
    def _terminal_interruption_report(
        ...
        *,
        ...
        payload: dict[str, object] = {
            ...
            "terminal_message": "INTRADAY EVENT PRIOR-LOW REJECTION 001 TERMINALLY INTERRUPTED",
            "source_commit": self.source_commit,
...
def _run_id(specification: Mapping[str, object]) -> str:
    ...
    if set(context) != {"candidate_id", "period_id", "scenario_id"}:
        raise ValueError("Event Prior-Low Rejection 001 canonical run context differs")
    identity = {key: _text(context, key) for key in ("candidate_id", "period_id", "scenario_id")}
    return f"ieplr001r-{fingerprint(identity)[:24]}"


def _reservation_id(run_fingerprint: str) -> str:
    return f"ieplr001q-{run_fingerprint[:24]}"
...
def _configuration_summary(
    ...
    return {
        ...
        "confirmation_bars": configuration.confirmation_bars,
...
def _deduplicate_specifications(
    ...
    for specification in specifications:
        ...
        if existing is not None and canonical_json(existing) != canonical_json(specification):
            raise ValueError("Event Prior-Low Rejection 001 canonical run identity collides")
...
def _event_session(bars: Sequence[OHLCVBar], day: date) -> dict[Symbol, tuple[OHLCVBar, ...]]:
    ...
    if not timestamps[0] or timestamps[0] != timestamps[1] or len(timestamps[0]) <= 32:
        raise ValueError("Event Prior-Low Rejection 001 event session bars differ")
    return session


def _event_history(bars: Sequence[OHLCVBar], day: date) -> dict[Symbol, tuple[OHLCVBar, ...]]:
    return {
        symbol: tuple(
            bar for bar in bars if bar.symbol == symbol and _account_day(bar.timestamp) <= day
        )
...
def _run_report(
    ...
    if len(eligible) != period.eligible_event_count:
        raise ValueError("Event Prior-Low Rejection 001 period event count differs")
    event_by_day = {date.fromisoformat(str(event.xnys_session)): event for event in eligible}
    if len(event_by_day) != len(eligible):
        raise ValueError("Event Prior-Low Rejection 001 eligible event sessions collide")
    ...
    if any(
        ...
        raise ValueError("Event Prior-Low Rejection 001 has an invalid evaluated fill")
    if any(
        ...
        raise ValueError("Event Prior-Low Rejection 001 has an invalid event round trip")
    fees_by_day = {
    ...
    if isinstance(delay, bool) or delay not in (1, 2, 3):
        raise ValueError("Event Prior-Low Rejection 001 execution delay differs")
    ledger: list[dict[str, object]] = []
    ...
    for event in eligible:
        ...
        signal = ScheduledEventSpyPriorLowRejectionStrategy(
            configuration.candidate_id,
            configuration.confirmation_bars,
            frozenset({day}),
            spy[0].timestamp,
        ).signal(_event_history(bars, day))
        ...
        if daily is None:
            raise ValueError("Event Prior-Low Rejection 001 event lacks a daily fee ledger")
        regulatory_fees = daily.charges.total
        active = signal.active
        reclaim_index = signal.reclaim_decision_bar_index
        reclaim_decision = signal.reclaim_decision_timestamp
        ...
        if not active:
            if fills or trades or regulatory_fees != _ZERO:
                raise ValueError(
                    "Event Prior-Low Rejection 001 inactive event has execution evidence"
                )
            entry_fill = exit_fill = None
            gross = slippage = net = _ZERO
        else:
            if reclaim_index is None or not 6 <= reclaim_index <= 11:
                raise ValueError("Event Prior-Low Rejection 001 reclaim index differs")
            entries = tuple(fill for fill in fills if fill.quantity > 0)
            exits = tuple(fill for fill in fills if fill.quantity < 0)
            expected_entry_fill = spy[reclaim_index + cast(int, delay)].timestamp
            ...
            if (
                ...
                or entries[0].decision_timestamp != reclaim_decision
                ...
                raise ValueError("Event Prior-Low Rejection 001 event execution differs")
        ...
        signal_trace.append(
            {
                ...
                "confirmation_bars": configuration.confirmation_bars,
                "prior_session_low": signal.prior_session_low,
                "opening_window_low": signal.opening_window_low,
                "opening_window_breach": signal.opening_window_breach,
                "active": active,
                "reclaim_decision_bar_index": reclaim_index,
                "reclaim_decision_timestamp": reclaim_decision,
    ...
    if reconciliation_error != _ZERO:
        raise ValueError("Event Prior-Low Rejection 001 event accounting does not reconcile")
    release_net = {
        name: sum(
...
def _event_ledger(report: Mapping[str, Any]) -> tuple[Mapping[str, Any], ...]:
    ...
    if not isinstance(value, list) or any(not isinstance(row, dict) for row in value):
        raise ValueError("Event Prior-Low Rejection 001 event ledger differs")
    return tuple(value)
...
def _aggregate_event_reports(reports: tuple[Mapping[str, Any], ...]) -> dict[str, object]:
    ...
    if len(set(event_ids)) != len(event_ids):
        raise ValueError("Event Prior-Low Rejection 001 aggregate event IDs collide")
    event_net = sum((Decimal(str(row["net_profit_loss"])) for row in ledger), _ZERO)
    reported_net = Decimal(str(aggregate["net_profit_loss"]))
    reconciliation_error = abs(event_net - reported_net).quantize(_ACCOUNTING_PRECISION)
    if reconciliation_error != _ZERO:
        raise ValueError("Event Prior-Low Rejection 001 aggregate event accounting differs")
    release_net = {
        name: sum(
            (Decimal(str(row["net_profit_loss"])) for row in ledger if row["release_name"] == name),
            _ZERO,
...
def _final_markdown(report: Mapping[str, object], json_sha256: str) -> str:
    ...
    lines = [
        "# Intraday Event Prior-Low Rejection 001 final report",
        "",
...
def _read_final_report(path: Path, *, source_commit: str | None = None) -> Mapping[str, Any]:
    ...
    if (
        ...
        raise ValueError("Event Prior-Low Rejection 001 final report differs")
    return value
...
def _validate_final_evidence(runtime: Path, report: Mapping[str, Any]) -> None:
    ...
    if database.get("path") != DATABASE_NAME or database.get("sha256") != _sha256_path(
        ...
        raise ValueError("Event Prior-Low Rejection 001 runtime database differs")
    ...
    if relative != "final-freeze.json":
        raise ValueError("Event Prior-Low Rejection 001 final freeze path differs")
    ...
    if (
        ...
        raise ValueError("Event Prior-Low Rejection 001 final freeze differs")


def _load_launch_control(repository: Path, *, source_commit: str) -> Mapping[str, Any]:
    if REVIEWED_LAUNCH_CONTROL_SHA256 is None or REVIEWED_LAUNCH_CONTROL_FINGERPRINT is None:
        raise ValueError("Intraday Event Prior-Low Rejection 001 launch control is not hash-bound")
    path = repository / LAUNCH_CONTROL_RELATIVE_PATH
    if not path.is_file():
        raise ValueError("Intraday Event Prior-Low Rejection 001 launch control review is missing")
    raw = path.read_bytes()
    if hashlib.sha256(raw).hexdigest() != REVIEWED_LAUNCH_CONTROL_SHA256:
        raise ValueError("Intraday Event Prior-Low Rejection 001 launch control SHA-256 differs")
    try:
        value = _mapping(json.loads(raw), "launch control review")
    except json.JSONDecodeError as error:
        raise ValueError(
            "Intraday Event Prior-Low Rejection 001 launch control is invalid JSON"
        ) from error
    unsigned = dict(value)
    ...
    if (
        ...
        raise ValueError("Intraday Event Prior-Low Rejection 001 launch control differs")
    _require_exact_keys(
        value,
        {
            "schema_version",
    ...
    if value.get("review_id") != _LAUNCH_CONTROL_SCHEMA:
        raise ValueError("Intraday Event Prior-Low Rejection 001 launch review identity differs")
    ...
    if (
        ...
        raise ValueError("Intraday Event Prior-Low Rejection 001 launch independent review differs")
...
def _verify_launch_inputs(value: Mapping[str, Any]) -> None:
    ...
    if dict(inputs) != expected:
        raise ValueError("Intraday Event Prior-Low Rejection 001 launch inputs differ")
...
def _verify_launch_implementation(repository: Path, value: Mapping[str, Any]) -> str:
    ...
    if not isinstance(files, list) or len(files) != len(_LAUNCH_CONTROL_FILES):
        raise ValueError("Intraday Event Prior-Low Rejection 001 launch files differ")
    for item, expected_path in zip(files, _LAUNCH_CONTROL_FILES, strict=True):
        ...
        if binding.get("path") != expected_path or binding.get("sha256") != _sha256_path(
            ...
            raise ValueError("Intraday Event Prior-Low Rejection 001 implementation file differs")
...
def _verify_launch_quality(value: Mapping[str, Any], source_commit: str) -> None:
    ...
    if quality.get("source_commit") != source_commit or not isinstance(results, list):
        raise ValueError("Intraday Event Prior-Low Rejection 001 launch quality gates differ")
    if len(results) != len(_LAUNCH_CONTROL_QUALITY_GATES):
        raise ValueError("Intraday Event Prior-Low Rejection 001 launch quality gate count differs")
    for result, command in zip(results, _LAUNCH_CONTROL_QUALITY_GATES, strict=True):
        ...
        if (
            ...
            raise ValueError("Intraday Event Prior-Low Rejection 001 launch quality gate differs")
...
def _verify_launch_equivalence(value: Mapping[str, Any], source_commit: str) -> None:
    ...
    if (
        ...
        != "intraday-event-prior-low-rejection-001-parallel-equivalence-v1"
        ...
        != "synthetic-non-protected-spy-prior-low-rejection-five-minute-bars"
        ...
        raise ValueError("Intraday Event Prior-Low Rejection 001 launch equivalence differs")
    ...
    for item in fixtures:
        ...
        if any(
            ...
            raise ValueError("Intraday Event Prior-Low Rejection 001 equivalence fixture differs")
        candidates.add(str(fixture["candidate_id"]))
        scenarios.add(str(fixture["scenario_id"]))
    if len(candidates) < 2 or len(scenarios) < 2:
        raise ValueError("Intraday Event Prior-Low Rejection 001 equivalence lacks design span")
...
def _verify_launch_source_lineage(
    ...
    try:
        ...
        raise ValueError(
            "Intraday Event Prior-Low Rejection 001 launch source lineage is unavailable"
        ) from error
    ...
    if (
        ...
        raise ValueError("Intraday Event Prior-Low Rejection 001 launch source lineage differs")


def _require_exact_keys(value: Mapping[str, object], expected: set[str], label: str) -> None:
    if set(value) != expected:
        raise ValueError(f"Event Prior-Low Rejection 001 {label} fields differ")
...
def _validated_source_commit(value: object) -> str:
    if (
        ...
        raise ValueError("Event Prior-Low Rejection 001 launch source commit differs")
    return value
...
def _required_sha256(value: object, label: str) -> str:
    if (
        ...
        raise ValueError(f"Event Prior-Low Rejection 001 {label} differs")
    return value
...
def _required_positive_decimal_text(value: object, label: str) -> None:
    try:
        ...
        raise ValueError(f"Event Prior-Low Rejection 001 {label} differs") from error
...
class _EquivalenceWorker:
    ...
    def __call__(self, task: Mapping[str, object]) -> Mapping[str, object]:
        ...
        if configuration is None:
            raise ValueError("Event Prior-Low Rejection 001 equivalence candidate differs")
        scenario = self.scenarios[scenario_id]
...
def _synthetic_equivalence_bars() -> tuple[OHLCVBar, ...]:
    ...
    for timestamp in expected_bar_timestamps(
        ...
        if day == date(2026, 1, 8) and index < 6:
            spy_open = spy_high = spy_close = Decimal("99")
            spy_low = Decimal("98.90")
        elif day == date(2026, 1, 8):
            spy_open = spy_high = spy_low = spy_close = (
                Decimal("100.20") if index < 30 else Decimal("101")
            )
        else:
            spy_open = spy_high = spy_low = spy_close = Decimal("100")
        bars.extend(
            (
                OHLCVBar(
                    Symbol("QQQ"),
...
def _synthetic_specification(
    ...
    return cast(
        ...
        canonicalize(
            {
                "schema_version": (
                    "intraday-event-prior-low-rejection-001-synthetic-equivalence-run-v1"
                ),
                "program_id": PROGRAM_ID,
                "runner_version": RUNNER_VERSION,
...
def _parallel_equivalence(repository: Path, *, source_commit: str) -> dict[str, object]:
    _require_non_broker_environment()
    plan = load_intraday_event_prior_low_rejection_001_plan(repository.resolve())
    choices = (
    ...
    tasks = tuple(
        cast(
            ...
            canonicalize(
                {
                    "schema_version": (
                        "intraday-event-prior-low-rejection-001-equivalence-task-v1"
                    ),
                    "program_id": PROGRAM_ID,
                    "source_commit": source_commit,
                    "context": {
    ...
    if not equivalent:
        raise ValueError("Event Prior-Low Rejection 001 one-worker/four-worker equivalence differs")
    ...
    return {
        "schema_version": "intraday-event-prior-low-rejection-001-parallel-equivalence-v1",
        "program_id": PROGRAM_ID,
        "verification_source_commit": source_commit,
        "fixture_kind": "synthetic-non-protected-spy-prior-low-rejection-five-minute-bars",
        "protected_inputs_accessed": False,
        "worker_counts": [1, 4],
...
def verify_intraday_event_prior_low_rejection_001_parallel_equivalence(
    repository: Path,
) -> dict[str, object]:
...
def intraday_event_prior_low_rejection_001_plan_summary(repository: Path) -> dict[str, object]:
    repository = repository.resolve()
    plan = load_intraday_event_prior_low_rejection_001_plan(repository)
    launch_bound = False
    if (
...
def intraday_event_prior_low_rejection_001_status(data_home: Path) -> dict[str, object]:
...
def run_intraday_event_prior_low_rejection_001_campaign(
    repository: Path,
    data_home: Path,
    *,
    workers: int = DEFAULT_RESEARCH_WORKERS,
    ...
    return IntradayEventPriorLowRejection001Runner(
        repository,
        data_home,
        workers=workers,
        progress=progress,
