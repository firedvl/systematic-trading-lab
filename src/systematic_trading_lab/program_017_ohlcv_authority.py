"""Single-process Program 017 recovery of the Program 012-016 exposed prefix."""
...
PROGRAM_ID = "multi-hour-sector-etf-research-016"
PROGRAM_ORDINAL = 17
CHILD_AUTHORITY_ID = (
...
PRIVATE_ROOT = Path(".trading-lab/program-017-exposed-prefix-raw-alpaca-sip-v1")
...
PREDECESSOR_PRIVATE_ROOT = PROGRAM_016_PRIVATE_ROOT
PUBLIC_TERMINAL_PATH = Path(
    "config/research/program-017-exposed-prefix-raw-alpaca-sip-recovery-and-"
...
OPERATION_MANIFEST = {
    ...
    "sha256": "10086f90ae320397914f95fdf9c780ae9c782f93a6b65f48f8d9af4f09cd41a7",
    "fingerprint": "ecb8bffdbc5af0c5ffc9d189c1d49d5ca99b6b7d2257dcfa1002d747142f8f98",
}
PROPOSAL_REVIEW = {
    "path": (
        "config/research/program-017-exposed-prefix-raw-alpaca-sip-recovery-and-"
    ...
    "sha256": "e2aaecb0c6cd44e359d0ead286aae5513cd6d08ed75e7aa96a91c0506de81000",
    "fingerprint": "03ac9a8a0109d3d4a65cfd9d6e2f85d1418b73fdc89928a573e8f83ab89bbf32",
}
...
_PRIVATE_EVIDENCE_KEYS = {
    "program_017_response_manifest_sha256",
...
class Program017AuthorityError(ValueError):
    """Fail-closed Program 017 runtime or authority error."""


class Program017PostClaimPersistenceError(Program017AuthorityError):
    """Terminal evidence could not be persisted after possible transport."""


class CombinedRequestBudgetExhausted(Program017AuthorityError):
...
class _IncompletePageCheckpoint(Program017AuthorityError):
    pass
...
class _CombinedProjection:
    ...
    program_017_response_manifest: Mapping[str, Any]
    budget: _Budget
...
class _ControlSnapshot:
    ...
    def _validate_immutable(self) -> None:
        if (
            ...
            raise Program017AuthorityError("Program 017 held control metadata changed")

    def validate_continuation(self) -> None:
        self._validate_immutable()
        if self.failed_write is not None:
            raise Program017AuthorityError("Program 017 is failure-closeout-only")
        if self.failure_only:
            raise Program017AuthorityError("Program 017 is failure-closeout-only")
        if _stat_tuple(self.mutable_root, _FULL_STAT_FIELDS) != self.mutable_expected:
            raise Program017AuthorityError("Program 017 mutable root changed")

    def refresh_after_write(self) -> None:
        self._validate_immutable()
        if self.failure_only and not self.allow_failure_writes:
            raise Program017AuthorityError("Program 017 failed write cannot resume")
    ...
    def validate_failure_closeout(self) -> None:
        ...
        if not self.failure_only:
            raise Program017AuthorityError("Program 017 failure closeout was not latched")
        if _stat_tuple(self.mutable_root, _FULL_STAT_FIELDS) != self.mutable_expected:
            raise Program017AuthorityError("Program 017 failure-closeout root changed")
    ...
    def begin_failure_closeout(self) -> None:
        ...
        if current != expected:
            raise Program017AuthorityError("Program 017 failed-write root changed")
...
class _Budget:
    def __init__(self, predecessor_state: _PredecessorState) -> None:
        ...
        if completed_requests != completed_responses or consumed_intents != 5:
            raise Program017AuthorityError("Program 017 predecessor budget differs")
        ...
        if (
            ...
            raise Program017AuthorityError("Program 017 inherited budget exceeds its ceiling")
    ...
    def reserve_request(self) -> None:
        if self.requests >= science.MAXIMUM_REQUESTS_AND_RESPONSES:
            raise CombinedRequestBudgetExhausted(
                "Program 017 combined request ceiling is exhausted"
            )
        self.requests += 1
    ...
    def accept_response(
        ...
        if (
            ...
            raise Program017AuthorityError("Program 017 response timestamp must be UTC")
        if self.responses >= self.effective_response_ceiling:
            raise CombinedRequestBudgetExhausted(
                "Program 017 combined response ceiling is exhausted"
            )
        self.responses += 1
        ...
        if len(body) > science.MAXIMUM_RESPONSE_PAGE_BYTES:
            raise Program017AuthorityError("Program 017 response exceeds the 8 MiB page ceiling")
        if self.session_bytes[request.session] > science.MAXIMUM_SESSION_RESPONSE_BYTES:
            raise Program017AuthorityError("Program 017 session exceeds the 8 MiB byte ceiling")
        if self.response_bytes > science.MAXIMUM_TOTAL_RESPONSE_BYTES:
            raise Program017AuthorityError("Program 017 total response byte ceiling exceeded")
...
class _ReadLockedRoot:
    ...
    def __enter__(self) -> None:
        try:
            ...
            raise Program017AuthorityError(f"{self._program} read-only lock is absent") from error
        self._handle = os.fdopen(descriptor, "rb", buffering=0)
        metadata = os.fstat(self._handle.fileno())
        if not stat.S_ISREG(metadata.st_mode) or stat.S_IMODE(metadata.st_mode) & 0o077:
            self._handle.close()
            raise Program017AuthorityError(f"{self._program} read-only lock is invalid")
...
def credential_presence_preflight(
    ...
    with _locked_controls(repository, authority, create_program_017=True) as (
        program_017_root,
        program_016_root,
        program_015_root,
        program_014_root,
        ...
        if locked != authority:
            raise Program017AuthorityError("Program 017 authority changed under policy lock")
        ...
        _ROOT_TRACKERS[program_017_root] = snapshot
        try:
...
def validate_operation_contract(
    ...
    try:
        program_016.validate_operation_contract(repository, commit=commit)
    except program_016.Program016AuthorityError as error:
        raise Program017AuthorityError(str(error).replace("Program 016", "Program 017")) from None
    ...
    first = _mapping(
        recovery.get("first_program_017_request_private_derivation"), "first request contract"
    )
    if (
        proposal.get("schema_version")
        != "program-017-exposed-prefix-raw-alpaca-sip-recovery-and-structural-admission-proposal-v5"
        ...
        or review.get("reviewed_source_commit") != "aa17bd0fc7f9201f8c0a3458dae8e54ad11e75aa"
        or review.get("reviewed_source_tree") != "e960fc7db444d814f940657b44f25ada10356041"
        or review.get("reviewed_proposal") != OPERATION_MANIFEST
        or recovery.get("predecessor_private_roots")
        != [
        ...
        or recovery.get("program_017_private_root") != PRIVATE_ROOT.as_posix()
        or recovery.get("incomplete_program_016_session_or_page_reuse_allowed") is not False
        or recovery.get("incomplete_program_016_request_reissue_allowed") is not False
        or recovery.get("program_017_request_reissue_after_durable_intent_allowed") is not False
        or recovery.get("ambiguous_program_017_transport_is_consumed_no_retry") is not True
        or any(
            recovery.get(key) is not False
            for key in (
                "retained_program_017_response_retry_allowed",
                "retained_program_017_http_result_retry_allowed",
                "parsed_program_017_missingness_retry_allowed",
                "program_017_data_quality_outcome_retry_allowed",
            )
        )
        ...
        or boundary.get("all_seven_controls_held_continuously") is not True
        ...
        or boundary.get("program_017_mutable_root_stable_identity_tuple")
        ...
        != science.MAXIMUM_REQUESTS_AND_RESPONSES - 5
        or cumulative.get("consumed_intent_without_response_count") != 5
        ...
        or credentials.get("credential_parsing_location")
        != "INLINE-ONLY-IN-LOCK-BOUND-LOADER-OPERATION"
        or credentials.get("launcher_revalidated_immediately_before_credential_attempt") is not True
        or credentials.get("process_global_latch_consumed_before_environment_access") is not True
        or credentials.get("process_global_latch_consumed_if_attempt_reservation_or_parsing_fails")
        is not True
        ...
        or private_terminal.get("schema_version") != "program-017-private-terminal-v1"
        ...
        or static_private
        != {
            "schema_version": "program-017-private-terminal-v1",
        ...
        or public_terminal.get("schema_version") != "program-017-exposed-prefix-terminal-result-v1"
        or set(_sequence(public_terminal.get("exact_top_level_keys"), "public terminal keys"))
        != _PUBLIC_TERMINAL_KEYS
        or static_public
        != {
            "schema_version": "program-017-exposed-prefix-terminal-result-v1",
        ...
        != _ENABLED_AUTHORITY
        or child.get(
            "source_qualification_strategy_research_qualification_controlled_protected_paper_broker_live_and_purchase_allowed"
        )
        is not False
        or firewall.get("inherit_program_016_registered_range_derivation") is not True
        or firewall.get(
        ...
        raise Program017AuthorityError("Program 017 operation contract differs")
    return proposal
...
def _derive_control_validated_authority(repository: Path) -> Mapping[str, Any]:
    ...
    try:
        ...
        raise Program017AuthorityError(str(error)) from None
    ...
    if (
        ...
        raise Program017AuthorityError("Program 017 reviewed child identity differs")
    ...
    try:
        ...
        raise Program017AuthorityError(str(error).replace("Program 011", "Program 017")) from None
    ...
    if _repository_preflight(repository, identity) != lineage:
        raise Program017AuthorityError("Program 017 repository changed during validation")
...
def _locked_controls(
    ...
    *,
    create_program_017: bool,
) -> Iterator[tuple[int, int, int, int, int, int]]:
    program_017_root = _open_root(repository, PRIVATE_ROOT, create=create_program_017)
    try:
        with git_controls._LockedRoot(program_017_root):
            program_016_root = _open_root(repository, PROGRAM_016_PRIVATE_ROOT, create=False)
            try:
                with _ReadLockedRoot(program_016_root, "Program 016"):
                    program_015_root = _open_root(
                        repository, PROGRAM_015_PRIVATE_ROOT, create=False
                    )
                    try:
                        with _ReadLockedRoot(program_015_root, "Program 015"):
                            program_014_root = _open_root(
                                repository, PROGRAM_014_PRIVATE_ROOT, create=False
                            )
                            try:
                                with _ReadLockedRoot(program_014_root, "Program 014"):
                                    program_013_root = _open_root(
                                        repository, PROGRAM_013_PRIVATE_ROOT, create=False
                                    )
                                    try:
                                        with _ReadLockedRoot(program_013_root, "Program 013"):
                                            program_012_root = _open_root(
                                                repository,
                                                PROGRAM_012_PRIVATE_ROOT,
                                                create=False,
                                            )
                                            try:
                                                with (
                                                    _ReadLockedRoot(
                                                        program_012_root, "Program 012"
                                                    ),
                                                    git_controls._GitPolicySnapshot(
                                                        repository, _authority_commit(authority)
                                                    ),
                                                ):
                                                    yield (
                                                        program_017_root,
                                ...
                                os.close(program_014_root)
                    finally:
                        os.close(program_015_root)
            finally:
                os.close(program_016_root)
    except git_controls.Program011AuthorityError as error:
        raise Program017AuthorityError(str(error).replace("Program 011", "Program 017")) from None
    finally:
        os.close(program_017_root)
...
def _activate_locked_authority(
    ...
    if entries not in (
        ...
        raise Program017AuthorityError("Program 017 one-use authority state already exists")
...
def _execute_mock_acquisition(
    ...
    *,
    ...
    if type(transport) is not MockBarsTransport or environ is os.environ:
        raise Program017AuthorityError("Program 017 test execution requires finite inputs")
    return _execute_acquisition(
        repository,
        environ=environ,
...
def _execute_acquisition(
    ...
    *,
    ...
    with _locked_controls(repository, expected_authority, create_program_017=True) as (
        program_017_root,
        program_016_root,
        program_015_root,
        program_014_root,
        ...
        _ROOT_TRACKERS[program_017_root] = control_snapshot
        try:
            if _derive_control_validated_authority(repository) != authority:
                raise Program017AuthorityError("Program 017 authority changed under lock")
            _recover_private_terminal_if_present(
                repository,
                program_017_root,
                program_016_root,
            ...
            if after_activation is not None:
                after_activation()
            _validate_launcher(program_017_root, authority, predecessor_state)
            _load_active(program_017_root, authority)
            ...
            def consume() -> None:
                ...
                if predecessor._exists(program_017_root, "claim.json"):
                    _validate_claim(program_017_root, authority, predecessor_state)
                    return
                _tracked_append_atomic(
                    program_017_root,
                    "claim.json",
            ...
            temp_key, temp_descriptor = _tracked_new_temp(
                program_017_root, "combined-canonical-raw"
            )
            completed = False
            try:
                ...
                if not completed:
                    with suppress(FileNotFoundError):
                        _tracked_unlink(program_017_root, temp_key)
            ...
            if budget.new_requests != budget.new_responses:
                raise Program017AuthorityError("Program 017 new request and response counts differ")
            ...
            for key, payload in cast(Mapping[str, bytes], evidence["payloads"]).items():
                _tracked_append_or_validate(program_017_root, key, payload)
            _FINAL_VALIDATIONS[program_017_root] = (projection, evidence)
            ...
            _tracked_append_atomic(program_017_root, _TERMINAL_KEY, terminal_payload)
            ...
            try:
                ...
                raise Program017PostClaimPersistenceError(
                    "Program 017 public terminal persistence failed after possible transport"
            ...
            if (
                ...
                or _has_consumed_state(program_017_root)
            ) and not predecessor._exists(program_017_root, _TERMINAL_KEY):
                try:
            ...
            if published_failure and isinstance(error, Exception):
                raise Program017AuthorityError(
                    "Program 017 acquisition ended with a sealed failure"
                ) from None
            raise
        finally:
            _FINAL_VALIDATIONS.pop(program_017_root, None)
            _ROOT_TRACKERS.pop(program_017_root, None)
        ...
        if not self._loaded:
            ...
            try:
                ...
                if any(not value or "\r" in value or "\n" in value for value in credentials):
                    raise Program017AuthorityError("Program 017 OHLCV credentials are required")
...
class _PersistentSessionSource:
    ...
    def response(self, intent: program_011.PageIntent) -> raw_contract.RawResponse:
        if self._pending is not None:
            raise Program017AuthorityError("Program 017 persistent source state differs")
        ...
        if any(present):
            if present[0]:
                ...
                if predecessor._read(self._root_descriptor, f"{prefix}.intent.json") != expected:
                    raise Program017AuthorityError(
                        "Program 017 partial request intent checkpoint differs"
                    )
            raise Program017AuthorityError(
                "Program 017 ambiguous page checkpoint forbids request reissue"
            )
    ...
    def retain(self, page_index: int, body: bytes) -> program_011.RetainedPage:
        ...
        if (
            ...
            raise Program017AuthorityError("Program 017 retained response differs")
        self._pending = None
        return pending
...
class _ReplaySource:
    ...
    def response(self, intent: program_011.PageIntent) -> raw_contract.RawResponse:
        ...
        if not all(present):
            if (
                ...
                raise Program017AuthorityError("Program 017 partial page checkpoint differs")
            self._budget.reserve_request()
    ...
    def retain(self, page_index: int, body: bytes) -> program_011.RetainedPage:
        ...
        if pending is None or pending.page_index != page_index:
            raise Program017AuthorityError("Program 017 replayed response differs")
        self._pending = None
        return pending
...
class _ExistingSessionSource:
    ...
    def retain(self, page_index: int, body: bytes) -> program_011.RetainedPage:
        ...
        if (
            ...
            raise Program017AuthorityError("Program 017 existing response differs")
        self._pending = None
        return pending
...
def _combined_projection(
    ...
    *,
    ...
    program_015_state = predecessor_state.program_015_state
    ...
    discarded_program_016_index = predecessor_state.frontier_request_index - 1
    ...
    for request_index, request in enumerate(science.acquisition_requests()):
        if request_index < program_012_frontier:
            ...
            source = _ExistingSessionSource(program_017_root, request, authority, predecessor_state)
        else:
        ...
        for row in result.rows:
            ...
            if canonical_bytes > _MAXIMUM_CANONICAL_PROJECTION_BYTES:
                raise Program017AuthorityError("Program 017 canonical projection exceeds 8 GiB")
            if canonical_file is not None and canonical_file.write(line) != len(line):
                raise Program017AuthorityError("Program 017 canonical raw write was incomplete")
            canonical_hash.update(line)
            raw_row_count += 1
    discarded = _mapping(
        predecessor_state.manifest.get("discarded_program_016_partial_session"),
        "discarded Program 016 session",
    )
    discarded_pages = _sequence(discarded.get("completed_pages"), "discarded Program 016 pages")
    discarded_bytes = sum(
    ...
    if (
        ...
        raise Program017AuthorityError("Program 017 predecessor replay budget differs")
    response_manifest = {
        "schema_version": "program-017-private-response-manifest-v1",
        "program_id": PROGRAM_ID,
    ...
    return _CombinedProjection(
        ...
        missing=missing,
        morning=morning,
        program_017_response_manifest=response_manifest,
        budget=budget,
    )
...
def _execute_session(
    ...
    try:
        ...
        replacement = Program017AuthorityError(str(error).replace("Program 012", "Program 017"))
        ...
        raise Program017AuthorityError(str(error).replace("Program 012", "Program 017")) from error
...
def _reconstruct_state(
    ...
    *,
    ...
    if entries & _DERIVED_KEYS and _TERMINAL_KEY not in entries:
        raise Program017AuthorityError("Program 017 derived evidence lacks a terminal")
    allowed = {
        "active-authority.json",
        "claim.json",
        _LAUNCHER_KEY,
    ...
    if page_entries:
        if claim_exists:
            if successful_loads == 0:
                raise Program017AuthorityError(
                    "Program 017 page evidence lacks a successful credential audit"
                )
            ...
            raise Program017AuthorityError("Program 017 provider evidence lacks a transport claim")
    elif claim_exists:
        raise Program017AuthorityError("Program 017 transport claim lacks page evidence")
    if credential_loads and not page_entries:
        raise Program017AuthorityError("Program 017 credential audit lacks request evidence")
    ...
    for entry in page_entries:
        ...
        try:
            ...
            raise Program017AuthorityError("Program 017 page key is invalid") from error
        if session not in expected_sessions or not 1 <= page_index <= 16:
            raise Program017AuthorityError("Program 017 page key is outside the recovery plan")
    ...
    for request in requests[predecessor_state.frontier_request_index :]:
        ...
        if frontier_found:
            if session_entries:
                raise Program017AuthorityError("Program 017 checkpoint skips a request session")
            continue
        ...
        try:
            ...
            if frozenset(session_entries) not in {
                ...
                raise Program017AuthorityError(
                    "Program 017 terminal checkpoint is not a reachable prefix"
                ) from None
            frontier_found = True
            ...
            if session_entries != predecessor._expected_session_entries(request, completed_pages):
                raise Program017AuthorityError(
                    "Program 017 checkpoint skips a request page"
                ) from None
            if require_complete:
                raise Program017AuthorityError(
                    "Program 017 acquisition evidence is incomplete"
                ) from None
            frontier_found = True
            ...
            if session_entries != predecessor._expected_session_entries(request, len(result.pages)):
                raise Program017AuthorityError(
                    "Program 017 checkpoint continues after a terminal page"
                )
    if require_complete and frontier_found:
        raise Program017AuthorityError("Program 017 acquisition evidence is incomplete")
    return budget
...
def _claim_value(
    ...
    unsigned: dict[str, Any] = {
        "schema_version": "program-017-private-acquisition-claim-v1",
        "program_id": PROGRAM_ID,
        "authority_id": authority["authority_id"],
        "authority_fingerprint": authority["authority_fingerprint"],
        "operation_manifest": OPERATION_MANIFEST,
...
def _validate_claim(
    ...
    if predecessor._read(root_descriptor, "claim.json") != _claim_payload(
        ...
        raise Program017AuthorityError("Program 017 transport claim differs")
...
def _intent_value(
    ...
    unsigned: dict[str, Any] = {
        "schema_version": "program-017-private-request-intent-v1",
        "program_id": PROGRAM_ID,
        "authority_fingerprint": authority["authority_fingerprint"],
        "operation_manifest": OPERATION_MANIFEST,
        "runtime_binding": dict(_runtime_binding(authority)),
...
def _response_receipt(
    ...
    if type(response.status) is not int or not 100 <= response.status <= 599:
        raise Program017AuthorityError("Program 017 response status is invalid")
    ...
    claim = _claim_value(authority, predecessor_state)
    unsigned: dict[str, Any] = {
        "schema_version": "program-017-private-response-receipt-v1",
        "program_id": PROGRAM_ID,
        "authority_fingerprint": authority["authority_fingerprint"],
        "operation_manifest": OPERATION_MANIFEST,
        "runtime_binding": dict(_runtime_binding(authority)),
...
def _load_completed_page(
    ...
    if predecessor._read(root_descriptor, f"{prefix}.intent.json") != expected_intent:
        raise Program017AuthorityError("Program 017 request intent checkpoint differs")
    ...
    if (
        ...
        raise Program017AuthorityError("Program 017 response checkpoint differs")
...
def _validate_page_intent(
    ...
    if (
        ...
        raise Program017AuthorityError("Program 017 request intent differs")
    ...
    if (
        ...
        raise Program017AuthorityError("Program 017 first intent differs from the frontier")
...
def _credential_attempt(
    ...
    if type(sequence) is not int or sequence < 1:
        raise Program017AuthorityError("Program 017 credential sequence differs")
    unsigned: dict[str, Any] = {
        "schema_version": "program-017-private-credential-load-attempt-v1",
        "authority_fingerprint": authority["authority_fingerprint"],
        "runtime_binding": dict(_runtime_binding(authority)),
...
def _credential_receipt_payload(attempt_identity: str, status: str) -> bytes:
    if _HEX_64.fullmatch(attempt_identity) is None or status not in {"SUCCESS", "FAILURE"}:
        raise Program017AuthorityError("Program 017 credential receipt differs")
    value = {
        "schema_version": "program-017-private-credential-load-receipt-v1",
...
def _launcher_value(
    ...
    unsigned = {
        "schema_version": "program-017-private-single-process-launcher-v1",
        "authority_fingerprint": authority["authority_fingerprint"],
        "predecessor_import_manifest_sha256": predecessor_state.sha256,
...
def _validate_launcher(
    ...
    if (
        ...
        raise Program017AuthorityError("Program 017 launcher changed or the process was restarted")
...
def _credential_load_counts(
    ...
    for entry in os.listdir(root_descriptor):
        ...
        if raw != (canonical_json(value) + "\n").encode():
            raise Program017AuthorityError("Program 017 credential audit is not canonical")
        target = attempts if match.group("kind") == "attempt" else receipts
        if sequence in target:
            raise Program017AuthorityError("Program 017 credential sequence is duplicated")
        target[sequence] = value
    sequences = sorted(attempts)
    if sequences != list(range(1, len(sequences) + 1)) or not set(receipts) <= set(attempts):
        raise Program017AuthorityError("Program 017 credential sequence is not contiguous")
    ...
    for sequence in sequences:
        ...
        if set(attempt) != _CREDENTIAL_ATTEMPT_KEYS or attempt != expected_attempt:
            raise Program017AuthorityError("Program 017 credential attempt binding differs")
        receipt = receipts.get(sequence)
        if receipt is None:
            if sequence != sequences[-1]:
                raise Program017AuthorityError("Program 017 credential receipt sequence has a gap")
            continue
        if (
            set(receipt) != _CREDENTIAL_RECEIPT_KEYS
            or receipt.get("schema_version") != "program-017-private-credential-load-receipt-v1"
            ...
            raise Program017AuthorityError("Program 017 credential receipt binding differs")
        if receipt["status"] == "SUCCESS":
            successful += 1
        elif sequence != sequences[-1]:
            raise Program017AuthorityError("Program 017 failed credential load is not terminal")
...
def _reserve_process_credential_access(write_attempt: Callable[[], None]) -> None:
    ...
    with _PROCESS_CREDENTIAL_LOCK:
        if process_id == _PROCESS_CREDENTIAL_PID:
            raise Program017AuthorityError(
                "Program 017 credential access is already consumed in this process"
            )
        try:
            write_attempt()
        finally:
...
def _revalidate_transport_boundary(
    ...
    if _derive_control_validated_authority(repository) != authority:
        raise Program017AuthorityError("Program 017 authority changed at transport boundary")
    snapshot = _ROOT_TRACKERS.get(program_017_root)
    if snapshot is None:
        raise Program017AuthorityError("Program 017 control snapshot is absent")
    snapshot.validate_continuation()
    _validate_predecessor_manifest(program_017_root, predecessor_state)
    _require_working_disk_capacity(program_017_root)
...
def _revalidate_closeout_boundary(
    ...
    if _derive_control_validated_authority(repository) != authority:
        raise Program017AuthorityError("Program 017 authority changed at closeout")
...
def _revalidate_failure_closeout_boundary(
    ...
    if _derive_control_validated_authority(repository) != authority:
        raise Program017AuthorityError("Program 017 authority changed at failure closeout")
    snapshot = _ROOT_TRACKERS.get(root_descriptor)
    if snapshot is not None:
...
def _require_zero_protected_overlap(repository: Path, commit: str) -> None:
    ...
    if any(
        ...
        raise Program017AuthorityError("Program 017 request chronology overlaps protected data")
...
def _require_working_disk_capacity(root_descriptor: int) -> None:
    ...
    if filesystem.f_bavail * filesystem.f_frsize < _WORKING_DISK_RESERVATION_BYTES:
        raise Program017AuthorityError("Program 017 requires 16 GiB of available working space")
...
def _runtime_binding(authority: Mapping[str, Any]) -> Mapping[str, Any]:
    ...
    if (
        ...
        raise Program017AuthorityError("Program 017 runtime binding differs")
    return runtime


def _count(value: Any, label: str) -> int:
    if type(value) is not int or value < 0:
        raise Program017AuthorityError(f"Program 017 {label} differs")
    return value
...
def _derived_admission_evidence(
    ...
    program_017_proposal = validate_operation_contract(
    ...
    admission["schema_version"] = "program-017-private-structural-admission-report-v1"
    admission["program_id"] = PROGRAM_ID
    admission["status"] = (
        "ADMITTED-PROGRAM-017-RAW-STRUCTURAL-PREFIX"
    ...
    missing_value = {
        "schema_version": "program-017-private-combined-missing-coordinates-v1",
        "program_id": PROGRAM_ID,
        "predecessor_import_manifest_sha256": predecessor_state.sha256,
        "sessions": {
    ...
    response_payload = (canonical_json(projection.program_017_response_manifest) + "\n").encode()
    ...
    private_dataset_identity = fingerprint(
        {
            ...
            "predecessor_import_manifest_sha256": predecessor_state.sha256,
            "program_017_response_manifest_sha256": response_sha,
    ...
    public_lineage = (
        {
            "schema_version": ("program-017-public-raw-structural-prefix-lineage-manifest-v1"),
            "program_id": PROGRAM_ID,
            "authority_id": authority["authority_id"],
            "authority_fingerprint": authority["authority_fingerprint"],
            "source_commit": _source_commit(authority),
            "status": "ADMITTED-PROGRAM-017-RAW-STRUCTURAL-PREFIX",
    ...
    if public_lineage is not None and (
        ...
        raise Program017AuthorityError("Program 017 public and private identities collide")
    dataset_manifest = {
        "schema_version": "program-017-private-combined-dataset-manifest-v1",
        "program_id": PROGRAM_ID,
        "authority_id": authority["authority_id"],
        "authority_fingerprint": authority["authority_fingerprint"],
        "source_commit": _source_commit(authority),
    ...
    return {
        ...
        "payloads": {
            "program-017-response-manifest.json": response_payload,
...
def _admission_terminal_payload(
    ...
    if _HEX_64.fullmatch(private_identity) is None:
        raise Program017AuthorityError("Program 017 private dataset identity differs")
    lineage_identity = (
    ...
    record: dict[str, Any] = {
        ...
        "program_017_credential_loads": _credential_load_counts(
            root_descriptor, authority, predecessor_state
        )[0],
...
def _runtime_failure_payload(
    ...
    private_evidence = {
        "program_017_response_manifest_sha256": _evidence_sha256_if_present(
            root_descriptor, "program-017-response-manifest.json"
        ),
    ...
    record: dict[str, Any] = {
        ...
        "program_017_credential_loads": _runtime_failure_credential_load_count(
            root_descriptor, authority, predecessor_state
        ),
...
def _load_terminal_record(
    ...
    if (
        ...
        or record.get("schema_version") != "program-017-private-terminal-v1"
        ...
        raise Program017AuthorityError("Program 017 private terminal binding differs")
    ...
    if _count(
        record.get("program_017_credential_loads"), "terminal credential count"
    ) != durable_credentials or record.get(
        "provider_transport_attempted"
    ) is not predecessor._exists(root_descriptor, "claim.json"):
        raise Program017AuthorityError("Program 017 terminal credential count differs")
    ...
    if result_kind == "RUNTIME-FAILURE":
        ...
        if (
            ...
            raise Program017AuthorityError("Program 017 runtime terminal semantics differ")
        _validate_runtime_failure_counts(record, counts)
        for field, key in (
            ("program_017_response_manifest_sha256", "program-017-response-manifest.json"),
            ...
            if private_evidence[field] != _evidence_sha256_if_present(root_descriptor, key):
                raise Program017AuthorityError("Program 017 runtime evidence hash differs")
        ...
        for key, expected in payloads.items():
            if predecessor._read(root_descriptor, key) != expected:
                raise Program017AuthorityError("Program 017 derived evidence differs")
        if (
            _evidence_sha256(root_descriptor, "combined-canonical-raw.jsonl")
            != projection.canonical_sha256
        ):
            raise Program017AuthorityError("Program 017 combined canonical evidence differs")
        admission = _mapping(evidence.get("admission"), "structural admission")
        admission_passed = admission.get("admission_passed") is True
        ...
        if (
            ...
            raise Program017AuthorityError("Program 017 admission terminal semantics differ")
        _validate_budget_counts(record, projection.budget)
    else:
        raise Program017AuthorityError("Program 017 private terminal result kind differs")
    return record
...
def _validate_budget_counts(record: Mapping[str, Any], budget: _Budget) -> None:
    if (
        ...
        raise Program017AuthorityError("Program 017 terminal budget differs")
...
def _publish_public_terminal(
    ...
    if validated != record:
        raise Program017AuthorityError("Program 017 terminal changed before publication")
    ...
    try:
        ...
        try:
            with os.fdopen(temp_descriptor, "wb") as handle:
                if handle.write(payload) != len(payload):
                    raise Program017AuthorityError(
                        "Program 017 public terminal write was incomplete"
                    )
                handle.flush()
            ...
            try:
                ...
                raise Program017AuthorityError(
                    "Program 017 public terminal artifact already exists"
                ) from None
            os.fsync(public_descriptor)
        finally:
            with suppress(FileNotFoundError):
...
def _recover_private_terminal_if_present(
    ...
    try:
        ...
        _load_active(root_descriptor, authority)
        raise Program017AuthorityError("Program 017 single-process transaction was interrupted")
        ...
        raise Program017AuthorityError("Program 017 acquisition is terminally sealed") from None
...
def _seal_runtime_failure(
    ...
    snapshot = _ROOT_TRACKERS.get(root_descriptor)
    if snapshot is None:
        raise Program017AuthorityError("Program 017 failure control snapshot is absent")
...
def _reject_terminal_state(repository: Path) -> None:
    ...
    try:
        ...
        if _PUBLIC_TERMINAL_SHA256 is None:
            return
        raise Program017AuthorityError("Program 017 terminal result artifact is absent") from error
    except OSError as error:
        raise Program017AuthorityError("Program 017 terminal result artifact is invalid") from error
    try:
        metadata = os.fstat(descriptor)
        if not stat.S_ISREG(metadata.st_mode) or metadata.st_size > 1_048_576:
            raise Program017AuthorityError("Program 017 terminal result artifact is invalid")
        with os.fdopen(descriptor, "rb") as handle:
            descriptor = -1
            raw = handle.read()
    finally:
    ...
    if raw != (canonical_json(value) + "\n").encode():
        raise Program017AuthorityError("Program 017 public terminal is not canonical")
    _validate_public_terminal_shape(value)
    if _PUBLIC_TERMINAL_SHA256 is None:
        raise Program017AuthorityError("Program 017 terminal result lacks immutable binding")
    if hashlib.sha256(raw).hexdigest() != _PUBLIC_TERMINAL_SHA256:
        raise Program017AuthorityError("Program 017 terminal result semantics differ")
    raise Program017AuthorityError("Program 017 authority is terminally revoked")


def _validate_public_terminal_shape(value: Mapping[str, Any]) -> None:
    if (
        set(value) != _PUBLIC_TERMINAL_KEYS
        or value.get("schema_version") != "program-017-exposed-prefix-terminal-result-v1"
        ...
        raise Program017AuthorityError("Program 017 public terminal schema differs")
    ...
    if result_kind == "RUNTIME-FAILURE":
        ...
        expected_status = "ADMITTED-PROGRAM-017-RAW-STRUCTURAL-PREFIX"
        expected_admission = True
    else:
        raise Program017AuthorityError("Program 017 public terminal result kind differs")
    if (
        value.get("status") != expected_status
        or value.get("admission_passed") is not expected_admission
    ):
        raise Program017AuthorityError("Program 017 public terminal branch differs")
    lineage = value.get("dataset_lineage_manifest")
    if expected_admission:
        _validate_public_lineage(lineage, value)
    elif lineage is not None:
        raise Program017AuthorityError("Program 017 failure terminal publishes lineage")
...
def _validate_public_lineage(value: Any, terminal: Mapping[str, Any]) -> dict[str, Any]:
    ...
    if (
        ...
        != "program-017-public-raw-structural-prefix-lineage-manifest-v1"
        ...
        or lineage.get("status") != "ADMITTED-PROGRAM-017-RAW-STRUCTURAL-PREFIX"
        or _HEX_64.fullmatch(str(lineage.get("dataset_lineage_identity"))) is None
    ):
        raise Program017AuthorityError("Program 017 public dataset lineage differs")
    return lineage


def _exact_object(value: Any, keys: set[str], label: str) -> dict[str, Any]:
    if type(value) is not dict or set(value) != keys:
        raise Program017AuthorityError(f"Program 017 {label} schema differs")
...
def _evidence_sha256(root_descriptor: int, key: str) -> str:
    if _EVIDENCE_KEY.fullmatch(key) is None:
        raise Program017AuthorityError("Program 017 evidence key is invalid")
    try:
        descriptor = os.open(
            key,
            os.O_RDONLY | getattr(os, "O_CLOEXEC", 0) | getattr(os, "O_NOFOLLOW", 0),
        ...
        raise Program017AuthorityError(f"Program 017 evidence is absent: {key}") from error
    with os.fdopen(descriptor, "rb") as handle:
        metadata = os.fstat(handle.fileno())
        if not stat.S_ISREG(metadata.st_mode) or stat.S_IMODE(metadata.st_mode) & 0o077:
            raise Program017AuthorityError("Program 017 evidence is not private")
    ...
    def publish() -> None:
        if predecessor._exists(root_descriptor, final_key):
            if _evidence_sha256(root_descriptor, final_key) != expected_sha256:
                raise Program017AuthorityError("Program 017 canonical raw dataset differs")
        else:
            try:
                os.link(
                    temp_key,
                ...
                raise Program017AuthorityError("Program 017 canonical raw publish raced") from None
            os.fsync(root_descriptor)
...
def _historical_fingerprinted_artifact(
    ...
    if _HEX_64.fullmatch(str(stored)) is None or stored != fingerprint(unsigned):
        raise Program017AuthorityError(f"Program 017 historical {label} fingerprint differs")
    return value, {
        "path": path.as_posix(),
...
def _validate_historical_source_binding(repository: Path, runtime: Mapping[str, Any]) -> None:
    ...
    if (
        ...
        raise Program017AuthorityError("Program 016 historical runtime binding differs")
    seen: set[str] = set()
    for item in source_files:
        ...
        if not path or path in seen or _HEX_64.fullmatch(str(binding.get("sha256"))) is None:
            raise Program017AuthorityError("Program 016 historical runtime source differs")
        seen.add(path)
...
def _historical_program_016_identity(
    repository: Path,
    proposal: Mapping[str, Any],
    active: Mapping[str, Any],
    program_017_commit: str,
) -> Mapping[str, Any]:
    forensic_binding = _mapping(
        _mapping(proposal.get("predecessor"), "predecessor bindings").get(
            "redacted_private_forensic_disposition"
        ),
        "Program 017 forensic binding",
    )
    forensic = _load_bound(
        repository,
        forensic_binding,
        "forensic_disposition_fingerprint",
        commit=program_017_commit,
    )
    historical = _mapping(forensic.get("predecessor"), "historical Program 016 binding")
    child_binding = _mapping(historical.get("child_authority"), "Program 016 child binding")
    review_binding = _mapping(
        historical.get("child_authority_review"), "Program 016 child review binding"
    )
    sync_commit = str(
        _mapping(active.get("control_lineage"), "Program 016 control lineage").get(
            "synchronized_main_commit"
        )
    )
    if _HEX_40.fullmatch(sync_commit) is None:
        raise Program017AuthorityError("Program 016 synchronized-main binding differs")
    ...
    runtime = _mapping(child.get("runtime_binding"), "Program 016 child runtime binding")
    child_authority = _mapping(child.get("authority"), "Program 016 child authority")
    required_challenges = tuple(
        cast(
            Sequence[str],
            _sequence(child.get("required_review_challenges"), "Program 016 review challenges"),
        )
    )
    ...
    if (
        child.get("child_authority_id") != program_016.CHILD_AUTHORITY_ID
        or child.get("program_ordinal") != program_016.PROGRAM_ORDINAL
        or child.get("program_id") != program_016.PROGRAM_ID
        or child.get("operation_manifest") != program_016.OPERATION_MANIFEST
        ...
        raise Program017AuthorityError("Program 016 historical reviewed child differs")
...
def _derive_predecessor_state(
    ...
    terminal_binding = _mapping(
        predecessor_bindings.get("program_016_terminal_result"),
        "Program 016 public terminal binding",
    )
    ...
    active_raw = predecessor._read(program_016_root, "active-authority.json")
    active = _json_object(active_raw, "Program 016 active authority")
    if active_raw != (canonical_json(active) + "\n").encode():
        raise Program017AuthorityError("Program 016 active authority is not canonical")
    unsigned_active = dict(active)
    active_fingerprint = unsigned_active.pop("authority_fingerprint", None)
    control_lineage = _mapping(active.get("control_lineage"), "Program 016 control lineage")
    runtime = _mapping(active.get("runtime_binding"), "Program 016 runtime binding")
    source_commit = str(control_lineage.get("runtime_source_commit"))
    historical_identity = _historical_program_016_identity(
        repository, proposal, active, _authority_commit(authority)
    )
    expected_unsigned = {
        "schema_version": "program-016-raw-sip-recovery-active-authority-v1",
        "status": "ACTIVE-ONE-USE-RECOVERABLE",
        "authority_id": program_016.CHILD_AUTHORITY_ID,
        "program_id": program_016.PROGRAM_ID,
        "activation_mode": "INTERNAL-STANDING-MANDATE-DERIVATION",
        "external_authorization_root_required": False,
        "child_identity_fingerprint": historical_identity["child_identity_fingerprint"],
        "operation_manifest": program_016.OPERATION_MANIFEST,
        "consumption_boundary": program_016.CONSUMPTION_BOUNDARY,
    ...
    if (
        ...
        raise Program017AuthorityError("Program 016 active authority binding differs")
    sync_commit = str(control_lineage.get("synchronized_main_commit"))
    try:
        program_016.validate_operation_contract(repository, commit=sync_commit)
        program_015_state = program_016._derive_predecessor_state(
            repository,
            program_015_root,
            program_014_root,
            program_013_root,
        ...
        if predecessor._read(program_016_root, "predecessor-import-manifest.json") != (
            program_015_state.payload
        ):
            raise Program017AuthorityError("Program 016 predecessor import manifest differs")
        private_terminal = program_016._load_terminal_record(
            repository,
            program_016_root,
            program_015_root,
            program_014_root,
    ...
    except (program_016.Program016AuthorityError, predecessor.Program012AuthorityError) as error:
        raise Program017AuthorityError(str(error).replace("Program 016", "Program 017")) from None
    if (
        ...
        or program_016._public_terminal_payload(private_terminal)
        != _git(repository, "show", f"{_authority_commit(authority)}:{terminal_binding['path']}")
    ):
        raise Program017AuthorityError("Program 016 terminal disposition differs")

    requests = science.acquisition_requests()
    page_entries = {
        entry for entry in os.listdir(program_016_root) if program_016._PAGE_KEY.fullmatch(entry)
    }
    budget = program_016._Budget(program_015_state)
    completed_sessions: list[dict[str, Any]] = []
    frontier: dict[str, Any] | None = None
    frontier_request_index = -1
    for request_index, request in enumerate(
        requests[program_015_state.frontier_request_index :],
        start=program_015_state.frontier_request_index,
    ):
        session_entries = {
            entry for entry in page_entries if entry.startswith(f"session-{request.session}-")
        }
        if frontier is not None:
            if session_entries:
                raise Program017AuthorityError("Program 016 evidence continues after frontier")
            continue
        source = program_016._ReplaySource(
            program_016_root, request, active, program_015_state, budget
        )
        before_requests = budget.requests
        before_responses = budget.responses
        before_bytes = budget.response_bytes
        try:
            result = program_016._execute_session(request, source)
        except program_016._IncompletePageCheckpoint:
            completed_pages = budget.responses - before_responses
            page_index = completed_pages + 1
            if completed_pages != 1 or page_index != 2:
                raise Program017AuthorityError(
                    "Program 016 reviewed frontier is not page-two intent-only"
                ) from None
            incoming_token = None
            if completed_pages:
                ...
                if (
                    ...
                    raise Program017AuthorityError("Program 016 frontier status differs") from None
            intent = program_011.PageIntent(
            ...
            if session_entries != expected_entries:
                raise Program017AuthorityError("Program 016 frontier checkpoint differs") from None
            frontier = {
                "session": request.session.isoformat(),
                ...
                "url": intent.url,
                "intent_sha256": hashlib.sha256(
                    predecessor._read(program_016_root, f"{prefix}.intent.json")
                ).hexdigest(),
                "body_present": False,
        ...
        except (program_016._TransportRequired, program_016.Program016AuthorityError) as error:
            raise Program017AuthorityError(
                "Program 016 evidence is not the reviewed intent-only frontier"
            ) from error
        except program_011.Program011Error as error:
            raise Program017AuthorityError(
                "Program 016 completed prefix does not reparse"
            ) from error
        else:
            expected_entries = predecessor._expected_session_entries(request, len(result.pages))
            if session_entries != expected_entries:
                raise Program017AuthorityError("Program 016 completed session evidence differs")
            pages = []
                ...
                pages.append(
                    {
                        ...
                        "body_sha256": hashlib.sha256(
                            predecessor._read(program_016_root, f"{prefix}.body")
                        ).hexdigest(),
                        "receipt_sha256": hashlib.sha256(
                            predecessor._read(program_016_root, f"{prefix}.receipt.json")
                        ).hexdigest(),
            ...
            if budget.requests - before_requests != len(result.pages):
                raise Program017AuthorityError("Program 016 completed request count differs")
    if (
        frontier is None
        ...
        raise Program017AuthorityError("Program 016 reviewed recovery frontier is absent")
    if (
        ...
        or budget.requests - budget.responses != 5
    ):
        raise Program017AuthorityError("Program 016 cumulative terminal budget differs")
    runtime_016 = _mapping(authority.get("runtime_binding"), "runtime binding")
    unsigned_manifest: dict[str, Any] = {
        ...
        "runtime_binding": dict(runtime_016),
        ...
        "program_016_predecessor_manifest_sha256": program_015_state.sha256,
        "program_012_completed_sessions": program_015_state.manifest[
            "program_012_completed_sessions"
        ],
        "program_012_frontier": program_015_state.manifest["program_012_frontier"],
        "program_013_public_terminal": program_015_state.manifest["program_013_public_terminal"],
        "program_013_active_authority_fingerprint": program_015_state.manifest[
            "program_013_active_authority_fingerprint"
        ],
        "program_013_child_identity_fingerprint": program_015_state.manifest[
            "program_013_child_identity_fingerprint"
        ],
        "program_013_source_commit": program_015_state.manifest["program_013_source_commit"],
        "program_013_private_terminal_fingerprint": program_015_state.manifest[
            "program_013_private_terminal_fingerprint"
        ],
        "program_013_completed_sessions": program_015_state.manifest[
            "program_013_completed_sessions"
        ],
        "program_014_public_terminal": program_015_state.manifest["program_014_public_terminal"],
        "program_014_active_authority_fingerprint": program_015_state.manifest[
            "program_014_active_authority_fingerprint"
        ],
        "program_014_child_identity_fingerprint": program_015_state.manifest[
            "program_014_child_identity_fingerprint"
        ],
        "program_014_source_commit": program_015_state.manifest["program_014_source_commit"],
        "program_014_private_terminal_fingerprint": program_015_state.manifest[
            "program_014_private_terminal_fingerprint"
        ],
        "program_014_completed_sessions": program_015_state.manifest[
            "program_014_completed_sessions"
        ],
        "program_015_public_terminal": program_015_state.manifest["program_015_public_terminal"],
        "program_015_active_authority_fingerprint": program_015_state.manifest[
            "program_015_active_authority_fingerprint"
        ],
        "program_015_child_identity_fingerprint": program_015_state.manifest[
            "program_015_child_identity_fingerprint"
        ],
        "program_015_source_commit": program_015_state.manifest["program_015_source_commit"],
        "program_015_private_terminal_fingerprint": program_015_state.manifest[
            "program_015_private_terminal_fingerprint"
        ],
        "program_015_completed_sessions": program_015_state.manifest[
            "program_015_completed_sessions"
        ],
        "program_016_public_terminal": dict(terminal_binding),
        "program_016_active_authority_fingerprint": active_fingerprint,
        "program_016_child_identity_fingerprint": active["child_identity_fingerprint"],
        "program_016_source_commit": source_commit,
        "program_016_private_terminal_fingerprint": private_terminal["terminal_fingerprint"],
        "program_016_completed_sessions": completed_sessions,
        ...
        "consumed_intent_frontiers": [
            *program_015_state.manifest["consumed_intent_frontiers"],
            frontier,
        ],
        "discarded_program_016_partial_session": frontier,
        "frontier": {
...
def _validate_predecessor_manifest(root_descriptor: int, expected: _PredecessorState) -> None:
    if predecessor._read(root_descriptor, "predecessor-import-manifest.json") != expected.payload:
        raise Program017AuthorityError("Program 017 predecessor import manifest differs")
...
def _append_atomic_or_validate(root_descriptor: int, key: str, payload: bytes) -> None:
    ...
    try:
        ...
        raise Program017AuthorityError(str(error).replace("Program 012", "Program 017")) from None
...
def _load_active(root_descriptor: int, expected: Mapping[str, Any]) -> Mapping[str, Any]:
    ...
    if value != expected or raw != (canonical_json(expected) + "\n").encode():
        raise Program017AuthorityError("Program 017 active authority differs")
    return value
...
def _repository_preflight(repository: Path, identity: Mapping[str, Any]) -> dict[str, str]:
    ...
    try:
        ...
        raise Program017AuthorityError("Program 017 repository identity is unavailable") from error
    if (
        ...
        raise Program017AuthorityError("Program 017 reviewed synchronized-main lineage differs")
    return {
...
def _authority_commit(authority: Mapping[str, Any]) -> str:
    ...
    if _HEX_40.fullmatch(value) is None:
        raise Program017AuthorityError("Program 017 authority policy commit differs")
    return value
...
def _source_commit(authority: Mapping[str, Any]) -> str:
    ...
    if _HEX_40.fullmatch(value) is None:
        raise Program017AuthorityError("Program 017 runtime source commit differs")
    return value
...
def _require_credentials_present(environ: Mapping[str, str] | None) -> None:
    ...
    if missing:
        raise Program017AuthorityError("Program 017 credentials missing: " + ", ".join(missing))
    ...
    *,
    ...
    try:
        ...
        if not fingerprint_optional:
            raise Program017AuthorityError(
                str(error).replace("Program 011", "Program 017")
            ) from None
        path = Path(str(binding.get("path")))
        raw = _git(repository, "show", f"{commit or 'HEAD'}:{path.as_posix()}")
        if hashlib.sha256(raw).hexdigest() != binding.get("sha256"):
            raise Program017AuthorityError("Program 017 bound artifact differs") from None
    ...
    try:
        for part in root.parts:
            ...
            try:
                ...
                raise Program017AuthorityError(
                    "Program 017 private evidence root is absent"
                ) from error
            os.close(descriptor)
            descriptor = child
        metadata = os.fstat(descriptor)
        if not stat.S_ISDIR(metadata.st_mode) or stat.S_IMODE(metadata.st_mode) & 0o077:
            raise Program017AuthorityError("Program 017 private evidence root is not private")
        return descriptor
...
def _repository(repository: Path) -> Path:
    if not isinstance(repository, Path):
        raise Program017AuthorityError("Program 017 repository root is invalid")
    resolved = repository.resolve()
    if not resolved.is_dir():
        raise Program017AuthorityError("Program 017 repository root is absent")
    return resolved


def _mapping(value: Any, label: str) -> Mapping[str, Any]:
    if not isinstance(value, Mapping):
        raise Program017AuthorityError(f"Program 017 {label} is invalid")
    return value
...
def _json_object(raw: bytes, label: str) -> dict[str, Any]:
    try:
        ...
        raise Program017AuthorityError(f"Program 017 {label} is invalid JSON") from error
    if type(value) is not dict:
        raise Program017AuthorityError(f"Program 017 {label} is not an object")
    return value
...
def _git(repository: Path, *arguments: str) -> bytes:
    try:
        ...
        raise Program017AuthorityError("Program 017 Git identity is unavailable") from error
