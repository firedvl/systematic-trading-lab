"""Single-process Program 016 recovery of the Program 012-015 exposed prefix."""
...
PROGRAM_ID = "multi-hour-sector-etf-research-015"
PROGRAM_ORDINAL = 16
CHILD_AUTHORITY_ID = (
    "program-016-exposed-prefix-raw-alpaca-sip-recovery-and-"
    "structural-admission-child-2026-09-06-v1"
)
CONSUMPTION_BOUNDARY = "immediately before first provider transport invocation"
PRIVATE_ROOT = Path(".trading-lab/program-016-exposed-prefix-raw-alpaca-sip-v1")
...
PREDECESSOR_PRIVATE_ROOT = PROGRAM_015_PRIVATE_ROOT
PUBLIC_TERMINAL_PATH = Path(
    "config/research/program-016-exposed-prefix-raw-alpaca-sip-recovery-and-"
...
OPERATION_MANIFEST = {
    "path": (
        "config/research/program-016-exposed-prefix-raw-alpaca-sip-recovery-and-"
        "structural-admission-proposal-v1.json"
    ),
    "sha256": "76f946df1a50f1e1a866f144c7fe5cf4d2f12b1c5cf874dc9d901f994ff59776",
    "fingerprint": "48a2ce7333f9422609d3bf9c00e4f7fbc3a58414768585063b6f8db68abacafe",
}
PROPOSAL_REVIEW = {
    "path": (
        "config/research/program-016-exposed-prefix-raw-alpaca-sip-recovery-and-"
    ...
    "sha256": "fab92d4355453a9971df60b2609af39ab8ff2bece9ea2bfa9c32ec235343ea62",
    "fingerprint": "c5f9626858ce72639becc0757351ae1451ec73fbfd5d710a2ff8621a2fac664d",
}
...
_DERIVED_KEYS = {
    ...
    "program-016-response-manifest.json",
}
...
_PRIVATE_EVIDENCE_KEYS = {
    "program_016_response_manifest_sha256",
...
class Program016AuthorityError(ValueError):
    """Fail-closed Program 016 runtime or authority error."""


class Program016PostClaimPersistenceError(Program016AuthorityError):
    """Terminal evidence could not be persisted after possible transport."""


class CombinedRequestBudgetExhausted(Program016AuthorityError):
...
class _IncompletePageCheckpoint(Program016AuthorityError):
    pass
...
class _CombinedProjection:
    ...
    program_016_response_manifest: Mapping[str, Any]
    budget: _Budget
...
class _Budget:
    def __init__(self, predecessor_state: _PredecessorState) -> None:
        ...
        if completed_requests != completed_responses or consumed_intents != 4:
            raise Program016AuthorityError("Program 016 predecessor budget differs")
        ...
        if (
            ...
            raise Program016AuthorityError("Program 016 inherited budget exceeds its ceiling")
    ...
    def reserve_request(self) -> None:
        if self.requests >= science.MAXIMUM_REQUESTS_AND_RESPONSES:
            raise CombinedRequestBudgetExhausted(
                "Program 016 combined request ceiling is exhausted"
            )
        self.requests += 1
    ...
    def accept_response(
        ...
        if (
            ...
            raise Program016AuthorityError("Program 016 response timestamp must be UTC")
        if self.responses >= self.effective_response_ceiling:
            raise CombinedRequestBudgetExhausted(
                "Program 016 combined response ceiling is exhausted"
            )
        self.responses += 1
        ...
        if len(body) > science.MAXIMUM_RESPONSE_PAGE_BYTES:
            raise Program016AuthorityError("Program 016 response exceeds the 8 MiB page ceiling")
        if self.session_bytes[request.session] > science.MAXIMUM_SESSION_RESPONSE_BYTES:
            raise Program016AuthorityError("Program 016 session exceeds the 8 MiB byte ceiling")
        if self.response_bytes > science.MAXIMUM_TOTAL_RESPONSE_BYTES:
            raise Program016AuthorityError("Program 016 total response byte ceiling exceeded")
...
class _ReadLockedRoot:
    ...
    def __enter__(self) -> None:
        try:
            ...
            raise Program016AuthorityError(f"{self._program} read-only lock is absent") from error
        self._handle = os.fdopen(descriptor, "rb", buffering=0)
        metadata = os.fstat(self._handle.fileno())
        if not stat.S_ISREG(metadata.st_mode) or stat.S_IMODE(metadata.st_mode) & 0o077:
            self._handle.close()
            raise Program016AuthorityError(f"{self._program} read-only lock is invalid")
...
def credential_presence_preflight(
    ...
    with _locked_controls(repository, authority, create_program_016=True) as (
        program_016_root,
        program_015_root,
        program_014_root,
        program_013_root,
        ...
        if locked != authority:
            raise Program016AuthorityError("Program 016 authority changed under policy lock")
...
def validate_operation_contract(
    ...
    try:
        program_015.validate_operation_contract(repository, commit=commit)
    except program_015.Program015AuthorityError as error:
        raise Program016AuthorityError(str(error).replace("Program 015", "Program 016")) from None
    predecessor_bindings = _mapping(proposal.get("predecessor"), "predecessor bindings")
    inheritance = _mapping(proposal.get("inheritance_contract"), "inheritance contract")
    recovery = _mapping(proposal.get("recovery_contract"), "recovery contract")
    restart = _mapping(proposal.get("restart_and_launch_contract"), "restart and launch contract")
    ...
    program_015_terminal = _mapping(
        predecessor_bindings.get("program_015_terminal_result"),
    ...
    if (
        ...
        or review.get("reviewed_source_commit") != "699e175f269763e974bdbd9f5d319b2e094bcb7c"
        or review.get("reviewed_proposal") != OPERATION_MANIFEST
        or predecessor_bindings.get("program_015_scientific_contract")
        != program_015.OPERATION_MANIFEST
        or predecessor_bindings.get("program_015_scientific_contract_review")
        != program_015.PROPOSAL_REVIEW
        or program_015_terminal.get("path") != program_015.PUBLIC_TERMINAL_PATH.as_posix()
        or inheritance.get("base_contract") != "program_015_scientific_contract"
        or inheritance.get("program_012_history_mutation_allowed") is not False
        or inheritance.get("program_013_history_mutation_allowed") is not False
        or inheritance.get("program_014_history_mutation_allowed") is not False
        or inheritance.get("program_015_history_mutation_allowed") is not False
        or inheritance.get("observed_result_gate_changes_allowed") is not False
        or inheritance.get("source_or_chronology_changes_allowed") is not False
        or inheritance.get("missingness_or_admission_changes_allowed") is not False
        or recovery.get("program_012_private_root") != PROGRAM_012_PRIVATE_ROOT.as_posix()
        or recovery.get("program_013_private_root") != PROGRAM_013_PRIVATE_ROOT.as_posix()
        or recovery.get("program_014_private_root") != PROGRAM_014_PRIVATE_ROOT.as_posix()
        or recovery.get("program_015_private_root") != PROGRAM_015_PRIVATE_ROOT.as_posix()
        or recovery.get("program_016_private_root") != PRIVATE_ROOT.as_posix()
        or recovery.get("program_012_private_evidence_access") != "READ-ONLY"
        or recovery.get("program_013_private_evidence_access") != "READ-ONLY"
        or recovery.get("program_014_private_evidence_access") != "READ-ONLY"
        or recovery.get("program_015_private_evidence_access") != "READ-ONLY"
        or recovery.get("program_016_private_root_must_be_distinct") is not True
        or recovery.get("program_016_private_root_override_allowed") is not False
        or recovery.get("accepted_predecessor_shape")
        != "EXACT-COMPLETED-WHOLE-SESSION-PREFIX-PLUS-ONE-INTENT-ONLY-PAGE-FRONTIER"
        or recovery.get("first_program_016_intent_must_equal_rederived_frontier_identity")
        is not True
        or recovery.get("frontier_request_under_program_016_max") != 1
        or recovery.get("program_016_request_reissue_allowed") is not False
        or recovery.get("program_016_automatic_retries") != 0
        or any(
            recovery.get(key) is not True
            for key in (
                "predecessor_shape_must_be_rederived_before_credential_presence",
                "predecessor_shape_must_be_rederived_before_activation",
                "predecessor_shape_must_be_rederived_before_every_transport",
                "program_012_program_013_program_014_and_program_015_public_and_private_terminal_binding_required",
                "completed_predecessor_pages_are_reparsed_locally",
                "completed_predecessor_pages_are_never_requested_again",
                "frontier_intent_must_be_rederived_exactly",
                "frontier_body_or_receipt_or_status_must_be_absent",
                "frontier_parsed_missingness_or_data_quality_outcome_must_be_absent",
            )
        )
        or recovery.get("incomplete_program_015_session_or_page_reuse_allowed") is not False
        or restart.get("program_016_lock_path") != f"{PRIVATE_ROOT.as_posix()}/run.lock"
        or restart.get("program_015_lock_path") != f"{PROGRAM_015_PRIVATE_ROOT.as_posix()}/run.lock"
        or restart.get("program_014_lock_path") != f"{PROGRAM_014_PRIVATE_ROOT.as_posix()}/run.lock"
        or restart.get("program_013_lock_path") != f"{PROGRAM_013_PRIVATE_ROOT.as_posix()}/run.lock"
        or restart.get("program_012_lock_path") != f"{PROGRAM_012_PRIVATE_ROOT.as_posix()}/run.lock"
        ...
        or restart.get("all_controls_held_through_private_and_public_terminal_fsync") is not True
        or restart.get("single_nonrestarting_process_required") is not True
        or restart.get("launcher_identity_and_pid_recorded_privately_before_activation") is not True
        or restart.get("launcher_contract_revalidated_before_credential_access") is not True
        or any(
            restart.get(key) is not False
            for key in (
                "automatic_restart",
                "automatic_relaunch",
        ...
        or topology.get("runtime_finalization_precedes_future_child_source_commit") is not True
        or topology.get(
            "secret_guard_reserves_both_future_child_paths_before_runtime_source_is_frozen"
        )
        is not True
        ...
        != science.MAXIMUM_REQUESTS_AND_RESPONSES - 4
        or budgets.get("consumed_intent_without_response_count") != 4
        or budgets.get("maximum_response_page_bytes") != science.MAXIMUM_RESPONSE_PAGE_BYTES
        or budgets.get("maximum_combined_session_response_bytes")
        ...
        or budgets.get("request_timeout_seconds") != 30
        or budgets.get("credential_loads_per_program_016_process_max") != 1
        or budgets.get("parallel_session_chains") != 1
        or budgets.get("automatic_retries") != 0
        or credentials.get("credential_names_inherited_from_program_006") is not True
        or credentials.get("presence_only_preflight_before_value_access") is not True
        or credentials.get("standalone_callable_credential_readers_allowed") is not False
        or credentials.get("prohibited_reader_scopes") != ["module", "class", "static", "instance"]
        or credentials.get("credential_parsing_location")
        != "INLINE-ONLY-IN-LOCK-BOUND-LOADER-OPERATION"
        or credentials.get("credential_values_loaded_only_inside_all_locks") is not True
        or credentials.get("launcher_revalidated_immediately_before_credential_attempt") is not True
        or credentials.get("credential_attempt_create_only_fsynced_before_environment_access")
        is not True
        or credentials.get("credential_attempt_binds_authority_source_and_predecessor_identity")
        is not True
        or credentials.get("process_global_credential_latch_consumed_before_environment_access")
        is not True
        or credentials.get("process_global_credential_latch_consumed_if_attempt_reservation_fails")
        is not True
        or credentials.get("process_global_credential_latch_consumed_if_parsing_fails") is not True
        or credentials.get("credential_failure_receipt_fsynced_before_error_propagation")
        is not True
        or credentials.get("credential_success_receipt_fsynced_before_client_construction")
        is not True
        or credentials.get("credential_success_receipt_fsynced_before_transport") is not True
        ...
        is not True
        or credentials.get(
            "credential_values_never_logged_hashed_persisted_or_passed_as_cli_arguments"
        )
        is not True
        or private_terminal.get("path") != f"{PRIVATE_ROOT.as_posix()}/terminal.json"
        or private_terminal.get("schema_version") != "program-016-private-terminal-v1"
        ...
        != _PRIVATE_EVIDENCE_KEYS
        or static_private.get("schema_version") != "program-016-private-terminal-v1"
        or static_private.get("program_id") != PROGRAM_ID
        or static_private.get("public_terminal_path") != PUBLIC_TERMINAL_PATH.as_posix()
        or type(static_private.get("strategy_calculations")) is not int
        or static_private.get("strategy_calculations") != 0
        or type(static_private.get("strategy_returns")) is not int
        or static_private.get("strategy_returns") != 0
        or any(
            _mapping(integer_zero.get(field), f"{field} integer-zero contract").get(
                "python_type_predicate"
            )
            != "type(value) is int"
            or type(
                _mapping(integer_zero.get(field), f"{field} integer-zero contract").get(
            ...
            is not int
            or _mapping(integer_zero.get(field), f"{field} integer-zero contract").get(
                "required_value"
            )
            != 0
            for field in ("strategy_calculations", "strategy_returns")
        )
        or public_terminal.get("path") != PUBLIC_TERMINAL_PATH.as_posix()
        or public_terminal.get("schema_version") != "program-016-exposed-prefix-terminal-result-v1"
        or set(_sequence(public_terminal.get("exact_top_level_keys"), "public terminal keys"))
        != _PUBLIC_TERMINAL_KEYS
        or static_public
        != {
            "schema_version": "program-016-exposed-prefix-terminal-result-v1",
        ...
        or any(
            ...
            for key in (
                "strategy_capabilities_allowed",
                "research_qualification_allowed",
                "controlled_or_protected_allowed",
                "paper_broker_or_live_allowed",
                "purchase_allowed",
            )
        )
        or set(authority) != standing.AUTHORITY_FIELDS
        ...
        or firewall.get("inherit_program_015_registered_range_derivation") is not True
        or any(
            firewall.get(key) is not False
            for key in ("controlled_evaluation", "protected_holdout", "paper", "broker", "live")
        )
    ):
        raise Program016AuthorityError("Program 016 operation contract differs")
    return proposal
...
def _derive_control_validated_authority(repository: Path) -> Mapping[str, Any]:
    ...
    try:
        ...
        raise Program016AuthorityError(str(error)) from None
    ...
    if (
        ...
        raise Program016AuthorityError("Program 016 reviewed child identity differs")
    ...
    try:
        ...
        raise Program016AuthorityError(str(error).replace("Program 011", "Program 016")) from None
    ...
    if _repository_preflight(repository, identity) != lineage:
        raise Program016AuthorityError("Program 016 repository changed during validation")
...
def _locked_controls(
    ...
    *,
    create_program_016: bool,
) -> Iterator[tuple[int, int, int, int, int]]:
    program_016_root = _open_root(repository, PRIVATE_ROOT, create=create_program_016)
    try:
        with git_controls._LockedRoot(program_016_root):
            program_015_root = _open_root(repository, PROGRAM_015_PRIVATE_ROOT, create=False)
            try:
                            ...
                            try:
                                with _ReadLockedRoot(program_013_root, "Program 013"):
                                    program_012_root = _open_root(
                                        repository, PROGRAM_012_PRIVATE_ROOT, create=False
                                    )
                                    try:
                                        with (
                                            _ReadLockedRoot(program_012_root, "Program 012"),
...
def _activate_locked_authority(
    ...
    if entries not in (
        ...
        raise Program016AuthorityError("Program 016 one-use authority state already exists")
...
def _execute_mock_acquisition(
    ...
    *,
    ...
    if type(transport) is not MockBarsTransport or environ is os.environ:
        raise Program016AuthorityError("Program 016 test execution requires finite inputs")
    return _execute_acquisition(
        repository,
        environ=environ,
...
def _execute_acquisition(
    ...
    *,
    ...
    with _locked_controls(repository, expected_authority, create_program_016=True) as (
        program_016_root,
        program_015_root,
        program_014_root,
        program_013_root,
        ...
        try:
            if _derive_control_validated_authority(repository) != authority:
                raise Program016AuthorityError("Program 016 authority changed under lock")
            _recover_private_terminal_if_present(
                repository,
                program_016_root,
                program_015_root,
            ...
            if after_activation is not None:
                after_activation()
            _validate_launcher(program_016_root, authority, predecessor_state)
            _load_active(program_016_root, authority)
            budget = _reconstruct_state(
                program_016_root,
                predecessor_state,
                authority=authority,
            )
            _require_working_disk_capacity(program_016_root)
            ...
            def consume() -> None:
                ...
                if predecessor._exists(program_016_root, "claim.json"):
                    _validate_claim(program_016_root, authority, predecessor_state)
                    return
                predecessor._append_atomic(
                    program_016_root,
                    "claim.json",
            ...
            temp_key, temp_descriptor = predecessor._new_temp(
                program_016_root, "combined-canonical-raw"
            )
            completed = False
            try:
                ...
                if not completed:
                    with suppress(FileNotFoundError):
                        os.unlink(temp_key, dir_fd=program_016_root)
                        os.fsync(program_016_root)
            ...
            if budget.new_requests != budget.new_responses:
                raise Program016AuthorityError("Program 016 new request and response counts differ")
            ...
            for key, payload in cast(Mapping[str, bytes], evidence["payloads"]).items():
                predecessor._append_or_validate(program_016_root, key, payload)
            ...
            predecessor._append_atomic(program_016_root, _TERMINAL_KEY, terminal_payload)
            ...
            try:
                ...
                raise Program016PostClaimPersistenceError(
                    "Program 016 public terminal persistence failed after possible transport"
            ...
            if (
                ...
                or _has_consumed_state(program_016_root)
            ) and not predecessor._exists(program_016_root, _TERMINAL_KEY):
                try:
                    ...
                    raise Program016PostClaimPersistenceError(
                        "Program 016 terminal persistence failed after possible transport"
                    ) from persistence_error
            if published_failure and isinstance(error, Exception):
                raise Program016AuthorityError(
                    "Program 016 acquisition ended with a sealed failure"
                ) from None
            raise
        ...
        if not self._loaded:
            ...
            try:
                ...
                if any(not value or "\r" in value or "\n" in value for value in credentials):
                    raise Program016AuthorityError("Program 016 OHLCV credentials are required")
...
class _PersistentSessionSource:
    ...
    def response(self, intent: program_011.PageIntent) -> raw_contract.RawResponse:
        if self._pending is not None:
            raise Program016AuthorityError("Program 016 persistent source state differs")
        ...
        if any(present):
            if present[0]:
                ...
                if predecessor._read(self._root_descriptor, f"{prefix}.intent.json") != expected:
                    raise Program016AuthorityError(
                        "Program 016 partial request intent checkpoint differs"
                    )
            raise Program016AuthorityError(
                "Program 016 ambiguous page checkpoint forbids request reissue"
            )
    ...
    def retain(self, page_index: int, body: bytes) -> program_011.RetainedPage:
        ...
        if (
            ...
            raise Program016AuthorityError("Program 016 retained response differs")
        self._pending = None
        return pending
...
class _ReplaySource:
    ...
    def response(self, intent: program_011.PageIntent) -> raw_contract.RawResponse:
        ...
        if not all(present):
            if (
                ...
                raise Program016AuthorityError("Program 016 partial page checkpoint differs")
            self._budget.reserve_request()
    ...
    def retain(self, page_index: int, body: bytes) -> program_011.RetainedPage:
        ...
        if pending is None or pending.page_index != page_index:
            raise Program016AuthorityError("Program 016 replayed response differs")
        self._pending = None
        return pending
...
class _ExistingSessionSource:
    ...
    def retain(self, page_index: int, body: bytes) -> program_011.RetainedPage:
        ...
        if (
            ...
            raise Program016AuthorityError("Program 016 existing response differs")
        self._pending = None
        return pending
...
def _combined_projection(
    ...
    *,
    ...
    program_012_state = predecessor_state.program_014_state.program_013_state.program_012_state
    program_013_state = predecessor_state.program_014_state.program_013_state
    program_014_state = predecessor_state.program_014_state
    ...
    for request_index, request in enumerate(science.acquisition_requests()):
        if request_index < program_012_frontier:
        ...
        elif request_index < predecessor_state.frontier_request_index:
            ...
            source = _ExistingSessionSource(program_016_root, request, authority, predecessor_state)
        else:
        ...
        for row in result.rows:
            ...
            if canonical_file is not None and canonical_file.write(line) != len(line):
                raise Program016AuthorityError("Program 016 canonical raw write was incomplete")
    ...
    if (
        ...
        raise Program016AuthorityError("Program 016 predecessor replay budget differs")
    response_manifest = {
        "schema_version": "program-016-private-response-manifest-v1",
        "program_id": PROGRAM_ID,
    ...
    return _CombinedProjection(
        ...
        missing=missing,
        morning=morning,
        program_016_response_manifest=response_manifest,
        budget=budget,
    )
...
def _execute_session(
    ...
    try:
        ...
        replacement = Program016AuthorityError(str(error).replace("Program 012", "Program 016"))
        ...
        raise Program016AuthorityError(str(error).replace("Program 012", "Program 016")) from error
...
def _reconstruct_state(
    ...
    *,
    ...
    if entries & _DERIVED_KEYS and _TERMINAL_KEY not in entries:
        raise Program016AuthorityError("Program 016 derived evidence lacks a terminal")
    allowed = {
        "active-authority.json",
        "claim.json",
        _LAUNCHER_KEY,
    ...
    for entry in entries - allowed:
        if _PAGE_KEY.fullmatch(entry):
            ...
            raise Program016AuthorityError("Program 016 private checkpoint contains unknown state")
    ...
    if page_entries:
        if claim_exists:
            if successful_loads == 0:
                raise Program016AuthorityError(
                    "Program 016 page evidence lacks a successful credential audit"
                )
            ...
            raise Program016AuthorityError("Program 016 provider evidence lacks a transport claim")
    elif claim_exists:
        raise Program016AuthorityError("Program 016 transport claim lacks page evidence")
    if credential_loads and not page_entries:
        raise Program016AuthorityError("Program 016 credential audit lacks request evidence")
    ...
    for entry in page_entries:
        ...
        try:
            ...
            raise Program016AuthorityError("Program 016 page key is invalid") from error
        if session not in expected_sessions or not 1 <= page_index <= 16:
            raise Program016AuthorityError("Program 016 page key is outside the recovery plan")
    ...
    for request in requests[predecessor_state.frontier_request_index :]:
        ...
        if frontier_found:
            if session_entries:
                raise Program016AuthorityError("Program 016 checkpoint skips a request session")
            continue
        ...
        try:
            ...
            if frozenset(session_entries) not in {
                ...
                raise Program016AuthorityError(
                    "Program 016 terminal checkpoint is not a reachable prefix"
                ) from None
            frontier_found = True
            ...
            if session_entries != predecessor._expected_session_entries(request, completed_pages):
                raise Program016AuthorityError(
                    "Program 016 checkpoint skips a request page"
                ) from None
            if require_complete:
                raise Program016AuthorityError(
                    "Program 016 acquisition evidence is incomplete"
                ) from None
            frontier_found = True
        except (Program016AuthorityError, program_011.Program011Error):
            if not allow_terminal_failure:
                raise
            completed_pages = sum(
                page["session"] == request.session.isoformat() for page in budget.pages
            ...
            if session_entries != predecessor._expected_session_entries(request, len(result.pages)):
                raise Program016AuthorityError(
                    "Program 016 checkpoint continues after a terminal page"
                )
    if require_complete and frontier_found:
        raise Program016AuthorityError("Program 016 acquisition evidence is incomplete")
    return budget
...
def _runtime_failure_credential_load_count(
    ...
    except Program016AuthorityError:
        return len(
            {
...
def _claim_value(
    ...
    unsigned: dict[str, Any] = {
        "schema_version": "program-016-private-acquisition-claim-v1",
        "program_id": PROGRAM_ID,
        "authority_id": authority["authority_id"],
        "authority_fingerprint": authority["authority_fingerprint"],
        "operation_manifest": OPERATION_MANIFEST,
...
def _validate_claim(
    ...
    if predecessor._read(root_descriptor, "claim.json") != _claim_payload(
        ...
        raise Program016AuthorityError("Program 016 transport claim differs")
...
def _intent_value(
    ...
    unsigned: dict[str, Any] = {
        "schema_version": "program-016-private-request-intent-v1",
        "program_id": PROGRAM_ID,
        "authority_fingerprint": authority["authority_fingerprint"],
        "operation_manifest": OPERATION_MANIFEST,
        "runtime_binding": dict(_runtime_binding(authority)),
...
def _response_receipt(
    ...
    if type(response.status) is not int or not 100 <= response.status <= 599:
        raise Program016AuthorityError("Program 016 response status is invalid")
    ...
    claim = _claim_value(authority, predecessor_state)
    unsigned: dict[str, Any] = {
        "schema_version": "program-016-private-response-receipt-v1",
        "program_id": PROGRAM_ID,
        "authority_fingerprint": authority["authority_fingerprint"],
        "operation_manifest": OPERATION_MANIFEST,
        "runtime_binding": dict(_runtime_binding(authority)),
...
def _load_completed_page(
    ...
    if predecessor._read(root_descriptor, f"{prefix}.intent.json") != expected_intent:
        raise Program016AuthorityError("Program 016 request intent checkpoint differs")
    ...
    if (
        ...
        raise Program016AuthorityError("Program 016 response checkpoint differs")
...
def _validate_page_intent(
    ...
    if (
        ...
        raise Program016AuthorityError("Program 016 request intent differs")
    ...
    if (
        ...
        raise Program016AuthorityError("Program 016 first intent differs from the frontier")
...
def _credential_attempt(
    ...
    if type(sequence) is not int or sequence < 1:
        raise Program016AuthorityError("Program 016 credential sequence differs")
    unsigned: dict[str, Any] = {
        "schema_version": "program-016-private-credential-load-attempt-v1",
        "authority_fingerprint": authority["authority_fingerprint"],
        "runtime_binding": dict(_runtime_binding(authority)),
...
def _credential_receipt_payload(attempt_identity: str, status: str) -> bytes:
    if _HEX_64.fullmatch(attempt_identity) is None or status not in {"SUCCESS", "FAILURE"}:
        raise Program016AuthorityError("Program 016 credential receipt differs")
    value = {
        "schema_version": "program-016-private-credential-load-receipt-v1",
...
def _launcher_value(
    ...
    unsigned = {
        "schema_version": "program-016-private-single-process-launcher-v1",
        "authority_fingerprint": authority["authority_fingerprint"],
        "predecessor_import_manifest_sha256": predecessor_state.sha256,
...
def _validate_launcher(
    ...
    if (
        ...
        raise Program016AuthorityError("Program 016 launcher changed or the process was restarted")
...
def _credential_load_counts(
    ...
    for entry in os.listdir(root_descriptor):
        ...
        if raw != (canonical_json(value) + "\n").encode():
            raise Program016AuthorityError("Program 016 credential audit is not canonical")
        target = attempts if match.group("kind") == "attempt" else receipts
        if sequence in target:
            raise Program016AuthorityError("Program 016 credential sequence is duplicated")
        target[sequence] = value
    sequences = sorted(attempts)
    if sequences != list(range(1, len(sequences) + 1)) or not set(receipts) <= set(attempts):
        raise Program016AuthorityError("Program 016 credential sequence is not contiguous")
    ...
    for sequence in sequences:
        ...
        if set(attempt) != _CREDENTIAL_ATTEMPT_KEYS or attempt != expected_attempt:
            raise Program016AuthorityError("Program 016 credential attempt binding differs")
        receipt = receipts.get(sequence)
        if receipt is None:
            if sequence != sequences[-1]:
                raise Program016AuthorityError("Program 016 credential receipt sequence has a gap")
            continue
        if (
            set(receipt) != _CREDENTIAL_RECEIPT_KEYS
            or receipt.get("schema_version") != "program-016-private-credential-load-receipt-v1"
            ...
            raise Program016AuthorityError("Program 016 credential receipt binding differs")
        if receipt["status"] == "SUCCESS":
            successful += 1
        elif sequence != sequences[-1]:
            raise Program016AuthorityError("Program 016 failed credential load is not terminal")
...
def _reserve_process_credential_access(write_attempt: Callable[[], None]) -> None:
    ...
    with _PROCESS_CREDENTIAL_LOCK:
        if process_id == _PROCESS_CREDENTIAL_PID:
            raise Program016AuthorityError(
                "Program 016 credential access is already consumed in this process"
            )
        try:
            write_attempt()
        finally:
...
def _revalidate_transport_boundary(
    ...
    if _derive_control_validated_authority(repository) != authority:
        raise Program016AuthorityError("Program 016 authority changed at transport boundary")
    current_predecessor = _derive_predecessor_state(
        repository,
        program_015_root,
        program_014_root,
        program_013_root,
    ...
    if current_predecessor.payload != predecessor_state.payload:
        raise Program016AuthorityError(
            "Program 016 predecessor evidence changed at transport boundary"
        )
    _require_working_disk_capacity(program_016_root)
...
def _revalidate_closeout_boundary(
    ...
    if _derive_control_validated_authority(repository) != authority:
        raise Program016AuthorityError("Program 016 authority changed at closeout")
    current_predecessor = _derive_predecessor_state(
        repository,
        program_015_root,
        program_014_root,
    ...
    if current_predecessor.payload != predecessor_state.payload:
        raise Program016AuthorityError("Program 016 predecessor evidence changed at closeout")
    _require_zero_protected_overlap(repository, _authority_commit(authority))
...
def _require_zero_protected_overlap(repository: Path, commit: str) -> None:
    ...
    if any(
        ...
        raise Program016AuthorityError("Program 016 request chronology overlaps protected data")
...
def _require_working_disk_capacity(root_descriptor: int) -> None:
    ...
    if filesystem.f_bavail * filesystem.f_frsize < science.WORKING_DISK_RESERVATION_BYTES:
        raise Program016AuthorityError("Program 016 requires 8 GiB of available working space")
...
def _runtime_binding(authority: Mapping[str, Any]) -> Mapping[str, Any]:
    ...
    if (
        ...
        raise Program016AuthorityError("Program 016 runtime binding differs")
    return runtime


def _count(value: Any, label: str) -> int:
    if type(value) is not int or value < 0:
        raise Program016AuthorityError(f"Program 016 {label} differs")
    return value
...
def _derived_admission_evidence(
    ...
    program_016_proposal = validate_operation_contract(
    ...
    admission["schema_version"] = "program-016-private-structural-admission-report-v1"
    ...
    missing_value = {
        "schema_version": "program-016-private-combined-missing-coordinates-v1",
        "program_id": PROGRAM_ID,
        "predecessor_import_manifest_sha256": predecessor_state.sha256,
        "sessions": {
    ...
    response_payload = (canonical_json(projection.program_016_response_manifest) + "\n").encode()
    ...
    private_dataset_identity = fingerprint(
        {
            ...
            "predecessor_import_manifest_sha256": predecessor_state.sha256,
            "program_016_response_manifest_sha256": response_sha,
    ...
    public_lineage = (
        {
            "schema_version": ("program-016-public-raw-structural-prefix-lineage-manifest-v1"),
            "program_id": PROGRAM_ID,
            "authority_id": authority["authority_id"],
            "authority_fingerprint": authority["authority_fingerprint"],
            "source_commit": _source_commit(authority),
            "status": "ADMITTED-PROGRAM-016-RAW-STRUCTURAL-PREFIX",
    ...
    if public_lineage is not None and (
        ...
        raise Program016AuthorityError("Program 016 public and private identities collide")
    dataset_manifest = {
        "schema_version": "program-016-private-combined-dataset-manifest-v1",
        "program_id": PROGRAM_ID,
        "authority_id": authority["authority_id"],
        "authority_fingerprint": authority["authority_fingerprint"],
        "source_commit": _source_commit(authority),
    ...
    return {
        ...
        "payloads": {
            "program-016-response-manifest.json": response_payload,
...
def _admission_terminal_payload(
    ...
    if _HEX_64.fullmatch(private_identity) is None:
        raise Program016AuthorityError("Program 016 private dataset identity differs")
    lineage_identity = (
    ...
    record: dict[str, Any] = {
        "schema_version": "program-016-private-terminal-v1",
        "program_id": PROGRAM_ID,
        "authority_id": authority["authority_id"],
        "authority_fingerprint": authority["authority_fingerprint"],
        "source_commit": _source_commit(authority),
        ...
        "program_016_credential_loads": _credential_load_counts(
            root_descriptor, authority, predecessor_state
        )[0],
...
def _runtime_failure_payload(
    ...
    private_evidence = {
        "program_016_response_manifest_sha256": _evidence_sha256_if_present(
            root_descriptor, "program-016-response-manifest.json"
        ),
    ...
    record: dict[str, Any] = {
        ...
        "program_016_credential_loads": _runtime_failure_credential_load_count(
            root_descriptor, authority, predecessor_state
        ),
...
def _load_terminal_record(
    ...
    if (
        ...
        or record.get("schema_version") != "program-016-private-terminal-v1"
        ...
        raise Program016AuthorityError("Program 016 private terminal binding differs")
    ...
    if current_predecessor.payload != predecessor_state.payload:
        raise Program016AuthorityError("Program 015 evidence changed before terminal validation")
    ...
    if _count(
        record.get("program_016_credential_loads"), "terminal credential count"
    ) != durable_credentials or record.get(
        "provider_transport_attempted"
    ) is not predecessor._exists(root_descriptor, "claim.json"):
        raise Program016AuthorityError("Program 016 terminal credential count differs")
    ...
    if result_kind == "RUNTIME-FAILURE":
        ...
        if (
            ...
            raise Program016AuthorityError("Program 016 runtime terminal semantics differ")
        _validate_runtime_failure_counts(record, counts)
        for field, key in (
            ("program_016_response_manifest_sha256", "program-016-response-manifest.json"),
            ...
            if private_evidence[field] != _evidence_sha256_if_present(root_descriptor, key):
                raise Program016AuthorityError("Program 016 runtime evidence hash differs")
        ...
        for key, expected in payloads.items():
            if predecessor._read(root_descriptor, key) != expected:
                raise Program016AuthorityError("Program 016 derived evidence differs")
        if (
            _evidence_sha256(root_descriptor, "combined-canonical-raw.jsonl")
            != projection.canonical_sha256
        ):
            raise Program016AuthorityError("Program 016 combined canonical evidence differs")
        admission = _mapping(evidence.get("admission"), "structural admission")
        admission_passed = admission.get("admission_passed") is True
        ...
        if (
            ...
            raise Program016AuthorityError("Program 016 admission terminal semantics differ")
        _validate_budget_counts(record, budget)
    else:
        raise Program016AuthorityError("Program 016 private terminal result kind differs")
    return record
...
def _validate_budget_counts(record: Mapping[str, Any], budget: _Budget) -> None:
    if (
        ...
        raise Program016AuthorityError("Program 016 terminal budget differs")
...
def _publish_public_terminal(
    ...
    if validated != record:
        raise Program016AuthorityError("Program 016 terminal changed before publication")
    ...
    try:
        ...
        if existing_descriptor is not None:
            ...
            if not stat.S_ISREG(metadata.st_mode) or existing != payload:
                raise Program016AuthorityError("Program 016 public terminal artifact differs")
        ...
        try:
            with os.fdopen(temp_descriptor, "wb") as handle:
                if handle.write(payload) != len(payload):
                    raise Program016AuthorityError(
                        "Program 016 public terminal write was incomplete"
                    )
                handle.flush()
            ...
            try:
                ...
                raise Program016AuthorityError(
                    "Program 016 public terminal artifact already exists"
                ) from None
            os.fsync(public_descriptor)
        finally:
            with suppress(FileNotFoundError):
...
def _recover_private_terminal_if_present(
    ...
    if predecessor._exists(root_descriptor, _TERMINAL_KEY):
        ...
        raise Program016AuthorityError("Program 016 acquisition is terminally sealed")
    ...
    try:
        ...
        _load_active(root_descriptor, authority)
        raise Program016AuthorityError("Program 016 single-process transaction was interrupted")
        ...
        raise Program016AuthorityError("Program 016 acquisition is terminally sealed") from None
...
def _reject_terminal_state(repository: Path) -> None:
    ...
    try:
        ...
        if _PUBLIC_TERMINAL_SHA256 is None:
            return
        raise Program016AuthorityError("Program 016 terminal result artifact is absent") from error
    except OSError as error:
        raise Program016AuthorityError("Program 016 terminal result artifact is invalid") from error
    try:
        metadata = os.fstat(descriptor)
        if not stat.S_ISREG(metadata.st_mode) or metadata.st_size > 1_048_576:
            raise Program016AuthorityError("Program 016 terminal result artifact is invalid")
    ...
    if raw != (canonical_json(value) + "\n").encode():
        raise Program016AuthorityError("Program 016 public terminal is not canonical")
    _validate_public_terminal_shape(value)
    if _PUBLIC_TERMINAL_SHA256 is None:
        raise Program016AuthorityError("Program 016 terminal result lacks immutable binding")
    if hashlib.sha256(raw).hexdigest() != _PUBLIC_TERMINAL_SHA256:
        raise Program016AuthorityError("Program 016 terminal result semantics differ")
    raise Program016AuthorityError("Program 016 authority is terminally revoked")


def _validate_public_terminal_shape(value: Mapping[str, Any]) -> None:
    if (
        set(value) != _PUBLIC_TERMINAL_KEYS
        or value.get("schema_version") != "program-016-exposed-prefix-terminal-result-v1"
        ...
        raise Program016AuthorityError("Program 016 public terminal schema differs")
    ...
    if result_kind == "RUNTIME-FAILURE":
        ...
        expected_status = "ADMITTED-PROGRAM-016-RAW-STRUCTURAL-PREFIX"
        expected_admission = True
    else:
        raise Program016AuthorityError("Program 016 public terminal result kind differs")
    if (
        value.get("status") != expected_status
        or value.get("admission_passed") is not expected_admission
    ):
        raise Program016AuthorityError("Program 016 public terminal branch differs")
    lineage = value.get("dataset_lineage_manifest")
    if expected_admission:
        _validate_public_lineage(lineage, value)
    elif lineage is not None:
        raise Program016AuthorityError("Program 016 failure terminal publishes lineage")
...
def _validate_public_lineage(value: Any, terminal: Mapping[str, Any]) -> dict[str, Any]:
    ...
    if (
        ...
        != "program-016-public-raw-structural-prefix-lineage-manifest-v1"
        ...
        or lineage.get("status") != "ADMITTED-PROGRAM-016-RAW-STRUCTURAL-PREFIX"
        or _HEX_64.fullmatch(str(lineage.get("dataset_lineage_identity"))) is None
    ):
        raise Program016AuthorityError("Program 016 public dataset lineage differs")
    return lineage


def _exact_object(value: Any, keys: set[str], label: str) -> dict[str, Any]:
    if type(value) is not dict or set(value) != keys:
        raise Program016AuthorityError(f"Program 016 {label} schema differs")
...
def _evidence_sha256(root_descriptor: int, key: str) -> str:
    if _EVIDENCE_KEY.fullmatch(key) is None:
        raise Program016AuthorityError("Program 016 evidence key is invalid")
    try:
        descriptor = os.open(
            key,
            os.O_RDONLY | getattr(os, "O_CLOEXEC", 0) | getattr(os, "O_NOFOLLOW", 0),
        ...
        raise Program016AuthorityError(f"Program 016 evidence is absent: {key}") from error
    with os.fdopen(descriptor, "rb") as handle:
        metadata = os.fstat(handle.fileno())
        if not stat.S_ISREG(metadata.st_mode) or stat.S_IMODE(metadata.st_mode) & 0o077:
            raise Program016AuthorityError("Program 016 evidence is not private")
...
def _publish_temp_or_validate(
    ...
    if predecessor._exists(root_descriptor, final_key):
        if _evidence_sha256(root_descriptor, final_key) != expected_sha256:
            raise Program016AuthorityError("Program 016 canonical raw dataset differs")
    else:
        try:
            os.link(
                temp_key,
            ...
            raise Program016AuthorityError("Program 016 canonical raw publish raced") from None
        os.fsync(root_descriptor)
    os.unlink(temp_key, dir_fd=root_descriptor)
    os.fsync(root_descriptor)
...
def _historical_fingerprinted_artifact(
    ...
    if _HEX_64.fullmatch(str(stored)) is None or stored != fingerprint(unsigned):
        raise Program016AuthorityError(f"Program 016 historical {label} fingerprint differs")
    return value, {
        "path": path.as_posix(),
...
def _validate_historical_source_binding(repository: Path, runtime: Mapping[str, Any]) -> None:
    ...
    if (
        ...
        raise Program016AuthorityError("Program 014 historical runtime binding differs")
    seen: set[str] = set()
    for item in source_files:
        ...
        if not path or path in seen or _HEX_64.fullmatch(str(binding.get("sha256"))) is None:
            raise Program016AuthorityError("Program 014 historical runtime source differs")
        seen.add(path)
...
def _historical_program_015_identity(
    repository: Path,
    proposal: Mapping[str, Any],
    active: Mapping[str, Any],
    program_016_commit: str,
) -> Mapping[str, Any]:
    forensic_binding = _mapping(
        _mapping(proposal.get("predecessor"), "predecessor bindings").get(
            "redacted_private_forensic_disposition"
        ),
        "Program 016 forensic binding",
    )
    forensic = _load_bound(
        repository,
        forensic_binding,
        "forensic_disposition_fingerprint",
        commit=program_016_commit,
    )
    historical = _mapping(forensic.get("predecessor"), "historical Program 015 binding")
    child_binding = _mapping(historical.get("child_authority"), "Program 015 child binding")
    review_binding = _mapping(
        historical.get("child_authority_review"), "Program 015 child review binding"
    )
    sync_commit = str(
        _mapping(active.get("control_lineage"), "Program 015 control lineage").get(
            "synchronized_main_commit"
        )
    )
    if _HEX_40.fullmatch(sync_commit) is None:
        raise Program016AuthorityError("Program 015 synchronized-main binding differs")
    ...
    runtime = _mapping(child.get("runtime_binding"), "Program 015 child runtime binding")
    child_authority = _mapping(child.get("authority"), "Program 015 child authority")
    required_challenges = tuple(
        cast(
            Sequence[str],
            _sequence(child.get("required_review_challenges"), "Program 015 review challenges"),
        )
    )
    ...
    if (
        child.get("child_authority_id") != program_015.CHILD_AUTHORITY_ID
        or child.get("program_ordinal") != program_015.PROGRAM_ORDINAL
        or child.get("program_id") != program_015.PROGRAM_ID
        or child.get("operation_manifest") != program_015.OPERATION_MANIFEST
        ...
        raise Program016AuthorityError("Program 015 historical reviewed child differs")
...
def _derive_predecessor_state(
    ...
    terminal_binding = _mapping(
        predecessor_bindings.get("program_015_terminal_result"),
        "Program 015 public terminal binding",
    )
    ...
    active_raw = predecessor._read(program_015_root, "active-authority.json")
    active = _json_object(active_raw, "Program 015 active authority")
    if active_raw != (canonical_json(active) + "\n").encode():
        raise Program016AuthorityError("Program 015 active authority is not canonical")
    unsigned_active = dict(active)
    active_fingerprint = unsigned_active.pop("authority_fingerprint", None)
    control_lineage = _mapping(active.get("control_lineage"), "Program 015 control lineage")
    runtime = _mapping(active.get("runtime_binding"), "Program 015 runtime binding")
    source_commit = str(control_lineage.get("runtime_source_commit"))
    historical_identity = _historical_program_015_identity(
        repository, proposal, active, _authority_commit(authority)
    )
    expected_unsigned = {
        "schema_version": "program-015-raw-sip-recovery-active-authority-v1",
        "status": "ACTIVE-ONE-USE-RECOVERABLE",
        "authority_id": program_015.CHILD_AUTHORITY_ID,
        "program_id": program_015.PROGRAM_ID,
        "activation_mode": "INTERNAL-STANDING-MANDATE-DERIVATION",
        "external_authorization_root_required": False,
        "child_identity_fingerprint": historical_identity["child_identity_fingerprint"],
        "operation_manifest": program_015.OPERATION_MANIFEST,
        "consumption_boundary": program_015.CONSUMPTION_BOUNDARY,
    ...
    if (
        ...
        raise Program016AuthorityError("Program 015 active authority binding differs")
    sync_commit = str(control_lineage.get("synchronized_main_commit"))
    try:
        program_015.validate_operation_contract(repository, commit=sync_commit)
        program_014_state = program_015._derive_predecessor_state(
            repository, program_014_root, program_013_root, program_012_root, active
        )
        if predecessor._read(program_015_root, "predecessor-import-manifest.json") != (
            program_014_state.payload
        ):
            raise Program016AuthorityError("Program 015 predecessor import manifest differs")
        private_terminal = program_015._load_terminal_record(
            repository,
            program_015_root,
            program_014_root,
            program_013_root,
    ...
    except (program_015.Program015AuthorityError, predecessor.Program012AuthorityError) as error:
        raise Program016AuthorityError(str(error).replace("Program 015", "Program 016")) from None
    if (
        ...
        or program_015._public_terminal_payload(private_terminal)
        != _git(repository, "show", f"{_authority_commit(authority)}:{terminal_binding['path']}")
    ):
        raise Program016AuthorityError("Program 015 terminal disposition differs")

    requests = science.acquisition_requests()
    page_entries = {
        entry for entry in os.listdir(program_015_root) if program_015._PAGE_KEY.fullmatch(entry)
    }
    budget = program_015._Budget(program_014_state)
    completed_sessions: list[dict[str, Any]] = []
    frontier: dict[str, Any] | None = None
    frontier_request_index = -1
    for request_index, request in enumerate(
        requests[program_014_state.frontier_request_index :],
        start=program_014_state.frontier_request_index,
    ):
        session_entries = {
            entry for entry in page_entries if entry.startswith(f"session-{request.session}-")
        }
        if frontier is not None:
            if session_entries:
                raise Program016AuthorityError("Program 015 evidence continues after frontier")
            continue
        source = program_015._ReplaySource(
            program_015_root, request, active, program_014_state, budget
        )
        before_requests = budget.requests
        before_responses = budget.responses
        before_bytes = budget.response_bytes
        try:
            result = program_015._execute_session(request, source)
        except program_015._IncompletePageCheckpoint:
            completed_pages = budget.responses - before_responses
            page_index = completed_pages + 1
            if page_index != 1:
                raise Program016AuthorityError(
                    "Program 015 reviewed frontier is not first-page intent-only"
                ) from None
            incoming_token = None
            if completed_pages:
                ...
                if receipt.get("status") != 200:
                    raise Program016AuthorityError("Program 015 frontier status differs") from None
            intent = program_011.PageIntent(
            ...
            if session_entries != expected_entries:
                raise Program016AuthorityError("Program 015 frontier checkpoint differs") from None
            frontier = {
                "session": request.session.isoformat(),
                ...
                "intent_sha256": hashlib.sha256(
                    predecessor._read(program_015_root, f"{prefix}.intent.json")
                ).hexdigest(),
                "body_present": False,
        ...
        except (program_015._TransportRequired, program_015.Program015AuthorityError) as error:
            raise Program016AuthorityError(
                "Program 015 evidence is not the reviewed intent-only frontier"
            ) from error
        except program_011.Program011Error as error:
            raise Program016AuthorityError(
                "Program 015 completed prefix does not reparse"
            ) from error
        else:
            expected_entries = predecessor._expected_session_entries(request, len(result.pages))
            if session_entries != expected_entries:
                raise Program016AuthorityError("Program 015 completed session evidence differs")
            pages = []
                ...
                pages.append(
                    {
                        ...
                        "body_sha256": hashlib.sha256(
                            predecessor._read(program_015_root, f"{prefix}.body")
                        ).hexdigest(),
                        "receipt_sha256": hashlib.sha256(
                            predecessor._read(program_015_root, f"{prefix}.receipt.json")
                        ).hexdigest(),
            ...
            if budget.requests - before_requests != len(result.pages):
                raise Program016AuthorityError("Program 015 completed request count differs")
    if frontier is None or frontier_request_index < 0 or not completed_sessions:
        raise Program016AuthorityError("Program 015 reviewed recovery frontier is absent")
    if (
        ...
        or budget.requests - budget.responses != 4
    ):
        raise Program016AuthorityError("Program 015 cumulative terminal budget differs")
    runtime_014 = _mapping(authority.get("runtime_binding"), "runtime binding")
    unsigned_manifest: dict[str, Any] = {
        ...
        "runtime_binding": dict(runtime_014),
        ...
        "program_015_predecessor_manifest_sha256": program_014_state.sha256,
        "program_012_completed_sessions": program_014_state.manifest[
            "program_012_completed_sessions"
        ],
        "program_012_frontier": program_014_state.manifest["program_012_frontier"],
        "program_013_public_terminal": program_014_state.manifest["program_013_public_terminal"],
        "program_013_active_authority_fingerprint": program_014_state.manifest[
            "program_013_active_authority_fingerprint"
        ],
        "program_013_child_identity_fingerprint": program_014_state.manifest[
            "program_013_child_identity_fingerprint"
        ],
        "program_013_source_commit": program_014_state.manifest["program_013_source_commit"],
        "program_013_private_terminal_fingerprint": program_014_state.manifest[
            "program_013_private_terminal_fingerprint"
        ],
        "program_013_completed_sessions": program_014_state.manifest[
            "program_013_completed_sessions"
        ],
        "program_014_public_terminal": program_014_state.manifest["program_014_public_terminal"],
        "program_014_active_authority_fingerprint": program_014_state.manifest[
            "program_014_active_authority_fingerprint"
        ],
        "program_014_child_identity_fingerprint": program_014_state.manifest[
            "program_014_child_identity_fingerprint"
        ],
        "program_014_source_commit": program_014_state.manifest["program_014_source_commit"],
        "program_014_private_terminal_fingerprint": program_014_state.manifest[
            "program_014_private_terminal_fingerprint"
        ],
        "program_014_completed_sessions": program_014_state.manifest[
            "program_014_completed_sessions"
        ],
        "program_015_public_terminal": dict(terminal_binding),
        "program_015_active_authority_fingerprint": active_fingerprint,
        "program_015_child_identity_fingerprint": active["child_identity_fingerprint"],
        "program_015_source_commit": source_commit,
        "program_015_private_terminal_fingerprint": private_terminal["terminal_fingerprint"],
        "program_015_completed_sessions": completed_sessions,
        ...
        "consumed_intent_frontiers": [
            *program_014_state.manifest["consumed_intent_frontiers"],
            frontier,
        ],
        "frontier": frontier,
...
def _validate_predecessor_manifest(root_descriptor: int, expected: _PredecessorState) -> None:
    if predecessor._read(root_descriptor, "predecessor-import-manifest.json") != expected.payload:
        raise Program016AuthorityError("Program 016 predecessor import manifest differs")
...
def _append_atomic_or_validate(root_descriptor: int, key: str, payload: bytes) -> None:
    if predecessor._exists(root_descriptor, key):
        if predecessor._read(root_descriptor, key) != payload:
            raise Program016AuthorityError(f"Program 016 derived evidence differs: {key}")
        return
    try:
        predecessor._append_atomic(root_descriptor, key, payload)
    except predecessor.Program012AuthorityError as error:
        raise Program016AuthorityError(str(error).replace("Program 012", "Program 016")) from None
...
def _load_active(root_descriptor: int, expected: Mapping[str, Any]) -> Mapping[str, Any]:
    ...
    if value != expected or raw != (canonical_json(expected) + "\n").encode():
        raise Program016AuthorityError("Program 016 active authority differs")
    return value
...
def _repository_preflight(repository: Path, identity: Mapping[str, Any]) -> dict[str, str]:
    ...
    try:
        ...
        raise Program016AuthorityError("Program 016 repository identity is unavailable") from error
    if (
        ...
        raise Program016AuthorityError("Program 016 reviewed synchronized-main lineage differs")
    return {
...
def _authority_commit(authority: Mapping[str, Any]) -> str:
    ...
    if _HEX_40.fullmatch(value) is None:
        raise Program016AuthorityError("Program 016 authority policy commit differs")
    return value
...
def _source_commit(authority: Mapping[str, Any]) -> str:
    ...
    if _HEX_40.fullmatch(value) is None:
        raise Program016AuthorityError("Program 016 runtime source commit differs")
    return value
...
def _require_credentials_present(environ: Mapping[str, str] | None) -> None:
    ...
    if missing:
        raise Program016AuthorityError("Program 016 credentials missing: " + ", ".join(missing))
    ...
    *,
    ...
    try:
        ...
        if not fingerprint_optional:
            raise Program016AuthorityError(
                str(error).replace("Program 011", "Program 016")
            ) from None
        path = Path(str(binding.get("path")))
        raw = _git(repository, "show", f"{commit or 'HEAD'}:{path.as_posix()}")
        if hashlib.sha256(raw).hexdigest() != binding.get("sha256"):
            raise Program016AuthorityError("Program 016 bound artifact differs") from None
    ...
    try:
        for part in root.parts:
            ...
            try:
                ...
                raise Program016AuthorityError(
                    "Program 016 private evidence root is absent"
                ) from error
            os.close(descriptor)
            descriptor = child
        metadata = os.fstat(descriptor)
        if not stat.S_ISDIR(metadata.st_mode) or stat.S_IMODE(metadata.st_mode) & 0o077:
            raise Program016AuthorityError("Program 016 private evidence root is not private")
        return descriptor
...
def _repository(repository: Path) -> Path:
    if not isinstance(repository, Path):
        raise Program016AuthorityError("Program 016 repository root is invalid")
    resolved = repository.resolve()
    if not resolved.is_dir():
        raise Program016AuthorityError("Program 016 repository root is absent")
    return resolved


def _mapping(value: Any, label: str) -> Mapping[str, Any]:
    if not isinstance(value, Mapping):
        raise Program016AuthorityError(f"Program 016 {label} is invalid")
    return value
...
def _json_object(raw: bytes, label: str) -> dict[str, Any]:
    try:
        ...
        raise Program016AuthorityError(f"Program 016 {label} is invalid JSON") from error
    if type(value) is not dict:
        raise Program016AuthorityError(f"Program 016 {label} is not an object")
    return value
...
def _git(repository: Path, *arguments: str) -> bytes:
    try:
        ...
        raise Program016AuthorityError("Program 016 Git identity is unavailable") from error
