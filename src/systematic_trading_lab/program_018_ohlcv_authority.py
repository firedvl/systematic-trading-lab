"""Single-process Program 018 recovery of the Program 012-017 exposed prefix."""
...
PROGRAM_ID = "multi-hour-sector-etf-research-017"
PROGRAM_ORDINAL = 18
CHILD_AUTHORITY_ID = (
    "program-018-exposed-prefix-raw-alpaca-sip-recovery-and-"
    "structural-admission-child-2026-09-07-v1"
)
CONSUMPTION_BOUNDARY = "immediately before first provider transport invocation"
PRIVATE_ROOT = Path(".trading-lab/program-018-exposed-prefix-raw-alpaca-sip-v1")
...
PREDECESSOR_PRIVATE_ROOT = PROGRAM_017_PRIVATE_ROOT
PUBLIC_TERMINAL_PATH = Path(
    "config/research/program-018-exposed-prefix-raw-alpaca-sip-recovery-and-"
...
OPERATION_MANIFEST = {
    ...
    "sha256": "4bb06e1097f2aa44af84aeb1570f74982a4db57fb16ba969cb85fae582dd59c3",
    "fingerprint": "b7c8ac3f0794b2119a7ac27129ce14c665eea63eacba892b3c459b16c8686732",
}
PROPOSAL_REVIEW = {
    "path": (
        "config/research/program-018-exposed-prefix-raw-alpaca-sip-recovery-and-"
    ...
    "sha256": "7c5f7fe2e0f875c4370151880e8c34506b34f5d256af85d0b4dcc6e8d63d2532",
    "fingerprint": "a7dd85efb12466597de9740bcf7b90ef4b44f29c74da5116d2ceeab76d4050d2",
}
...
_DERIVED_KEYS = {
    ...
    "program-018-response-manifest.json",
}
...
_PUBLIC_TERMINAL_SHA256: str | None = (
    "90102e72d90391ec5f9e342bf21993daf9613f4fcfaf0218bfd25f041e73d396"
)
...
_PRIVATE_EVIDENCE_KEYS = {
    "program_018_response_manifest_sha256",
...
class Program018AuthorityError(ValueError):
    """Fail-closed Program 018 runtime or authority error."""


class Program018PostClaimPersistenceError(Program018AuthorityError):
    """Terminal evidence could not be persisted after possible transport."""


class CombinedRequestBudgetExhausted(Program018AuthorityError):
...
class _IncompletePageCheckpoint(Program018AuthorityError):
    pass
...
class _CombinedProjection:
    ...
    program_018_response_manifest: Mapping[str, Any]
    budget: _Budget
...
def _recognized_temporary_target(entry: str) -> str | None:
    if entry == "tmp-program-018-combined-canonical-raw":
        return "combined-canonical-raw.jsonl"
    if entry == "tmp-program-018-public-terminal":
        return "public-terminal"
    prefix = "tmp-program-018-write-"
...
class _ControlSnapshot:
    ...
    def _validate_immutable(self) -> None:
        if (
            ...
            raise Program018AuthorityError("Program 018 held control metadata changed")

    def validate_continuation(self) -> None:
        self._validate_immutable()
        if self.failed_write is not None:
            raise Program018AuthorityError("Program 018 is failure-closeout-only")
        if self.failure_only:
            raise Program018AuthorityError("Program 018 is failure-closeout-only")
        if _stat_tuple(self.mutable_root, _FULL_STAT_FIELDS) != self.mutable_expected:
            raise Program018AuthorityError("Program 018 mutable root changed")

    def refresh_after_write(self) -> None:
        self._validate_immutable()
        if self.failure_only and not self.allow_failure_writes:
            raise Program018AuthorityError("Program 018 failed write cannot resume")
    ...
    def validate_failure_closeout(self) -> None:
        ...
        if not self.failure_only:
            raise Program018AuthorityError("Program 018 failure closeout was not latched")
        if _stat_tuple(self.mutable_root, _FULL_STAT_FIELDS) != self.mutable_expected:
            raise Program018AuthorityError("Program 018 failure-closeout root changed")
    ...
    def begin_failure_closeout(self) -> None:
        ...
        if current != expected:
            raise Program018AuthorityError("Program 018 failed-write root changed")
        ...
        try:
            with os.fdopen(descriptor, "wb") as handle:
                if handle.write(payload) != len(payload):
                    raise Program018AuthorityError("Program 018 atomic write was incomplete")
                handle.flush()
            ...
            try:
                ...
                raise Program018AuthorityError(
...
def _tracked_new_temp(root_descriptor: int, label: str) -> tuple[str, int]:
    ...
    def create() -> None:
        ...
        try:
            ...
            raise Program018AuthorityError(
                f"Program 018 temporary evidence exists: {key}"
            ) from None
...
def _tracked_append_or_validate(root_descriptor: int, key: str, payload: bytes) -> None:
    if predecessor._exists(root_descriptor, key):
        if predecessor._read(root_descriptor, key) != payload:
            raise Program018AuthorityError(f"Program 018 derived evidence differs: {key}")
        return
...
class _Budget:
    def __init__(self, predecessor_state: _PredecessorState) -> None:
        ...
        if completed_requests != completed_responses or consumed_intents != 6:
            raise Program018AuthorityError("Program 018 predecessor budget differs")
        ...
        if (
            ...
            raise Program018AuthorityError("Program 018 inherited budget exceeds its ceiling")
    ...
    def reserve_request(self) -> None:
        if self.requests >= science.MAXIMUM_REQUESTS_AND_RESPONSES:
            raise CombinedRequestBudgetExhausted(
                "Program 018 combined request ceiling is exhausted"
            )
        self.requests += 1
    ...
    def accept_response(
        ...
        if (
            ...
            raise Program018AuthorityError("Program 018 response timestamp must be UTC")
        if self.responses >= self.effective_response_ceiling:
            raise CombinedRequestBudgetExhausted(
                "Program 018 combined response ceiling is exhausted"
            )
        self.responses += 1
        ...
        if len(body) > science.MAXIMUM_RESPONSE_PAGE_BYTES:
            raise Program018AuthorityError("Program 018 response exceeds the 8 MiB page ceiling")
        if self.session_bytes[request.session] > science.MAXIMUM_SESSION_RESPONSE_BYTES:
            raise Program018AuthorityError("Program 018 session exceeds the 8 MiB byte ceiling")
        if self.response_bytes > science.MAXIMUM_TOTAL_RESPONSE_BYTES:
            raise Program018AuthorityError("Program 018 total response byte ceiling exceeded")
...
class _ReadLockedRoot:
    ...
    def __enter__(self) -> None:
        try:
            ...
            raise Program018AuthorityError(f"{self._program} read-only lock is absent") from error
        self._handle = os.fdopen(descriptor, "rb", buffering=0)
        metadata = os.fstat(self._handle.fileno())
        if not stat.S_ISREG(metadata.st_mode) or stat.S_IMODE(metadata.st_mode) & 0o077:
            self._handle.close()
            raise Program018AuthorityError(f"{self._program} read-only lock is invalid")
...
def credential_presence_preflight(
    ...
    with _locked_controls(repository, authority, create_program_018=True) as (
        program_018_root,
        program_017_root,
        program_016_root,
        program_015_root,
        ...
        if locked != authority:
            raise Program018AuthorityError("Program 018 authority changed under policy lock")
        ...
        _ROOT_TRACKERS[program_018_root] = snapshot
        try:
            ...
            _require_working_disk_capacity(program_018_root)
            return credential_contract.credential_presence_preflight(environ)
        finally:
            _ROOT_TRACKERS.pop(program_018_root, None)
            snapshot.close()
...
def validate_operation_contract(
    ...
    try:
        program_017.validate_operation_contract(repository, commit=commit)
    except program_017.Program017AuthorityError as error:
        raise Program018AuthorityError(str(error).replace("Program 017", "Program 018")) from None
    ...
    first = _mapping(
        recovery.get("first_program_018_request_private_derivation"), "first request contract"
    )
    if (
        proposal.get("schema_version")
        != "program-018-exposed-prefix-raw-alpaca-sip-recovery-and-structural-admission-proposal-v3"
        ...
        or review.get("reviewed_source_commit") != "ac312d173a2c5404575a06316db633ce59a9ade8"
        or review.get("reviewed_source_tree") != "d7df0953e405d9068fc92cea7a6a50e9796f8264"
        or review.get("reviewed_proposal") != OPERATION_MANIFEST
        or inheritance.get("base_contract") != "predecessor.program_017_scientific_contract"
        or inheritance.get("all_unlisted_program_017_v5_fields_and_semantics_inherited_exactly")
        is not True
        or any(
            ...
            for key in (
                ...
                "authority_expanded",
                "program_012_through_program_017_history_mutation_allowed",
            )
        )
        ...
        or recovery.get("program_018_private_root") != PRIVATE_ROOT.as_posix()
        or recovery.get("incomplete_program_017_session_or_page_reuse_allowed") is not False
        or recovery.get("incomplete_program_017_request_reissue_allowed") is not False
        or recovery.get("program_018_request_reissue_after_durable_intent_allowed") is not False
        or recovery.get("ambiguous_program_018_transport_is_consumed_no_retry") is not True
        or any(
            recovery.get(key) is not False
            for key in (
                "retained_program_018_response_retry_allowed",
                "retained_program_018_http_result_retry_allowed",
                "parsed_program_018_missingness_retry_allowed",
                "program_018_data_quality_outcome_retry_allowed",
            )
        )
        ...
        or boundary.get("all_eight_controls_held_continuously") is not True
        ...
        or boundary.get("program_018_mutable_root_stable_identity_tuple")
        != ["st_dev", "st_ino", "st_mode", "st_uid", "st_gid"]
        or boundary.get("program_018_mutable_root_expected_full_tuple")
        != [
            "st_dev",
            "st_ino",
            "st_mode",
        ...
        != science.MAXIMUM_REQUESTS_AND_RESPONSES - 6
        or cumulative.get("consumed_intent_without_response_count") != 6
        ...
        or credentials.get("inherit_program_017_v5_credential_contract_exactly") is not True
        ...
        or private_terminal.get("schema_version") != "program-018-private-terminal-v1"
        ...
        or static_private
        != {
            "schema_version": "program-018-private-terminal-v1",
        ...
        or public_terminal.get("schema_version") != "program-018-exposed-prefix-terminal-result-v1"
        or set(_sequence(public_terminal.get("exact_top_level_keys"), "public terminal keys"))
        != _PUBLIC_TERMINAL_KEYS
        or static_public
        != {
            "schema_version": "program-018-exposed-prefix-terminal-result-v1",
        ...
        is not True
        or substitutions.get("program_017_to_program_018_substitutions_are_exhaustive") is not True
        ...
        != _PUBLIC_LINEAGE_KEYS
        or public_lineage.get("status") != "ADMITTED-PROGRAM-018-RAW-STRUCTURAL-PREFIX"
        ...
        or firewall.get("inherit_program_017_registered_range_derivation") is not True
        or firewall.get(
        ...
        raise Program018AuthorityError("Program 018 operation contract differs")
    return proposal
...
def _derive_control_validated_authority(repository: Path) -> Mapping[str, Any]:
    ...
    try:
        ...
        raise Program018AuthorityError(str(error)) from None
    ...
    if (
        ...
        raise Program018AuthorityError("Program 018 reviewed child identity differs")
    ...
    try:
        ...
        raise Program018AuthorityError(str(error).replace("Program 011", "Program 018")) from None
    ...
    if _repository_preflight(repository, identity) != lineage:
        raise Program018AuthorityError("Program 018 repository changed during validation")
...
def _locked_controls(
    ...
    *,
    create_program_018: bool,
) -> Iterator[tuple[int, int, int, int, int, int, int]]:
    program_018_root = _open_root(repository, PRIVATE_ROOT, create=create_program_018)
    try:
        with git_controls._LockedRoot(program_018_root), ExitStack() as stack:
            roots: list[int] = []
            for path, label in (
...
def _activate_locked_authority(
    ...
    if entries not in (
        ...
        raise Program018AuthorityError("Program 018 one-use authority state already exists")
...
def _execute_mock_acquisition(
    ...
    *,
    ...
    if type(transport) is not MockBarsTransport or environ is os.environ:
        raise Program018AuthorityError("Program 018 test execution requires finite inputs")
    return _execute_acquisition(
        repository,
        environ=environ,
...
def _execute_acquisition(
    ...
    *,
    ...
    with _locked_controls(repository, expected_authority, create_program_018=True) as (
        program_018_root,
        program_017_root,
        program_016_root,
        program_015_root,
        ...
        _ROOT_TRACKERS[program_018_root] = control_snapshot
        try:
            if _derive_control_validated_authority(repository) != authority:
                raise Program018AuthorityError("Program 018 authority changed under lock")
            _recover_private_terminal_if_present(
                repository,
                program_018_root,
                program_017_root,
            ...
            if after_activation is not None:
                after_activation()
            _validate_launcher(program_018_root, authority, predecessor_state)
            _load_active(program_018_root, authority)
            ...
            def consume() -> None:
                ...
                if predecessor._exists(program_018_root, "claim.json"):
                    _validate_claim(program_018_root, authority, predecessor_state)
                    return
                _tracked_append_atomic(
                    program_018_root,
                    "claim.json",
            ...
            temp_key, temp_descriptor = _tracked_new_temp(
                program_018_root, "combined-canonical-raw"
            )
            completed = False
            try:
                ...
                if not completed:
                    with suppress(FileNotFoundError):
                        _tracked_unlink(program_018_root, temp_key)
            ...
            if budget.new_requests != budget.new_responses:
                raise Program018AuthorityError("Program 018 new request and response counts differ")
            ...
            for key, payload in cast(Mapping[str, bytes], evidence["payloads"]).items():
                _tracked_append_or_validate(program_018_root, key, payload)
            _FINAL_VALIDATIONS[program_018_root] = (projection, evidence)
            ...
            _tracked_append_atomic(program_018_root, _TERMINAL_KEY, terminal_payload)
            ...
            try:
                ...
                raise Program018PostClaimPersistenceError(
                    "Program 018 public terminal persistence failed after possible transport"
            ...
            if (
                ...
                or _has_consumed_state(program_018_root)
            ) and not predecessor._exists(program_018_root, _TERMINAL_KEY):
                try:
                    ...
                    raise Program018PostClaimPersistenceError(
                        "Program 018 terminal persistence failed after possible transport"
                    ) from persistence_error
            if published_failure and isinstance(error, Exception):
                raise Program018AuthorityError(
                    "Program 018 acquisition ended with a sealed failure"
                ) from None
            raise
        finally:
            _FINAL_VALIDATIONS.pop(program_018_root, None)
            _ROOT_TRACKERS.pop(program_018_root, None)
        ...
        if not self._loaded:
            ...
            try:
                ...
                if any(not value or "\r" in value or "\n" in value for value in credentials):
                    raise Program018AuthorityError("Program 018 OHLCV credentials are required")
...
class _PersistentSessionSource:
    ...
    def response(self, intent: program_011.PageIntent) -> raw_contract.RawResponse:
        if self._pending is not None:
            raise Program018AuthorityError("Program 018 persistent source state differs")
        ...
        if any(present):
            if present[0]:
                ...
                if predecessor._read(self._root_descriptor, f"{prefix}.intent.json") != expected:
                    raise Program018AuthorityError(
                        "Program 018 partial request intent checkpoint differs"
                    )
            raise Program018AuthorityError(
                "Program 018 ambiguous page checkpoint forbids request reissue"
            )
    ...
    def retain(self, page_index: int, body: bytes) -> program_011.RetainedPage:
        ...
        if (
            ...
            raise Program018AuthorityError("Program 018 retained response differs")
        self._pending = None
        return pending
...
class _ReplaySource:
    ...
    def response(self, intent: program_011.PageIntent) -> raw_contract.RawResponse:
        ...
        if not all(present):
            if (
                ...
                raise Program018AuthorityError("Program 018 partial page checkpoint differs")
            self._budget.reserve_request()
    ...
    def retain(self, page_index: int, body: bytes) -> program_011.RetainedPage:
        ...
        if pending is None or pending.page_index != page_index:
            raise Program018AuthorityError("Program 018 replayed response differs")
        self._pending = None
        return pending
...
class _ExistingSessionSource:
    ...
    def retain(self, page_index: int, body: bytes) -> program_011.RetainedPage:
        ...
        if (
            ...
            raise Program018AuthorityError("Program 018 existing response differs")
        self._pending = None
        return pending
...
def _combined_projection(
    ...
    *,
    ...
    program_016_state = predecessor_state.program_016_state
    ...
    discarded_program_017_index = predecessor_state.frontier_request_index - 1
    ...
    for request_index, request in enumerate(science.acquisition_requests()):
        if request_index < program_012_frontier:
            ...
            source = _ExistingSessionSource(program_018_root, request, authority, predecessor_state)
        else:
        ...
        for row in result.rows:
            ...
            if canonical_bytes > _MAXIMUM_CANONICAL_PROJECTION_BYTES:
                raise Program018AuthorityError("Program 018 canonical projection exceeds 8 GiB")
            if canonical_file is not None and canonical_file.write(line) != len(line):
                raise Program018AuthorityError("Program 018 canonical raw write was incomplete")
    ...
    discarded = _mapping(
        predecessor_state.manifest.get("discarded_program_017_partial_session"),
        "discarded Program 017 session",
    )
    discarded_pages = _sequence(discarded.get("completed_pages"), "discarded Program 017 pages")
    discarded_bytes = sum(
        _count(_mapping(page, "discarded page").get("response_bytes"), "discarded bytes")
    ...
    if (
        ...
        raise Program018AuthorityError("Program 018 predecessor replay budget differs")
    response_manifest = {
        "schema_version": "program-018-private-response-manifest-v1",
        "program_id": PROGRAM_ID,
    ...
    return _CombinedProjection(
        ...
        missing=missing,
        morning=morning,
        program_018_response_manifest=response_manifest,
        budget=budget,
    )
...
def _execute_session(
    ...
    try:
        ...
        replacement = Program018AuthorityError(str(error).replace("Program 012", "Program 018"))
        ...
        raise Program018AuthorityError(str(error).replace("Program 012", "Program 018")) from error
...
def _reconstruct_state(
    ...
    *,
    ...
    if entries & _DERIVED_KEYS and _TERMINAL_KEY not in entries:
        raise Program018AuthorityError("Program 018 derived evidence lacks a terminal")
    allowed = {
        "active-authority.json",
        "claim.json",
        _LAUNCHER_KEY,
    ...
    for entry in entries - allowed:
        if _PAGE_KEY.fullmatch(entry):
            ...
            raise Program018AuthorityError("Program 018 private checkpoint contains unknown state")
    if len(temporary_entries) > 1:
        raise Program018AuthorityError(
            "Program 018 private checkpoint has multiple temporary files"
        )
    ...
    if page_entries:
        if claim_exists:
            if successful_loads == 0:
                raise Program018AuthorityError(
                    "Program 018 page evidence lacks a successful credential audit"
                )
            ...
            raise Program018AuthorityError("Program 018 provider evidence lacks a transport claim")
    elif claim_exists:
        raise Program018AuthorityError("Program 018 transport claim lacks page evidence")
    if credential_loads and not page_entries:
        raise Program018AuthorityError("Program 018 credential audit lacks request evidence")
    ...
    for entry in page_entries:
        ...
        try:
            ...
            raise Program018AuthorityError("Program 018 page key is invalid") from error
        if session not in expected_sessions or not 1 <= page_index <= 16:
            raise Program018AuthorityError("Program 018 page key is outside the recovery plan")
    ...
    for request in requests[predecessor_state.frontier_request_index :]:
        ...
        if frontier_found:
            if session_entries:
                raise Program018AuthorityError("Program 018 checkpoint skips a request session")
            continue
        ...
        try:
            ...
            if frozenset(session_entries) not in {
                ...
                raise Program018AuthorityError(
                    "Program 018 terminal checkpoint is not a reachable prefix"
                ) from None
            frontier_found = True
            ...
            if session_entries != predecessor._expected_session_entries(request, completed_pages):
                raise Program018AuthorityError(
                    "Program 018 checkpoint skips a request page"
                ) from None
            if require_complete:
                raise Program018AuthorityError(
                    "Program 018 acquisition evidence is incomplete"
                ) from None
            frontier_found = True
        except (Program018AuthorityError, program_011.Program011Error):
            if not allow_terminal_failure:
                raise
            completed_pages = sum(
                page["session"] == request.session.isoformat() for page in budget.pages
            ...
            if session_entries != predecessor._expected_session_entries(request, len(result.pages)):
                raise Program018AuthorityError(
                    "Program 018 checkpoint continues after a terminal page"
                )
    if require_complete and frontier_found:
        raise Program018AuthorityError("Program 018 acquisition evidence is incomplete")
    return budget
...
def _runtime_failure_credential_load_count(
    ...
    except Program018AuthorityError:
        return len(
            {
...
def _claim_value(
    ...
    unsigned: dict[str, Any] = {
        "schema_version": "program-018-private-acquisition-claim-v1",
        "program_id": PROGRAM_ID,
        "authority_id": authority["authority_id"],
        "authority_fingerprint": authority["authority_fingerprint"],
        "operation_manifest": OPERATION_MANIFEST,
...
def _validate_claim(
    ...
    if predecessor._read(root_descriptor, "claim.json") != _claim_payload(
        ...
        raise Program018AuthorityError("Program 018 transport claim differs")
...
def _intent_value(
    ...
    unsigned: dict[str, Any] = {
        "schema_version": "program-018-private-request-intent-v1",
        "program_id": PROGRAM_ID,
        "authority_fingerprint": authority["authority_fingerprint"],
        "operation_manifest": OPERATION_MANIFEST,
        "runtime_binding": dict(_runtime_binding(authority)),
...
def _response_receipt(
    ...
    if type(response.status) is not int or not 100 <= response.status <= 599:
        raise Program018AuthorityError("Program 018 response status is invalid")
    ...
    claim = _claim_value(authority, predecessor_state)
    unsigned: dict[str, Any] = {
        "schema_version": "program-018-private-response-receipt-v1",
        "program_id": PROGRAM_ID,
        "authority_fingerprint": authority["authority_fingerprint"],
        "operation_manifest": OPERATION_MANIFEST,
        "runtime_binding": dict(_runtime_binding(authority)),
...
def _load_completed_page(
    ...
    if predecessor._read(root_descriptor, f"{prefix}.intent.json") != expected_intent:
        raise Program018AuthorityError("Program 018 request intent checkpoint differs")
    ...
    if (
        ...
        raise Program018AuthorityError("Program 018 response checkpoint differs")
...
def _validate_page_intent(
    ...
    if (
        ...
        raise Program018AuthorityError("Program 018 request intent differs")
    ...
    if (
        ...
        raise Program018AuthorityError("Program 018 first intent differs from the frontier")
...
def _credential_attempt(
    ...
    if type(sequence) is not int or sequence < 1:
        raise Program018AuthorityError("Program 018 credential sequence differs")
    unsigned: dict[str, Any] = {
        "schema_version": "program-018-private-credential-load-attempt-v1",
        "authority_fingerprint": authority["authority_fingerprint"],
        "runtime_binding": dict(_runtime_binding(authority)),
...
def _credential_receipt_payload(attempt_identity: str, status: str) -> bytes:
    if _HEX_64.fullmatch(attempt_identity) is None or status not in {"SUCCESS", "FAILURE"}:
        raise Program018AuthorityError("Program 018 credential receipt differs")
    value = {
...
def _launcher_value(
    ...
    unsigned = {
        "schema_version": "program-018-private-single-process-launcher-v1",
        "authority_fingerprint": authority["authority_fingerprint"],
        "predecessor_import_manifest_sha256": predecessor_state.sha256,
...
def _validate_launcher(
    ...
    if (
        ...
        raise Program018AuthorityError("Program 018 launcher changed or the process was restarted")
...
def _credential_load_counts(
    ...
    for entry in os.listdir(root_descriptor):
        ...
        if raw != (canonical_json(value) + "\n").encode():
            raise Program018AuthorityError("Program 018 credential audit is not canonical")
        target = attempts if match.group("kind") == "attempt" else receipts
        if sequence in target:
            raise Program018AuthorityError("Program 018 credential sequence is duplicated")
        target[sequence] = value
    sequences = sorted(attempts)
    if sequences != list(range(1, len(sequences) + 1)) or not set(receipts) <= set(attempts):
        raise Program018AuthorityError("Program 018 credential sequence is not contiguous")
    ...
    for sequence in sequences:
        ...
        if set(attempt) != _CREDENTIAL_ATTEMPT_KEYS or attempt != expected_attempt:
            raise Program018AuthorityError("Program 018 credential attempt binding differs")
        receipt = receipts.get(sequence)
        if receipt is None:
            if sequence != sequences[-1]:
                raise Program018AuthorityError("Program 018 credential receipt sequence has a gap")
            continue
        if (
            set(receipt) != _CREDENTIAL_RECEIPT_KEYS
            or receipt.get("schema_version") != "program-018-private-credential-load-receipt-v1"
            ...
            raise Program018AuthorityError("Program 018 credential receipt binding differs")
        if receipt["status"] == "SUCCESS":
            successful += 1
        elif sequence != sequences[-1]:
            raise Program018AuthorityError("Program 018 failed credential load is not terminal")
...
def _reserve_process_credential_access(write_attempt: Callable[[], None]) -> None:
    ...
    with _PROCESS_CREDENTIAL_LOCK:
        if process_id == _PROCESS_CREDENTIAL_PID:
            raise Program018AuthorityError(
                "Program 018 credential access is already consumed in this process"
            )
        try:
            write_attempt()
        finally:
...
def _revalidate_transport_boundary(
    ...
    if _derive_control_validated_authority(repository) != authority:
        raise Program018AuthorityError("Program 018 authority changed at transport boundary")
    snapshot = _ROOT_TRACKERS.get(program_018_root)
    if snapshot is None:
        raise Program018AuthorityError("Program 018 control snapshot is absent")
    snapshot.validate_continuation()
    _validate_predecessor_manifest(program_018_root, predecessor_state)
    _require_working_disk_capacity(program_018_root)
...
def _revalidate_closeout_boundary(
    ...
    if _derive_control_validated_authority(repository) != authority:
        raise Program018AuthorityError("Program 018 authority changed at closeout")
...
def _revalidate_failure_closeout_boundary(
    ...
    if _derive_control_validated_authority(repository) != authority:
        raise Program018AuthorityError("Program 018 authority changed at failure closeout")
    snapshot = _ROOT_TRACKERS.get(root_descriptor)
    if snapshot is not None:
        ...
        if snapshot.failed_write is not None:
            ...
            if _recognized_temporary_target(failed_key) is not None:
                ...
                expected = {f"tmp-program-018-{failed_key.removeprefix('tmp-')}"}
            elif failed_key == "combined-canonical-raw.jsonl":
                expected = {"tmp-program-018-combined-canonical-raw"}
            elif (
                ...
                expected = {f"tmp-program-018-write-{failed_key}"}
            else:
                expected = set()
            if temporary_entries not in (set(), expected):
                raise Program018AuthorityError("Program 018 failure temp does not match its stage")
            ...
            if target is None or not _restart_temp_stage_is_reachable(
                ...
                raise Program018AuthorityError("Program 018 recovery temp stage is unreachable")
    _validate_predecessor_manifest(root_descriptor, predecessor_state)
    _reconstruct_state(
        root_descriptor,
        predecessor_state,
...
def _require_zero_protected_overlap(repository: Path, commit: str) -> None:
    ...
    if any(
        ...
        raise Program018AuthorityError("Program 018 request chronology overlaps protected data")
...
def _require_working_disk_capacity(root_descriptor: int) -> None:
    ...
    if filesystem.f_bavail * filesystem.f_frsize < _WORKING_DISK_RESERVATION_BYTES:
        raise Program018AuthorityError("Program 018 requires 16 GiB of available working space")
...
def _runtime_binding(authority: Mapping[str, Any]) -> Mapping[str, Any]:
    ...
    if (
        ...
        raise Program018AuthorityError("Program 018 runtime binding differs")
    return runtime


def _count(value: Any, label: str) -> int:
    if type(value) is not int or value < 0:
        raise Program018AuthorityError(f"Program 018 {label} differs")
    return value
...
def _derived_admission_evidence(
    ...
    program_018_proposal = validate_operation_contract(
    ...
    admission["schema_version"] = "program-018-private-structural-admission-report-v1"
    ...
    missing_value = {
        "schema_version": "program-018-private-combined-missing-coordinates-v1",
        "program_id": PROGRAM_ID,
        "predecessor_import_manifest_sha256": predecessor_state.sha256,
        "sessions": {
    ...
    response_payload = (canonical_json(projection.program_018_response_manifest) + "\n").encode()
    ...
    private_dataset_identity = fingerprint(
        {
            ...
            "predecessor_import_manifest_sha256": predecessor_state.sha256,
            "program_018_response_manifest_sha256": response_sha,
    ...
    public_lineage = (
        {
            "schema_version": ("program-018-public-raw-structural-prefix-lineage-manifest-v1"),
            "program_id": PROGRAM_ID,
            "authority_id": authority["authority_id"],
            "authority_fingerprint": authority["authority_fingerprint"],
            "source_commit": _source_commit(authority),
            "status": "ADMITTED-PROGRAM-018-RAW-STRUCTURAL-PREFIX",
    ...
    if public_lineage is not None and (
        ...
        raise Program018AuthorityError("Program 018 public and private identities collide")
    dataset_manifest = {
        "schema_version": "program-018-private-combined-dataset-manifest-v1",
        "program_id": PROGRAM_ID,
        "authority_id": authority["authority_id"],
        "authority_fingerprint": authority["authority_fingerprint"],
        "source_commit": _source_commit(authority),
    ...
    return {
        ...
        "payloads": {
            "program-018-response-manifest.json": response_payload,
...
def _admission_terminal_payload(
    ...
    if _HEX_64.fullmatch(private_identity) is None:
        raise Program018AuthorityError("Program 018 private dataset identity differs")
    lineage_identity = (
    ...
    record: dict[str, Any] = {
        ...
        "program_018_credential_loads": _credential_load_counts(
            root_descriptor, authority, predecessor_state
        )[0],
...
def _runtime_failure_payload(
    ...
    private_evidence = {
        "program_018_response_manifest_sha256": _evidence_sha256_if_present(
            root_descriptor, "program-018-response-manifest.json"
        ),
    ...
    record: dict[str, Any] = {
        ...
        "program_018_credential_loads": _runtime_failure_credential_load_count(
            root_descriptor, authority, predecessor_state
        ),
...
def _load_terminal_record(
    ...
    *,
    ...
    if (
        ...
        or record.get("schema_version") != "program-018-private-terminal-v1"
        ...
        raise Program018AuthorityError("Program 018 private terminal binding differs")
    ...
    if _count(
        record.get("program_018_credential_loads"), "terminal credential count"
    ) != durable_credentials or record.get(
        "provider_transport_attempted"
    ) is not predecessor._exists(root_descriptor, "claim.json"):
        raise Program018AuthorityError("Program 018 terminal credential count differs")
    ...
    if result_kind == "RUNTIME-FAILURE":
        ...
        if (
            ...
            raise Program018AuthorityError("Program 018 runtime terminal semantics differ")
        _validate_runtime_failure_counts(record, counts)
        for field, key in (
            ("program_018_response_manifest_sha256", "program-018-response-manifest.json"),
            ...
            if private_evidence[field] != _evidence_sha256_if_present(root_descriptor, key):
                raise Program018AuthorityError("Program 018 runtime evidence hash differs")
        ...
        for key, expected in payloads.items():
            if predecessor._read(root_descriptor, key) != expected:
                raise Program018AuthorityError("Program 018 derived evidence differs")
        if (
            _evidence_sha256(root_descriptor, "combined-canonical-raw.jsonl")
            != projection.canonical_sha256
        ):
            raise Program018AuthorityError("Program 018 combined canonical evidence differs")
        admission = _mapping(evidence.get("admission"), "structural admission")
        admission_passed = admission.get("admission_passed") is True
        ...
        if (
            ...
            raise Program018AuthorityError("Program 018 admission terminal semantics differ")
        _validate_budget_counts(record, projection.budget)
    else:
        raise Program018AuthorityError("Program 018 private terminal result kind differs")
    return record
...
def _validate_budget_counts(record: Mapping[str, Any], budget: _Budget) -> None:
    if (
        ...
        raise Program018AuthorityError("Program 018 terminal budget differs")
...
def _read_public_terminal_temp(root_descriptor: int, key: str) -> bytes:
    try:
        ...
        raise Program018AuthorityError("Program 018 public terminal temp is invalid") from error
    with os.fdopen(descriptor, "rb") as handle:
        ...
        if (
            ...
            raise Program018AuthorityError("Program 018 public terminal temp is invalid")
        return handle.read()
...
def _publish_public_terminal(
    ...
    if validated != record:
        raise Program018AuthorityError("Program 018 terminal changed before publication")
    ...
    try:
        ...
        if existing_descriptor is not None:
            ...
            if not stat.S_ISREG(metadata.st_mode) or existing != payload:
                raise Program018AuthorityError("Program 018 public terminal artifact differs")
            os.fsync(public_descriptor)
            return
        recovery_temp = "tmp-program-018-public-terminal"
        if predecessor._exists(root_descriptor, recovery_temp):
            ...
            if retained == payload:
                try:
                    ...
                    raise Program018AuthorityError(
                        "Program 018 public terminal recovery raced"
                    ) from None
                os.fsync(public_descriptor)
                _tracked_unlink(root_descriptor, recovery_temp)
                return
            if not payload.startswith(retained):
                raise Program018AuthorityError("Program 018 public terminal temp differs")
            _tracked_unlink(root_descriptor, recovery_temp)
        ...
        try:
            with os.fdopen(temp_descriptor, "wb") as handle:
                if handle.write(payload) != len(payload):
                    raise Program018AuthorityError(
                        "Program 018 public terminal write was incomplete"
                    )
                handle.flush()
            ...
            try:
                ...
                raise Program018AuthorityError(
                    "Program 018 public terminal artifact already exists"
                ) from None
            os.fsync(public_descriptor)
        finally:
            with suppress(FileNotFoundError):
...
def _recover_predecessor_manifest_temp(
    ...
    if predecessor._read(root_descriptor, temp_key) != predecessor_state.payload:
        raise Program018AuthorityError("Program 018 predecessor manifest temp differs")
    _publish_temp_or_validate(
        root_descriptor,
        temp_key,
        "predecessor-import-manifest.json",
...
def _recover_private_terminal_if_present(
    ...
    terminal_temp = "tmp-program-018-write-terminal.json"
    ...
    if predecessor._exists(root_descriptor, _TERMINAL_KEY):
        ...
        raise Program018AuthorityError("Program 018 acquisition is terminally sealed")
    ...
    try:
        ...
        _load_active(root_descriptor, authority)
        raise Program018AuthorityError("Program 018 single-process transaction was interrupted")
    except Exception as error:
        try:
            _seal_runtime_failure(
                repository,
        ...
        raise Program018AuthorityError("Program 018 acquisition is terminally sealed") from None
...
def _seal_runtime_failure(
    ...
    snapshot = _ROOT_TRACKERS.get(root_descriptor)
    if snapshot is None:
        raise Program018AuthorityError("Program 018 failure control snapshot is absent")
...
def _reject_terminal_state(repository: Path) -> None:
    ...
    try:
        ...
        if _PUBLIC_TERMINAL_SHA256 is None:
            return
        raise Program018AuthorityError("Program 018 terminal result artifact is absent") from error
    except OSError as error:
        raise Program018AuthorityError("Program 018 terminal result artifact is invalid") from error
    try:
        metadata = os.fstat(descriptor)
        if not stat.S_ISREG(metadata.st_mode) or metadata.st_size > 1_048_576:
            raise Program018AuthorityError("Program 018 terminal result artifact is invalid")
        with os.fdopen(descriptor, "rb") as handle:
            descriptor = -1
            raw = handle.read()
    finally:
    ...
    if raw != (canonical_json(value) + "\n").encode():
        raise Program018AuthorityError("Program 018 public terminal is not canonical")
    _validate_public_terminal_shape(value)
    if _PUBLIC_TERMINAL_SHA256 is None:
        raise Program018AuthorityError("Program 018 terminal result lacks immutable binding")
    if hashlib.sha256(raw).hexdigest() != _PUBLIC_TERMINAL_SHA256:
        raise Program018AuthorityError("Program 018 terminal result semantics differ")
    raise Program018AuthorityError("Program 018 authority is terminally revoked")


def _validate_public_terminal_shape(value: Mapping[str, Any]) -> None:
    if (
        set(value) != _PUBLIC_TERMINAL_KEYS
        or value.get("schema_version") != "program-018-exposed-prefix-terminal-result-v1"
        ...
        raise Program018AuthorityError("Program 018 public terminal schema differs")
    ...
    if result_kind == "RUNTIME-FAILURE":
        ...
        expected_status = "ADMITTED-PROGRAM-018-RAW-STRUCTURAL-PREFIX"
        expected_admission = True
    else:
        raise Program018AuthorityError("Program 018 public terminal result kind differs")
    if (
        value.get("status") != expected_status
        or value.get("admission_passed") is not expected_admission
    ):
        raise Program018AuthorityError("Program 018 public terminal branch differs")
    lineage = value.get("dataset_lineage_manifest")
    if expected_admission:
        _validate_public_lineage(lineage, value)
    elif lineage is not None:
        raise Program018AuthorityError("Program 018 failure terminal publishes lineage")
...
def _validate_public_lineage(value: Any, terminal: Mapping[str, Any]) -> dict[str, Any]:
    ...
    if (
        ...
        or lineage.get("status") != "ADMITTED-PROGRAM-018-RAW-STRUCTURAL-PREFIX"
        or _HEX_64.fullmatch(str(lineage.get("dataset_lineage_identity"))) is None
    ):
        raise Program018AuthorityError("Program 018 public dataset lineage differs")
    return lineage


def _exact_object(value: Any, keys: set[str], label: str) -> dict[str, Any]:
    if type(value) is not dict or set(value) != keys:
        raise Program018AuthorityError(f"Program 018 {label} schema differs")
...
def _evidence_sha256(root_descriptor: int, key: str) -> str:
    if _EVIDENCE_KEY.fullmatch(key) is None:
        raise Program018AuthorityError("Program 018 evidence key is invalid")
    try:
        descriptor = os.open(
            key,
            os.O_RDONLY | getattr(os, "O_CLOEXEC", 0) | getattr(os, "O_NOFOLLOW", 0),
        ...
        raise Program018AuthorityError(f"Program 018 evidence is absent: {key}") from error
    with os.fdopen(descriptor, "rb") as handle:
        metadata = os.fstat(handle.fileno())
        if not stat.S_ISREG(metadata.st_mode) or stat.S_IMODE(metadata.st_mode) & 0o077:
            raise Program018AuthorityError("Program 018 evidence is not private")
    ...
    def publish() -> None:
        if predecessor._exists(root_descriptor, final_key):
            if _evidence_sha256(root_descriptor, final_key) != expected_sha256:
                raise Program018AuthorityError("Program 018 canonical raw dataset differs")
        else:
            try:
                os.link(
                    temp_key,
                ...
                raise Program018AuthorityError("Program 018 canonical raw publish raced") from None
            os.fsync(root_descriptor)
...
def _historical_fingerprinted_artifact(
    ...
    if _HEX_64.fullmatch(str(stored)) is None or stored != fingerprint(unsigned):
        raise Program018AuthorityError(f"Program 018 historical {label} fingerprint differs")
    return value, {
        "path": path.as_posix(),
...
def _validate_historical_source_binding(repository: Path, runtime: Mapping[str, Any]) -> None:
    ...
    if (
        ...
        raise Program018AuthorityError("Program 016 historical runtime binding differs")
    seen: set[str] = set()
    for item in source_files:
        ...
        if not path or path in seen or _HEX_64.fullmatch(str(binding.get("sha256"))) is None:
            raise Program018AuthorityError("Program 016 historical runtime source differs")
        seen.add(path)
...
def _historical_program_017_identity(
    repository: Path,
    proposal: Mapping[str, Any],
    active: Mapping[str, Any],
    program_018_commit: str,
) -> Mapping[str, Any]:
    forensic_binding = _mapping(
        _mapping(proposal.get("predecessor"), "predecessor bindings").get(
            "redacted_private_forensic_disposition"
        ),
        "Program 018 forensic binding",
    )
    forensic = _load_bound(
        repository,
        forensic_binding,
        "forensic_disposition_fingerprint",
        commit=program_018_commit,
    )
    historical = _mapping(forensic.get("predecessor"), "historical Program 017 binding")
    child_binding = _mapping(historical.get("child_authority"), "Program 017 child binding")
    review_binding = _mapping(
        historical.get("child_authority_review"), "Program 017 child review binding"
    )
    sync_commit = str(
        _mapping(active.get("control_lineage"), "Program 017 control lineage").get(
            "synchronized_main_commit"
        )
    )
    if _HEX_40.fullmatch(sync_commit) is None:
        raise Program018AuthorityError("Program 017 synchronized-main binding differs")
    ...
    runtime = _mapping(child.get("runtime_binding"), "Program 017 child runtime binding")
    child_authority = _mapping(child.get("authority"), "Program 017 child authority")
    required_challenges = tuple(
        cast(
            Sequence[str],
            _sequence(child.get("required_review_challenges"), "Program 017 review challenges"),
        )
    )
    ...
    if (
        child.get("child_authority_id") != program_017.CHILD_AUTHORITY_ID
        or child.get("program_ordinal") != program_017.PROGRAM_ORDINAL
        or child.get("program_id") != program_017.PROGRAM_ID
        or child.get("operation_manifest") != program_017.OPERATION_MANIFEST
        ...
        raise Program018AuthorityError("Program 017 historical reviewed child differs")
...
def _derive_predecessor_state(
    ...
    terminal_binding = _mapping(
        predecessor_bindings.get("program_017_terminal_result"),
        "Program 017 public terminal binding",
    )
    ...
    active_raw = predecessor._read(program_017_root, "active-authority.json")
    active = _json_object(active_raw, "Program 017 active authority")
    if active_raw != (canonical_json(active) + "\n").encode():
        raise Program018AuthorityError("Program 017 active authority is not canonical")
    unsigned_active = dict(active)
    active_fingerprint = unsigned_active.pop("authority_fingerprint", None)
    control_lineage = _mapping(active.get("control_lineage"), "Program 017 control lineage")
    runtime = _mapping(active.get("runtime_binding"), "Program 017 runtime binding")
    source_commit = str(control_lineage.get("runtime_source_commit"))
    historical_identity = _historical_program_017_identity(
    ...
    if (
        ...
        raise Program018AuthorityError("Program 017 active authority binding differs")
    sync_commit = str(control_lineage.get("synchronized_main_commit"))
    try:
        program_017.validate_operation_contract(repository, commit=sync_commit)
        program_016_state = program_017._derive_predecessor_state(
            repository,
            program_016_root,
            program_015_root,
            program_014_root,
        ...
        if predecessor._read(program_017_root, "predecessor-import-manifest.json") != (
            program_016_state.payload
        ):
            raise Program018AuthorityError("Program 017 predecessor import manifest differs")
        private_terminal = program_017._load_terminal_record(
            repository,
            program_017_root,
            program_016_root,
            program_015_root,
    ...
    except (program_017.Program017AuthorityError, predecessor.Program012AuthorityError) as error:
        raise Program018AuthorityError(str(error).replace("Program 017", "Program 018")) from None
    if (
        ...
        or program_017._public_terminal_payload(private_terminal)
        != _git(repository, "show", f"{_authority_commit(authority)}:{terminal_binding['path']}")
    ):
        raise Program018AuthorityError("Program 017 terminal disposition differs")

    requests = science.acquisition_requests()
    page_entries = {
        entry for entry in os.listdir(program_017_root) if program_017._PAGE_KEY.fullmatch(entry)
    }
    budget = program_017._Budget(program_016_state)
    completed_sessions: list[dict[str, Any]] = []
    frontier: dict[str, Any] | None = None
    frontier_request_index = -1
    for request_index, request in enumerate(
        requests[program_016_state.frontier_request_index :],
        start=program_016_state.frontier_request_index,
    ):
        session_entries = {
            entry for entry in page_entries if entry.startswith(f"session-{request.session}-")
        }
        if frontier is not None:
            if session_entries:
                raise Program018AuthorityError("Program 017 evidence continues after frontier")
            continue
        source = program_017._ReplaySource(
            program_017_root, request, active, program_016_state, budget
        )
        before_requests = budget.requests
        before_responses = budget.responses
        before_bytes = budget.response_bytes
        try:
            result = program_017._execute_session(request, source)
        except program_017._IncompletePageCheckpoint:
            completed_pages = budget.responses - before_responses
            page_index = completed_pages + 1
            if completed_pages != 1 or page_index != 2:
                raise Program018AuthorityError(
                    "Program 017 reviewed frontier is not page-two intent-only"
                ) from None
            incoming_token = None
            if completed_pages:
                ...
                if (
                    ...
                    raise Program018AuthorityError("Program 017 frontier status differs") from None
            intent = program_011.PageIntent(
            ...
            if session_entries != expected_entries:
                raise Program018AuthorityError("Program 017 frontier checkpoint differs") from None
            frontier = {
                "session": request.session.isoformat(),
                ...
                "url": intent.url,
                "intent_sha256": hashlib.sha256(
                    predecessor._read(program_017_root, f"{prefix}.intent.json")
                ).hexdigest(),
                "body_present": False,
        ...
        except (program_017._TransportRequired, program_017.Program017AuthorityError) as error:
            raise Program018AuthorityError(
                "Program 017 evidence is not the reviewed intent-only frontier"
            ) from error
        except program_011.Program011Error as error:
            raise Program018AuthorityError(
                "Program 017 completed prefix does not reparse"
            ) from error
        else:
            expected_entries = predecessor._expected_session_entries(request, len(result.pages))
            if session_entries != expected_entries:
                raise Program018AuthorityError("Program 017 completed session evidence differs")
            pages = []
                ...
                pages.append(
                    {
                        ...
                        "body_sha256": hashlib.sha256(
                            predecessor._read(program_017_root, f"{prefix}.body")
                        ).hexdigest(),
                        "receipt_sha256": hashlib.sha256(
                            predecessor._read(program_017_root, f"{prefix}.receipt.json")
                        ).hexdigest(),
            ...
            if budget.requests - before_requests != len(result.pages):
                raise Program018AuthorityError("Program 017 completed request count differs")
    if (
        frontier is None
        ...
        raise Program018AuthorityError("Program 017 reviewed recovery frontier is absent")
    if (
        ...
        or budget.requests - budget.responses != 6
    ):
        raise Program018AuthorityError("Program 017 cumulative terminal budget differs")
    runtime_017 = _mapping(authority.get("runtime_binding"), "runtime binding")
    unsigned_manifest: dict[str, Any] = {
        ...
        "runtime_binding": dict(runtime_017),
        ...
        "program_017_predecessor_manifest_sha256": program_016_state.sha256,
        "program_012_completed_sessions": program_016_state.manifest[
            "program_012_completed_sessions"
        ],
        "program_012_frontier": program_016_state.manifest["program_012_frontier"],
        "program_013_public_terminal": program_016_state.manifest["program_013_public_terminal"],
        "program_013_active_authority_fingerprint": program_016_state.manifest[
            "program_013_active_authority_fingerprint"
        ],
        "program_013_child_identity_fingerprint": program_016_state.manifest[
            "program_013_child_identity_fingerprint"
        ],
        "program_013_source_commit": program_016_state.manifest["program_013_source_commit"],
        "program_013_private_terminal_fingerprint": program_016_state.manifest[
            "program_013_private_terminal_fingerprint"
        ],
        "program_013_completed_sessions": program_016_state.manifest[
            "program_013_completed_sessions"
        ],
        "program_014_public_terminal": program_016_state.manifest["program_014_public_terminal"],
        "program_014_active_authority_fingerprint": program_016_state.manifest[
            "program_014_active_authority_fingerprint"
        ],
        "program_014_child_identity_fingerprint": program_016_state.manifest[
            "program_014_child_identity_fingerprint"
        ],
        "program_014_source_commit": program_016_state.manifest["program_014_source_commit"],
        "program_014_private_terminal_fingerprint": program_016_state.manifest[
            "program_014_private_terminal_fingerprint"
        ],
        "program_014_completed_sessions": program_016_state.manifest[
            "program_014_completed_sessions"
        ],
        "program_015_public_terminal": program_016_state.manifest["program_015_public_terminal"],
        "program_015_active_authority_fingerprint": program_016_state.manifest[
            "program_015_active_authority_fingerprint"
        ],
        "program_015_child_identity_fingerprint": program_016_state.manifest[
            "program_015_child_identity_fingerprint"
        ],
        "program_015_source_commit": program_016_state.manifest["program_015_source_commit"],
        "program_015_private_terminal_fingerprint": program_016_state.manifest[
            "program_015_private_terminal_fingerprint"
        ],
        "program_015_completed_sessions": program_016_state.manifest[
            "program_015_completed_sessions"
        ],
        "program_016_public_terminal": program_016_state.manifest["program_016_public_terminal"],
        "program_016_active_authority_fingerprint": program_016_state.manifest[
            "program_016_active_authority_fingerprint"
        ],
        "program_016_child_identity_fingerprint": program_016_state.manifest[
            "program_016_child_identity_fingerprint"
        ],
        "program_016_source_commit": program_016_state.manifest["program_016_source_commit"],
        "program_016_private_terminal_fingerprint": program_016_state.manifest[
            "program_016_private_terminal_fingerprint"
        ],
        "program_016_completed_sessions": program_016_state.manifest[
            "program_016_completed_sessions"
        ],
        "program_017_public_terminal": dict(terminal_binding),
        "program_017_active_authority_fingerprint": active_fingerprint,
        "program_017_child_identity_fingerprint": active["child_identity_fingerprint"],
        "program_017_source_commit": source_commit,
        "program_017_private_terminal_fingerprint": private_terminal["terminal_fingerprint"],
        "program_017_completed_sessions": completed_sessions,
        ...
        "consumed_intent_frontiers": [
            *program_016_state.manifest["consumed_intent_frontiers"],
            frontier,
        ],
        "discarded_program_017_partial_session": frontier,
        "frontier": {
...
def _validate_predecessor_manifest(root_descriptor: int, expected: _PredecessorState) -> None:
    if predecessor._read(root_descriptor, "predecessor-import-manifest.json") != expected.payload:
        raise Program018AuthorityError("Program 018 predecessor import manifest differs")
...
def _append_atomic_or_validate(root_descriptor: int, key: str, payload: bytes) -> None:
    ...
    try:
        ...
        raise Program018AuthorityError(str(error).replace("Program 012", "Program 018")) from None
...
def _load_active(root_descriptor: int, expected: Mapping[str, Any]) -> Mapping[str, Any]:
    ...
    if value != expected or raw != (canonical_json(expected) + "\n").encode():
        raise Program018AuthorityError("Program 018 active authority differs")
    return value
...
def _repository_preflight(repository: Path, identity: Mapping[str, Any]) -> dict[str, str]:
    ...
    try:
        ...
        raise Program018AuthorityError("Program 018 repository identity is unavailable") from error
    if (
        ...
        raise Program018AuthorityError("Program 018 reviewed synchronized-main lineage differs")
    return {
...
def _authority_commit(authority: Mapping[str, Any]) -> str:
    ...
    if _HEX_40.fullmatch(value) is None:
        raise Program018AuthorityError("Program 018 authority policy commit differs")
    return value
...
def _source_commit(authority: Mapping[str, Any]) -> str:
    ...
    if _HEX_40.fullmatch(value) is None:
        raise Program018AuthorityError("Program 018 runtime source commit differs")
    return value
...
def _require_credentials_present(environ: Mapping[str, str] | None) -> None:
    ...
    if missing:
        raise Program018AuthorityError("Program 018 credentials missing: " + ", ".join(missing))
    ...
    *,
    ...
    try:
        ...
        if not fingerprint_optional:
            raise Program018AuthorityError(
                str(error).replace("Program 011", "Program 018")
            ) from None
        path = Path(str(binding.get("path")))
        raw = _git(repository, "show", f"{commit or 'HEAD'}:{path.as_posix()}")
        if hashlib.sha256(raw).hexdigest() != binding.get("sha256"):
            raise Program018AuthorityError("Program 018 bound artifact differs") from None
    ...
    try:
        for part in root.parts:
            ...
            try:
                ...
                raise Program018AuthorityError(
                    "Program 018 private evidence root is absent"
                ) from error
            os.close(descriptor)
            descriptor = child
        metadata = os.fstat(descriptor)
        if not stat.S_ISDIR(metadata.st_mode) or stat.S_IMODE(metadata.st_mode) & 0o077:
            raise Program018AuthorityError("Program 018 private evidence root is not private")
        return descriptor
    except BaseException:
        os.close(descriptor)
        raise
...
def _repository(repository: Path) -> Path:
    if not isinstance(repository, Path):
        raise Program018AuthorityError("Program 018 repository root is invalid")
    resolved = repository.resolve()
    if not resolved.is_dir():
        raise Program018AuthorityError("Program 018 repository root is absent")
    return resolved


def _mapping(value: Any, label: str) -> Mapping[str, Any]:
    if not isinstance(value, Mapping):
        raise Program018AuthorityError(f"Program 018 {label} is invalid")
    return value
...
def _json_object(raw: bytes, label: str) -> dict[str, Any]:
    try:
        ...
        raise Program018AuthorityError(f"Program 018 {label} is invalid JSON") from error
    if type(value) is not dict:
        raise Program018AuthorityError(f"Program 018 {label} is not an object")
    return value
...
def _git(repository: Path, *arguments: str) -> bytes:
    try:
        ...
        raise Program018AuthorityError("Program 018 Git identity is unavailable") from error
