"""Process-parallel runner for the frozen Intraday Exposed 004 campaign."""
...
from .intraday_exposed_004_launch_control import (
...
LAUNCH_CONTROL_RELATIVE_PATH = Path(
    "config/research/intraday-exposed-004-launch-control-review-v1.json"
)
_LAUNCH_CONTROL_SCHEMA = "intraday-exposed-004-launch-control-review-v1"
...
_LAUNCH_CONTROL_POST_REVIEW_FILES = frozenset(
    {
        ...
        "tests/unit/test_intraday_exposed_004_runner.py",
        "CURRENT_STATE.md",
        "DECISIONS.md",
...
class IntradayExposed004Store:
    """004 view over the generic append-only attempt store."""
    ...
    def reserve(self, specifications: Sequence[Mapping[str, object]]) -> None:
        ...
        if len(set(run_ids)) != len(run_ids):
            raise ValueError("Intraday Exposed 004 run specifications collide")
...
class _Worker:
    ...
    def __init__(
        ...
        self.control_plan = load_intraday_exposed_004_plan(self.repository)
        ...
        self.attempt_store = IntradayExposed004Store(runtime_root)
    ...
    def _configuration(self, candidate_id: str) -> Exposed002Configuration:
        for item in self.plan.configurations:
            if item.candidate_id == candidate_id:
                return item
        raise ValueError(f"unknown Intraday Exposed 004 candidate: {candidate_id}")

    def _source_configuration(self, candidate_id: str) -> Exposed002Configuration:
        control = self._control_configuration(candidate_id)
        for item in self.control_plan.source_plan.source_plan.configurations:
            if item.candidate_id == control.source_candidate_id:
                return item
        raise ValueError(
            f"unknown Intraday Exposed 004 source candidate: {control.source_candidate_id}"
        )

    def _control_configuration(self, candidate_id: str) -> Exposed004Configuration:
    ...
    def _period(self, period_id: str) -> Exposed002Period:
        for item in self.plan.periods:
            if item.period_id == period_id:
                return item
        raise ValueError(f"unknown Intraday Exposed 004 period: {period_id}")


class IntradayExposed004Runner(IntradayExposed002Runner):
    ...
    def __init__(
        ...
        *,
        ...
        self.attempt_store = IntradayExposed004Store(self.runtime_root)
    ...
    def _program_binding(self) -> dict[str, object]:
        source = self.control_plan.source_plan.source_plan
        return {
    ...
    def _require_no_failures(self) -> None:
        ...
        if failed:
            raise AttemptStateError(
                f"Intraday Exposed 004 has {len(failed)} terminal failed run(s); "
                "no retry is allowed"
            )
        ...
        specification.update(
            {
                ...
                "source_exposed_003_plan_sha256": self.control_plan.source_plan.sha256,
                "source_exposed_003_plan_fingerprint": (
                    self.control_plan.source_plan.plan_fingerprint
                ),
                "source_plan_sha256": self.control_plan.source_plan.source_plan.sha256,
    ...
    def _execute(self, specifications: Sequence[Mapping[str, object]]) -> None:
        ...
        for specification in specifications:
            ...
            if row["status"] == "completed":
                ...
                raise AttemptStateError(f"Intraday Exposed 004 run is terminal: {run_id}")
            elif row["status"] == "running":
                raise AttemptStateError(f"Intraday Exposed 004 run has an active attempt: {run_id}")
            else:
    ...
    def _source_configuration(self, candidate_id: str) -> Exposed002Configuration:
        ...
        for item in self.control_plan.source_plan.source_plan.configurations:
            if item.candidate_id == source_id:
                return item
        raise ValueError(f"unknown Intraday Exposed 004 source candidate: {source_id}")
    ...
    def _load_report(self, row: Mapping[str, object]) -> Mapping[str, Any]:
        if row.get("status") != "completed":
            raise ValueError("Intraday Exposed 004 run is not completed")
        relative = Path(_required_text(row.get("report_path"), "report path"))
        if relative.is_absolute() or ".." in relative.parts:
            raise ValueError("Intraday Exposed 004 report path is unsafe")
        raw = (self.runtime_root / relative).read_bytes()
        if hashlib.sha256(raw).hexdigest() != row.get("report_sha256"):
            raise ValueError("Intraday Exposed 004 report SHA-256 differs")
        ...
        if (
            ...
            raise ValueError("Intraday Exposed 004 report fingerprint differs")
        return value
    ...
    def _freeze(
        ...
        payload: dict[str, object] = {
            ...
            "june_blocker": {
                "path": "config/research/intraday-exposed-004-june-disposition-v1.json",
    ...
    def _final_report(
        ...
        payload: dict[str, object] = {
            ...
            "terminal_message": (
                "AUTONOMOUS INTRADAY EXPOSED 004 COMPLETE — NO CONTROLLED-QUALIFIED CANDIDATE"
                if empty
                else "AUTONOMOUS INTRADAY EXPOSED 004 TERMINALLY INTERRUPTED"
            ),
            "source_commit": self.source_commit,
    ...
    def _terminal_interruption_report(
        ...
        payload: dict[str, object] = {
            ...
            "terminal_message": "AUTONOMOUS INTRADAY EXPOSED 004 TERMINALLY INTERRUPTED",
            "source_commit": self.source_commit,
    ...
    def _plan_evidence(self) -> dict[str, object]:
        source_003 = self.control_plan.source_plan
...
def _effective_plan(control: IntradayExposed004Plan) -> IntradayExposed002Plan:
    source = control.source_plan.source_plan
...
def _read_only_dataset_services(
    ...
    for binding in bindings:
        ...
        try:
            ...
            raise ValueError(
                f"Intraday Exposed 004 dataset location is missing: {binding.dataset_id}"
            ) from error
...
def _run_id(specification: Mapping[str, object]) -> str:
    return f"ie004r-{fingerprint(specification)[:24]}"


def _reservation_id(run_fingerprint: str) -> str:
    return f"ie004q-{run_fingerprint[:24]}"
...
def _final_markdown(report: Mapping[str, object], json_sha256: str) -> str:
    ...
    lines = [
        "# Intraday Exposed 004 final report",
        "",
...
def _read_final_report(path: Path, *, source_commit: str | None = None) -> Mapping[str, Any]:
    ...
    if (
        ...
        raise ValueError("Intraday Exposed 004 final report differs")
    return value
...
def _validate_final_evidence(runtime: Path, report: Mapping[str, Any]) -> None:
    ...
    if database.get("path") != DATABASE_NAME or database.get("sha256") != _sha256_path(
        ...
        raise ValueError("Intraday Exposed 004 runtime database differs")
    ...
    if relative != "final-freeze.json":
        raise ValueError("Intraday Exposed 004 final freeze path differs")
    ...
    if (
        ...
        raise ValueError("Intraday Exposed 004 final freeze differs")


def _load_launch_control(repository: Path, *, source_commit: str) -> Mapping[str, Any]:
    path = repository / LAUNCH_CONTROL_RELATIVE_PATH
    if not path.is_file():
        raise ValueError("Intraday Exposed 004 launch control review is missing")
    if REVIEWED_LAUNCH_CONTROL_SHA256 is None or REVIEWED_LAUNCH_CONTROL_FINGERPRINT is None:
        raise ValueError("Intraday Exposed 004 launch control review is not hash-bound")
    raw = path.read_bytes()
    if hashlib.sha256(raw).hexdigest() != REVIEWED_LAUNCH_CONTROL_SHA256:
        raise ValueError("Intraday Exposed 004 launch control review SHA-256 differs")
    try:
        value = _mapping(json.loads(raw), "launch control review")
    except json.JSONDecodeError as error:
        raise ValueError("Intraday Exposed 004 launch control review is invalid JSON") from error
    unsigned = dict(value)
    ...
    if (
        ...
        raise ValueError("Intraday Exposed 004 launch control review differs")
    _require_exact_keys(
        value,
        {
            "schema_version",
    ...
    if (
        ...
        raise ValueError("Intraday Exposed 004 launch independent review differs")
...
def _verify_launch_implementation(repository: Path, value: Mapping[str, Any]) -> str:
    ...
    if reviewed_plan != {
        ...
        raise ValueError("Intraday Exposed 004 launch reviewed plan differs")
    ...
    if not isinstance(files, list) or len(files) != len(_LAUNCH_CONTROL_FILES):
        raise ValueError("Intraday Exposed 004 launch implementation files differ")
    for item, expected_path in zip(files, _LAUNCH_CONTROL_FILES, strict=True):
        ...
        if binding.get("path") != expected_path or binding.get("sha256") != _sha256_path(
            ...
            raise ValueError("Intraday Exposed 004 launch implementation file differs")
...
def _verify_launch_source_lineage(
    ...
    try:
        ...
        raise ValueError("Intraday Exposed 004 launch source lineage is unavailable") from error
    ...
    if (
        ...
        raise ValueError("Intraday Exposed 004 launch source lineage differs")
...
def _verify_launch_quality_gates(value: Mapping[str, Any], source_commit: str) -> None:
    ...
    if quality.get("source_commit") != source_commit or not isinstance(results, list):
        raise ValueError("Intraday Exposed 004 launch quality gates differ")
    if len(results) != len(_LAUNCH_CONTROL_QUALITY_GATES):
        raise ValueError("Intraday Exposed 004 launch quality gate count differs")
    for result, command in zip(results, _LAUNCH_CONTROL_QUALITY_GATES, strict=True):
        ...
        if (
            ...
            raise ValueError("Intraday Exposed 004 launch quality gate differs")
...
def _verify_launch_equivalence(value: Mapping[str, Any], source_commit: str) -> None:
    ...
    if (
        ...
        raise ValueError("Intraday Exposed 004 launch equivalence differs")
    ...
    for item in fixtures:
        ...
        if any(
            ...
            raise ValueError("Intraday Exposed 004 launch equivalence fixture differs")
        candidates.add(str(fixture["candidate_id"]))
        scenarios.add(str(fixture["scenario_id"]))
    if len(candidates) < 2 or len(scenarios) < 2:
        raise ValueError("Intraday Exposed 004 launch equivalence fixtures lack design span")
...
def _verify_launch_disposition(value: Mapping[str, Any]) -> None:
    ...
    if (
        ...
        raise ValueError("Intraday Exposed 004 launch Intraday Exposed 003 disposition differs")
    ...
    required = disposition.get("intraday_exposed_004_required")
    ...
    if any(
        ...
        raise ValueError("Intraday Exposed 004 launch Intraday Exposed 003 flags differ")
    expected: tuple[object, ...]
    if classification == "valid-completed-terminal":
        raise ValueError(
            "Intraday Exposed 004 launch is not required after valid Exposed 003 completion"
        )
        ...
        raise ValueError("Intraday Exposed 004 launch Intraday Exposed 003 classification differs")
    observed = (
        terminal,
        valid,
        required,
    ...
    if observed != expected:
        raise ValueError("Intraday Exposed 004 launch Intraday Exposed 003 action differs")
...
def _validated_source_commit(value: object) -> str:
    if (
        ...
        raise ValueError("Intraday Exposed 004 launch source commit differs")
    return value
...
def intraday_exposed_004_plan_summary(repository: Path) -> dict[str, object]:
    ...
    return {
        ...
        "plan_review_sha256": REVIEWED_PLAN_REVIEW_SHA256,
        "plan_review_fingerprint": REVIEWED_PLAN_REVIEW_FINGERPRINT,
        "source_exposed_003_plan_sha256": plan.source_plan.sha256,
        "source_exposed_003_plan_fingerprint": plan.source_plan.plan_fingerprint,
        "source_plan_sha256": plan.source_plan.source_plan.sha256,
        "source_plan_fingerprint": plan.source_plan.source_plan.plan_fingerprint,
...
def run_intraday_exposed_004_campaign(
    ...
    *,
    ...
    return IntradayExposed004Runner(
        repository,
        data_home,
        workers=workers,
        progress=progress,
