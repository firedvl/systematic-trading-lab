...
def parser() -> argparse.ArgumentParser:
    ...
    program_008_metadata = acquire_program.add_parser(
        "program-008-metadata",
        help="control the one-use Program 008 corporate-action metadata qualification",
    )
    ...
    program_010_ohlcv = acquire_program.add_parser(
        "program-010-ohlcv",
    ...
    program_012_ohlcv = acquire_program.add_parser(
        "program-012-ohlcv",
    ...
    program_014_ohlcv = acquire_program.add_parser(
        "program-014-ohlcv",
    ...
    program_016_ohlcv = acquire_program.add_parser(
        "program-016-ohlcv",
        help="control the single-process Program 016 raw SIP recovery and admission",
    )
    program_016_ohlcv.add_argument("action", choices=("credential-preflight", "run"))
    program_017_ohlcv = acquire_program.add_parser(
        "program-017-ohlcv",
        help="control the single-process Program 017 raw SIP recovery and admission",
    )
    program_017_ohlcv.add_argument("action", choices=("credential-preflight", "run"))
    program_018_ohlcv = acquire_program.add_parser(
        "program-018-ohlcv",
...
def _run_program_014_ohlcv(arguments: argparse.Namespace) -> int:
    from .program_014_ohlcv_authority import (
        credential_presence_preflight as program_014_ohlcv_credential_preflight,
    )
    from .program_014_ohlcv_authority import (
        execute_acquisition as execute_program_014_ohlcv,
    )

    repository = Path(__file__).resolve().parents[2]
    if arguments.action == "credential-preflight":
        missing = program_014_ohlcv_credential_preflight(repository)
        print("PASS" if not missing else "\n".join(f"MISSING: {name}" for name in missing))
        return 0 if not missing else 1
    if os.environ.get("TRADING_LAB_MODE", TradingMode.OFFLINE.value).strip() != (
        TradingMode.RESEARCH.value
    ):
        raise ValueError("Program 014 OHLCV acquisition requires TRADING_LAB_MODE=research")
    sys.stdout.buffer.write(execute_program_014_ohlcv(repository).public_payload())
    return 0
...
def _run_program_016_ohlcv(arguments: argparse.Namespace) -> int:
    from .program_016_ohlcv_authority import (
        credential_presence_preflight as program_016_ohlcv_credential_preflight,
    )
    from .program_016_ohlcv_authority import (
        execute_acquisition as execute_program_016_ohlcv,
    )

    repository = Path(__file__).resolve().parents[2]
    if arguments.action == "credential-preflight":
        missing = program_016_ohlcv_credential_preflight(repository)
        print("PASS" if not missing else "\n".join(f"MISSING: {name}" for name in missing))
        return 0 if not missing else 1
    if os.environ.get("TRADING_LAB_MODE", TradingMode.OFFLINE.value).strip() != (
        TradingMode.RESEARCH.value
    ):
        raise ValueError("Program 016 OHLCV acquisition requires TRADING_LAB_MODE=research")
    sys.stdout.buffer.write(execute_program_016_ohlcv(repository).public_payload())
    return 0


def _run_program_017_ohlcv(arguments: argparse.Namespace) -> int:
    from .program_017_ohlcv_authority import (
        credential_presence_preflight as program_017_ohlcv_credential_preflight,
    )
    from .program_017_ohlcv_authority import (
        execute_acquisition as execute_program_017_ohlcv,
    )

    repository = Path(__file__).resolve().parents[2]
    if arguments.action == "credential-preflight":
        missing = program_017_ohlcv_credential_preflight(repository)
        print("PASS" if not missing else "\n".join(f"MISSING: {name}" for name in missing))
        return 0 if not missing else 1
    if os.environ.get("TRADING_LAB_MODE", TradingMode.OFFLINE.value).strip() != (
        TradingMode.RESEARCH.value
    ):
        raise ValueError("Program 017 OHLCV acquisition requires TRADING_LAB_MODE=research")
    sys.stdout.buffer.write(execute_program_017_ohlcv(repository).public_payload())
    return 0


def _run_program_018_ohlcv(arguments: argparse.Namespace) -> int:
    from .program_018_ohlcv_authority import (
        credential_presence_preflight as program_018_ohlcv_credential_preflight,
    )
    from .program_018_ohlcv_authority import (
        execute_acquisition as execute_program_018_ohlcv,
    )

    repository = Path(__file__).resolve().parents[2]
    if arguments.action == "credential-preflight":
        missing = program_018_ohlcv_credential_preflight(repository)
        print("PASS" if not missing else "\n".join(f"MISSING: {name}" for name in missing))
        return 0 if not missing else 1
    if os.environ.get("TRADING_LAB_MODE", TradingMode.OFFLINE.value).strip() != (
        TradingMode.RESEARCH.value
    ):
        raise ValueError("Program 018 OHLCV acquisition requires TRADING_LAB_MODE=research")
    sys.stdout.buffer.write(execute_program_018_ohlcv(repository).public_payload())
    return 0
    ...
    try:
        ...
        if (
            ...
            if arguments.acquire_program == "program-017-ohlcv":
                return _run_program_017_ohlcv(arguments)
            if arguments.acquire_program == "program-016-ohlcv":
                return _run_program_016_ohlcv(arguments)
...
def run(arguments: argparse.Namespace, settings: Settings) -> int:
    ...
    if arguments.data_command == "acquire":
        ...
        if arguments.acquire_program == "program-010-ohlcv":
            ...
            from .program_010_ohlcv_authority import (
                credential_presence_preflight as program_010_ohlcv_credential_preflight,
            )
            from .program_010_ohlcv_authority import (
                execute_qualification as execute_program_010_ohlcv,
            )

            repository = Path(__file__).resolve().parents[2]
            if arguments.action == "credential-preflight":
                missing = program_010_ohlcv_credential_preflight()
                print("PASS" if not missing else "\n".join(f"MISSING: {name}" for name in missing))
                return 0 if not missing else 1
            if settings.mode is not TradingMode.RESEARCH:
                raise ValueError(
                    "Program 010 OHLCV qualification requires TRADING_LAB_MODE=research"
                )
            if arguments.action == "activate":
                _print(activate_program_010_ohlcv_authority(repository))
                return 0
            _print(execute_program_010_ohlcv(repository).public_summary())
            return 0
        ...
        if arguments.acquire_program == "program-008-metadata":
            ...
            from .program_008_corporate_action_authority import (
                credential_presence_preflight as program_008_metadata_credential_preflight,
            )
            from .program_008_corporate_action_authority import (
                execute_qualification as execute_program_008_metadata,
            )
            ...
            if arguments.action == "credential-preflight":
                if arguments.authorization_root is not None:
                    raise ValueError(
                        "Program 008 metadata credential preflight does not accept an "
                        "authorization root"
                    )
                missing = program_008_metadata_credential_preflight()
                print("PASS" if not missing else "\n".join(f"MISSING: {name}" for name in missing))
                return 0 if not missing else 1
            if settings.mode is not TradingMode.RESEARCH:
                raise ValueError(
                    "Program 008 metadata qualification requires TRADING_LAB_MODE=research"
                )
            if arguments.authorization_root is None:
                raise ValueError(
                    "Program 008 metadata activate and run require --authorization-root"
                )
            if arguments.action == "activate":
                _print(
                    activate_program_008_metadata_authority(
                        repository, arguments.authorization_root
                    )
                )
                return 0
            program_008_metadata_result = execute_program_008_metadata(
                repository, arguments.authorization_root
            )
            _print(
                {
                    "response_count": program_008_metadata_result.page_count,
                    "response_bytes": program_008_metadata_result.response_bytes,
                    "event_count": len(program_008_metadata_result.events),
                }
            )
            return 0
