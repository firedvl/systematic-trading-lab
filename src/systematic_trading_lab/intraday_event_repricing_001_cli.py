"""CLI extension for Intraday Event Repricing 001."""
...
from .intraday_event_repricing_001_plan import PROGRAM_ID
from .intraday_event_repricing_001_runner import (
...
def event_repricing_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        prog=f"trading-lab research {PROGRAM_ID}",
        description="Run or inspect the frozen Intraday Event Repricing 001 campaign.",
    )
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    parsed = event_repricing_parser().parse_args(arguments[2:])
    try:
        load_dotenv()
        settings = load_settings()
        if settings.mode not in {TradingMode.OFFLINE, TradingMode.RESEARCH}:
            raise ConfigurationError("Intraday Event Repricing 001 requires research mode")
        if settings.paper_write_request is not None:
            raise ConfigurationError(
                "Intraday Event Repricing 001 cannot run with paper-write opt-in"
            )
        repository = Path(__file__).resolve().parents[2]
        if parsed.action == "plan":
            result = intraday_event_repricing_001_plan_summary(repository)
        elif parsed.action == "status":
            result = intraday_event_repricing_001_status(settings.home)
        else:
            result = run_intraday_event_repricing_001_campaign(
                repository,
                settings.home,
